...
from src_v2.core.updates import InteractionUpdate, InventoryUpdate, ResourceNodeUpdate
        ...
        for e_id, ent_upd in update.entity_updates.items():
            ...
            if ent_upd.interaction.progress_delta >= 0:
                ...
                if new_progress >= node.required_ticks:
                    ...
                    item_weight = InteractionSystem.get_weight(item_kind)
                    
                    if entity.inventory.current_slots_used >= entity.inventory.max_slots:
                        refined_entity_updates[e_id] = replace(
                            ent_upd,
                            interaction=InteractionUpdate(reset=True)
                        )
                    ...
                    if entity.inventory.current_weight + item_weight > entity.inventory.max_weight:
                        # Weight Pressure: Cannot carry
                        refined_entity_updates[e_id] = replace(
                            ent_upd,
                            interaction=InteractionUpdate(reset=True)
                        )
                    ...
                    refined_entity_updates[e_id] = replace(
                        ent_upd,
                        interaction=InteractionUpdate(reset=True),
                        inventory=InventoryUpdate(items_added=[item_kind])
                    )
