...
- `engine/world_loop.py`
  Atomic checklist:
- [x] TOWN-001: Action proposals are typed intents, not direct world mutation.
- [x] TOWN-002: Every gameplay side effect is represented as a typed update bucket (mind, perception, navigation, progression, identity, routine, interaction, spatial, building, social, social-event, reputation, strategic, world, combat-trace).
- [x] TOWN-003: Legacy reason strings and targets are coerced into structured authoritative reason/target models.
- [x] TOWN-004: World mutation happens after proposal generation, not inside worker thought code.
- [x] TOWN-005: Action application supports partial rejection without corrupting unrelated update domains.
- [x] TOWN-006: Conflict resolution preserves one authoritative outcome per tick.
- [x] TOWN-007: Worker decision-making is decoupled from authoritative application.
- [x] TOWN-008: Replay and observability consume authoritative results rather than defining them.
...
- `core/models/combat.py`
  Atomic checklist:
- [x] COMB-001: Manhattan distance is the shared spatial metric for movement and combat range where claimed.
- [x] COMB-002: Cardinal/tile movement and occupancy legality are explicit.
- [x] COMB-003: Occupied-tile movement is rejected or redirected rather than silently overlapped.
- [x] COMB-004: Melee legality depends on adjacency/engagement rules, not raw damage stats.
- [x] COMB-005: Ranged legality depends on range and line-of-sight rules.
- [x] COMB-197: AoE legality is a function of target position and area-of-effect radius.
- [x] COMB-007: World-time progression is distinct from readiness-based action cadence.
- [x] COMB-198: Pathfinding avoids occupied tiles but allows targeting them.
- [x] COMB-199: Movement cost and speed are applied during authoritative application, not in worker proposals.
- [x] COMB-200: Damage calculation math is consistent with legacy rules.
- [x] COMB-201: Movement intentions are distinct from movement execution results.
- [x] COMB-008: Quiet ticks still advance passive world consequences.
- [x] COMB-009: Disengagement, pursuit, target stickiness, and opportunity consequences are explicit rules.
...
- `core/gameplay/items/items.py`
  Atomic checklist:
- [x] TOWN-009: Looting is a channeled state with progress, interruption, and completion semantics.
- [x] TOWN-010: Harvesting is a channeled state tied to nearby resource-node legality and harvest duration.
- [x] TOWN-011: Loot/harvest can abort because of inventory slot pressure.
- [x] TOWN-012: Loot/harvest can abort because of inventory weight pressure.
- [x] TOWN-013: Inventory state tracks both slots and weight/carry burden.
- [x] TOWN-014: Ground items, node yields, and inventory additions/removals are authoritative side effects.
- [x] TOWN-015: Town return is a real gameplay state, not a cosmetic teleport.
- [x] TOWN-016: Shop visits resolve bounded buy/sell behavior using inventory/gold truth.
- [x] TOWN-017: Blacksmith visits resolve recipe/crafting/material-gating behavior.
- [x] TOWN-018: Guild visits produce intel, quests, and material/resource hints.
- [x] TOWN-019: Inn/home/class-hall visits have distinct progression or recovery semantics.
- [x] TOWN-020: Building interactions are explicit gameplay slices, not generic proximity triggers.
...
- [x] STRAT-011: Event interpretation can mutate directives, projects, concerns, and source trust. <!-- RECOVERED: process_outcome and social integration -->
- [x] STRAT-012: Knowledge remains uncertain (leads/candidate zones/hypotheses) until resolved. <!-- RECOVERED: LeadService handles LeadCertainty -->
- [x] STRAT-013: Cognition graph export exposes persisted strategic state without becoming the source of truth. <!-- RECOVERED: CognitionGraphExporter implemented and verified -->
...
- `core/models/social.py`
  Atomic checklist:
- [x] SOC-001: Private betrayal history can override public recruiter reputation.
- [x] SOC-002: Social learning updates familiarity/trust-like bonds from interaction evidence.
- [x] SOC-003: Social contracts and obligations are explicit strategic objects, not flavor text.
- [x] SOC-004: Breaking or honoring contracts has persistent consequences.
- [x] SOC-005: Public reputation is distinct from private narrative meaning.
- [x] SOC-006: Turning points and interpreted life events feed future strategic and social behavior.
- [x] SOC-007: Party/group cooperation is purpose-driven, not just proximity clustering.
- [x] SOC-008: Recruitment evaluates trust, debt, greed, capability fit, and prior trauma.
...
- [x] PROG-001: Attributes have domain ownership and scaling semantics. <!-- ENHANCED: ALLOCATE_AP implemented in DomainLogic with derived stat boosts -->
- [x] PROG-002: Class choice affects starting gear, skills, and progression paths.
- [x] PROG-003: Skill scaling and breakthroughs are explicit progression systems.
- [x] PROG-004: Combat and progression rewards update gold, XP, veterancy, effects, and consequences through authoritative updates. <!-- ENHANCED: EvolutionSystem handles XP-to-Level-to-Stat loop -->
- [x] PROG-005: Items obey contract rules (type, weight, equipment legality, consumable semantics).
- [x] PROG-006: NPC/hero contracts define role/class/gear boundaries.
- [x] PROG-007: Specialization and milestone progression can mutate capability ceilings. <!-- ENHANCED: Milestone AP bonuses every 5 levels -->
- [x] PROG-008: RPG math and synergy rules are tested as stable contracts, not intuition.

### World / entities / regions / spawning / deterministic substrate [100%]
- [x] WORLD-001: Quiet tick consequences work (hunger/sleep decay).
- [x] WORLD-002: Dead entity cleanup and corpse spawning.
- [x] WORLD-003: Corpse decay after fixed duration. <!-- ENHANCED: Implemented in ApplyPath -->
- [x] WORLD-004: Regional hazard damage and starvation effects.
- [x] WORLD-005: Spawn rules are deterministic (seeded generator).
- [x] WORLD-006: Regional trauma and stability recovery over time.
...
- `systems/world/generator.py`
  Atomic checklist:
- [x] SUB-001: World generation is deterministic under seed and domain-specific RNG use.
- [x] SUB-002: Town, sanctuary, camps, buildings, corpses, and entities are authoritative world objects.
- [x] SUB-003: Entity snapshots are immutable enough for worker reasoning and deterministic replay.
- [x] SUB-004: No hidden mutation leaks occur from snapshot or AI evaluation paths.
- [x] SUB-005: Deterministic replay/delta behavior is preserved across runs with same seed.
- [x] SUB-006: Regional hazards, calamities, local scars, and world consequences can feed gameplay and strategy. <!-- RECOVERED: EnvironmentService and TransformationService -->
- [x] SUB-007: Entity builder and serialization preserve gameplay-relevant state safely.
- [x] SUB-008: Engine phase order preserves gameplay semantics and subsystem tick integrity.
...
- [x] COMB-014: `test_reactive_cover_seeking`: Reactive cover seeking — Verify that actor seeks cover only when a ranged threat is visible..
- [x] COMB-015: `test_chokepoint_holding`: Chokepoint holding — Verify that actor identifies and holds a 1-tile gap..
- [x] COMB-016: `test_cardinal_opposite_bracketing`: Cardinal opposite bracketing — Verify that two allies bracket a target from opposite sides..
...
- [x] INFRA-001: `test_rejection_audit_aggregation`: Rejection audit aggregation — Verify that authoritative rejections are captured in ScenarioReport. [Milestone 7].
- [x] INFRA-002: `test_out_of_range_rejection`: Out of range rejection — Verify that combat out-of-range is explicitly rejected with structured reason. [Milestone 7].
...
- [x] COMB-027: `test_check_range`: Check range — Verify range enforcement..
- [x] COMB-028: `test_check_occupancy`: Check occupancy — Verify 1-unit-per-tile occupancy rule..
- [x] COMB-029: `test_aoe_legality`: Aoe legality — Verify AoE impact constraints..
- [x] COMB-030: `test_aoe_splash_radius`: Aoe splash radius — Verify entities affected by splash radius..
- [x] COMB-031: `test_get_occupant_id`: Get occupant id — Verify occupant lookup..
- [x] COMB-032: `test_check_targeting_legality`: Check targeting legality — Verify consolidated targeting rules (Range + LOS)..

#### `combat/test_engagement_contract.py`

- [x] COMB-033: `test_engagement_detection`: Engagement detection.
- [x] COMB-034: `test_engagement_clears_on_separation`: Engagement clears on separation.
- [x] COMB-035: `test_engagement_respects_hostility`: Engagement respects hostility.

#### `combat/test_opportunity_attacks.py`

- [x] COMB-036: `test_oa_triggered_on_disengagement`: Oa triggered on disengagement.
- [x] COMB-037: `test_oa_not_triggered_if_staying_engaged_with_same_attacker`: Oa not triggered if staying engaged with same attacker.

#### `combat/test_target_stickiness.py`

- [x] COMB-038: `test_target_stickiness_bias`: Target stickiness bias.

#### `combat/test_world_time_progression.py`

- [x] COMB-039: `test_passive_progression_on_quiet_tick`: Passive progression on quiet tick — Verify that biological decay and lifecycle systems run even if entity doesn't act..
- [x] COMB-040: `test_hero_lifecycle_on_quiet_tick`: Hero lifecycle on quiet tick — Verify that HeroLifecycle (e.g. proximity bonding) runs even if no entity acts..

#### `engine/test_quiet_tick_integrity.py`

- [x] SUB-010: `test_scenario_1_dead_world_progression`: Scenario 1 dead world progression — Scenario 1: No living entities. Verify tick still increments and systems advance..
- [x] SUB-011: `test_scenario_2_sleeping_world_biological_decay`: Scenario 2 sleeping world biological decay — Scenario 2: All entities have high next_act_at. Verify biological decay hits..
- [x] SUB-012: `test_scenario_3_stationary_world_proximity_bonding`: Scenario 3 stationary world proximity bonding — Scenario 3: Two heroes are stationary. Verify bonding occurs via HeroLifecycleSystem..
- [x] SUB-013: `test_scenario_4_subsystem_advancement`: Scenario 4 subsystem advancement — Scenario 4: Verify that registered subsystems receive the tick signal even if no actions apply..

#### `integration/ai/test_wind_pillar_navigation.py`

- [x] SUB-014: `test_navigation_uses_flow_field_for_far_town`: Navigation uses flow field for far town.
- [x] SUB-015: `test_navigation_uses_astar_for_near_target`: Navigation uses astar for near target.
- [x] SUB-016: `test_navigation_uses_flow_field_for_world_boss`: Navigation uses flow field for world boss.

#### `test_party_tactics.py`

- [x] COMB-041: `test_vanguard_biases`: Vanguard biases.
- [x] COMB-042: `test_support_biases`: Support biases.
- [x] COMB-043: `test_protector_biases`: Protector biases.

#### `unit/ai/test_skirmish.py`

- [x] COMB-044: `test_skirmish_boosts_move_for_ranged`: Skirmish boosts move for ranged.
- [x] COMB-045: `test_skirmish_does_not_boost_melee`: Skirmish does not boost melee.

#### `unit/ai/test_tactical_behavior_contract.py`

- [x] SOC-020: `test_melee_striker_closes_distance`: Melee striker closes distance.
- [x] SOC-021: `test_ranged_skirmisher_kites_when_close`: Ranged skirmisher kites when close.
- [x] SOC-022: `test_ranged_skirmisher_maintains_distance`: Ranged skirmisher maintains distance.
- [x] SOC-023: `test_safe_shot_detection`: Safe shot detection.
- [x] SOC-024: `test_tactical_retreat_at_low_hp`: Tactical retreat at low hp.
- [x] SOC-025: `test_group_spacing_preservation`: Group spacing preservation.

#### `unit/core/logic/test_movement_model.py`

- [x] COMB-046: `test_movement_model_basic_path`: Movement model basic path.
- [x] COMB-047: `test_movement_model_yielding_priority`: Movement model yielding priority.
- [x] COMB-048: `test_movement_model_stuck_threshold`: Movement model stuck threshold.

### Resource interaction / inventory / town loop

#### `integration/gameplay/test_toughness_decay.py`

- [x] SOC-042: `test_near_death_hardening`: Near death hardening — Verify that surviving at low HP increases Max HP..
- [x] TOWN-022: `test_stat_decay_inactivity`: Stat decay inactivity — Verify that stat decay can be triggered..
- [x] TOWN-023: `test_toughness_hardening_integration`: Toughness hardening integration — Integration test for the restored hardening logic in CombatAction..

#### `test_building_unification.py`

- [x] TOWN-024: `test_actor`: Actor.
- [x] TOWN-025: `test_visit_guild_no_legacy_goals`: Visit guild no legacy goals — Verify that visiting the guild produces StrategicUpdate and PerceptionUpdate, but no string goals..
- [x] TOWN-026: `test_visit_blacksmith_blocker_emission`: Visit blacksmith blocker emission — Verify that visiting the blacksmith without materials generates a BlockerRecord, not a string state..
- [x] TOWN-027: `test_visit_class_hall_resolution`: Visit class hall resolution — Verify that learning a skill emits a strategic resolution for the corresponding capability blocker..
- [x] TOWN-028: `test_visit_blacksmith_crafting_resolution`: Visit blacksmith crafting resolution — Verify that crafting an item emits a strategic resolution for the material blocker..
- [x] TOWN-029: `test_visit_home_upgrade_resolution`: Visit home upgrade resolution — Verify that home storage upgrade emits a strategic resolution for home maintenance..
- [x] TOWN-030: `test_detour_suggestion_lifecycle_awareness`: Detour suggestion lifecycle awareness — Verify DetourSuggestionService ignores exhausted leads and prioritizes untested ones..

#### `unit/ai/test_routine_cycle.py`

- [x] TOWN-031: `test_biological_decay_authoritative`: Biological decay authoritative.
- [x] TOWN-032: `test_sleep_goal_utility_at_night`: Sleep goal utility at night.
- [x] TOWN-033: `test_nocturnal_predator_bonus`: Nocturnal predator bonus.

#### `unit/ai/test_routine_needs.py`

- [x] TOWN-034: `test_biological_utility_biasing`: Biological utility biasing.
- [x] TOWN-035: `test_inn_visit_leads_to_sleeping`: Inn visit leads to sleeping.
- [x] TOWN-036: `test_home_visit_leads_to_eating`: Home visit leads to eating.
- [x] TOWN-037: `test_sleeping_recovery_cycle`: Sleeping recovery cycle.

#### `unit/core/gameplay/test_item_contracts.py`

- [x] SOC-026: `test_weapon_ranges_integrity`: Weapon ranges integrity — Verify that specific weapons have their intended ranges in the registry..
- [x] SOC-027: `test_weapon_power_integrity`: Weapon power integrity — Verify that core progression weapons have their primary power correctly set..
- [x] SOC-028: `test_registry_identity_integrity`: Registry identity integrity — Ensure all core items are successfully loaded and have consistent IDs..

#### `unit/systems/test_difficulty_scaling.py`

- [x] SOC-029: `test_tier1_is_baseline`: Tier1 is baseline.
- [x] SOC-030: `test_tier4_has_higher_stats_than_tier1`: Tier4 has higher stats than tier1 — Same seed, same enemy tier - tier 4 difficulty should have higher HP/ATK..
- [x] SOC-031: `test_tier4_hp_significantly_higher`: Tier4 hp significantly higher — Tier 4 HP multiplier is 4.0x on base stats; with flat bonuses from traits/attributes the effective ratio will be lower but still substantial..
- [x] SOC-032: `test_difficulty_sets_level_range`: Difficulty sets level range — Entities in tier 3 should have level in [5, 10]..
- [x] SOC-033: `test_gold_scales_with_difficulty`: Gold scales with difficulty — Tier 4 gold multiplier is 4.0x..
- [x] SOC-034: `test_race_tier4_stronger_than_tier1`: Race tier4 stronger than tier1.
- [x] SOC-035: `test_race_difficulty_tier_set`: Race difficulty tier set.
- [x] SOC-036: `test_race_level_in_range`: Race level in range.
- [x] SOC-037: `test_all_races_scale`: All races scale — All four races should scale with difficulty..
- [x] SOC-038: `test_boss_diff_capped_at_4`: Boss diff capped at 4.
- [x] SOC-039: `test_boss_diff_adds_one`: Boss diff adds one.
- [x] SOC-040: `test_spawn_default_is_tier1`: Spawn default is tier1.
- [x] SOC-041: `test_spawn_race_default_is_tier1`: Spawn race default is tier1.

#### `unit/systems/test_toughness_decay.py`

- [x] SOC-042: `test_near_death_hardening`: Near death hardening — Verify that surviving at low HP increases Max HP..
- [x] SOC-043: `test_stat_decay_inactivity`: Stat decay inactivity — Verify that idling for 1000+ ticks triggers stat decay..

### Strategic mind / cognition / projects / blockers / leads

#### `ai/test_bounded_blockers.py`

- [x] STRAT-014: `test_accurate_diagnosis_high_wisdom`: Accurate diagnosis high wisdom.
- [x] STRAT-015: `test_misdiagnosis_low_wisdom`: Misdiagnosis low wisdom.

#### `ai/test_bounded_detours.py`

- [x] STRAT-016: `test_detour_breadth_limit`: Detour breadth limit.
- [x] STRAT-017: `test_detour_depth_limit_fallback`: Detour depth limit fallback.
- [x] STRAT-018: `test_retry_suppression`: Retry suppression.

#### `ai/test_bounded_objective_continuity.py`

- [x] STRAT-019: `test_objective_derivation_precedence_blocker_first`: Objective derivation precedence blocker first.
- [x] STRAT-020: `test_objective_derivation_precedence_active_objective_if_no_blocker`: Objective derivation precedence active objective if no blocker.
- [x] STRAT-021: `test_objective_derivation_precedence_first_unresolved_if_no_active`: Objective derivation precedence first unresolved if no active.

#### `ai/test_bounded_project_continuity.py`

- [x] STRAT-022: `test_project_retention_when_rival_is_below_margin`: Project retention when rival is below margin.
...
- [x] STRAT-037: `test_ui_contract_alignment`: Ui contract alignment — Verify that every field in bounded_cognition_ui_contract.md exists in Pydantic schemas..
- [x] STRAT-038: `test_feature_spec_replay_alignment`: Feature spec replay alignment — Verify that replay fields mentioned in feature spec are present in recorder..
- [x] STRAT-039: `test_feature_spec_graph_export_alignment`: Feature spec graph export alignment — Verify that graph export fields mentioned in feature spec are present in exporter..
- [x] STRAT-040: `test_test_matrix_existence`: Test matrix existence — Verify that all test modules mentioned in test_matrix.md actually exist..
- [x] STRAT-041: `test_populated_artifact_consistency`: Populated artifact consistency — Verify that a live HeadlessRunner execution produces populated and consistent artifacts. [TRACK 1 HARDENING].
- [x] STRAT-042: `test_truth_surface_parity`: Truth surface parity — Verify that Replay, API Schema, and Cognition Graph maintain strict parity. <!-- RECOVERED: StatePresenter and CanonicalStateHasher maintain strict parity -->
- [x] STRAT-043: `test_documentation_alignment`: Documentation alignment — Verify that documented fields in intel_capacity_implementation_updated.md are real. [MILESTONE 8 PROOF].

#### `ai/test_directive_mutation_thresholds.py`

- [x] STRAT-044: `test_directive_mutation_thresholds`: Directive mutation thresholds — Verify that directives only mutate after repeated thresholded events..

#### `ai/test_event_interpretation.py`

- [x] STRAT-045: `test_stable_concern_generation`: Stable concern generation.
- [x] STRAT-046: `test_unstable_panic_concern`: Unstable panic concern.
- [x] STRAT-047: `test_interruption_resistance_stable`: Interruption resistance stable.
- [x] STRAT-048: `test_interruption_resistance_unstable`: Interruption resistance unstable.
- [x] STRAT-049: `test_identity_drift_resistance`: Identity drift resistance.
- [x] STRAT-050: `test_rumor_sensitivity_unstable`: Rumor sensitivity unstable.

#### `ai/test_lead_learning.py`

- [x] STRAT-051: `test_learning_success`: Learning success.
- [x] STRAT-052: `test_learning_failure`: Learning failure.

#### `ai/test_social_cognition.py`

- [x] STRAT-053: `test_social_blocker_detection_solo`: Social blocker detection solo.
- [x] STRAT-054: `test_social_misjudgment_low_stability`: Social misjudgment low stability.
- [x] STRAT-055: `test_social_bandwidth_pool_limiting`: Social bandwidth pool limiting.

#### `ai/test_source_trust_learning_loop.py`

- [x] SOC-044: `test_source_trust_learning_loop`: Source trust learning loop — Prove that future weighting is affected by source trust after a learning event..

#### `ai/test_uncertainty_resolution_loop.py`

- [x] SOC-045: `test_uncertainty_resolution_loop`: Uncertainty resolution loop — Prove that proximity to a rumored zone resolves imprecise leads into precise targets..

#### `core/test_cognition_graph_exporter.py`

- [x] STRAT-056: `test_export_empty_strategy`: Export empty strategy — Verify export from an entity with no strategic state..
- [x] STRAT-057: `test_export_with_core_strategic_state`: Export with core strategic state — Verify export of directives, projects, and objectives..
- [x] STRAT-058: `test_export_determinism`: Export determinism — Verify that multiple exports from the same state are identical..
- [x] STRAT-059: `test_non_mutation`: Non mutation — Verify that exporter does not mutate the source entity..

#### `core/test_strategy_models.py`

- [x] STRAT-060: `test_strategic_model_rebuild`: Strategic model rebuild — Verify pydantic model rebuild handles recursive refs..
- [x] STRAT-061: `test_directive_creation`: Directive creation.
- [x] STRAT-062: `test_strategic_state_defaults`: Strategic state defaults.
- [x] STRAT-063: `test_strategic_state_serialization`: Strategic state serialization.

#### `integration/strategy/test_cognition_graph_regression.py`

- [x] STRAT-064: `test_cognition_graph_deterministic_simulation`: Cognition graph deterministic simulation — Verify that a simulation produces a valid, repeatable cognition graph..
- [x] STRAT-065: `test_graph_structural_invariants`: Graph structural invariants — Verify that the graph follows structural rules across ticks..

#### `integration/strategy/test_strategic_brain_integration.py`

- [x] STRAT-066: `test_strategic_pivot_on_regional_danger`: Strategic pivot on regional danger — Verify that heroes pivot from personal quests to regional stabilization during high-danger events..
- [x] STRAT-067: `test_scar_detection`: Scar detection — Verify that heroes sense nearby world trauma (scars) and investigate..
- [x] STRAT-068: `test_near_death_triggers_survival_consequences`: Near death triggers survival consequences — Verify that a NEAR_DEATH event generates a concern and suspends the current project via applicator..
- [x] STRAT-069: `test_betrayal_mutates_directives`: Betrayal mutates directives — Verify that a salient betrayal turning point adds an 'Avenge' directive..
- [x] STRAT-070: `test_divergent_home_response`: Divergent home response — Verify that only entities with place attachment react strongly to home damage..
- [x] STRAT-071: `test_betrayal_trauma_affects_recruitment`: Betrayal trauma affects recruitment — Verify that a recent betrayal makes entities less willing to accept recruitment offers..

#### `integration/strategy/test_strategic_capacity_enforcement.py`

- [x] STRAT-072: `test_budget_enforcement_truncation`: Budget enforcement truncation — Verify that candidate_zone_limit correctly truncates the pool AND preserves highest-scored zones..
- [x] STRAT-073: `test_source_trust_behavioral_impact`: Source trust behavioral impact — Verify that updating source trust results in different weighting in the next cycle. [MILESTONE 3].
- [x] STRAT-074: `test_overload_metrics_visibility`: Overload metrics visibility — Verify that primary_overload_source and metrics are populated when stressed..

#### `integration/strategy/test_strategic_continuity.py`

- [x] STRAT-075: `test_directive_mutation_salience_threshold`: Directive mutation salience threshold — Verify that only high-salience turning points trigger mutations..
- [x] STRAT-076: `test_directive_priority_strengthening`: Directive priority strengthening — Verify that repeated high-salience events strengthen directive priority..

#### `integration/strategy/test_strategic_continuity_hardening.py`

- [x] STRAT-077: `test_strategic_objective_continuity`: Strategic objective continuity — Prove that an existing objective is preserved if the project remains stable and no high blockers appear..
- [x] STRAT-078: `test_objective_resumption_aligns_with_tactical`: Objective resumption aligns with tactical — Verify that a resumed objective correctly drives goal selection..

#### `integration/strategy/test_strategic_determinism.py`

- [x] SUB-020: `test_harness_determinism`: Harness determinism — Verify that two runs with the same seed produce byte-identical results..
- [x] SUB-021: `test_harness_non_determinism_different_seed`: Harness non determinism different seed — Verify that different seeds produce different outcomes (basic sanity check)..

#### `integration/strategy/test_strategic_explainability.py`

- [x] STRAT-079: `test_candidate_zone_enforcement`: Candidate zone enforcement — Verify that candidate_zone_limit is enforced and drops excess zones..
- [x] STRAT-080: `test_ally_evaluation_enforcement`: Ally evaluation enforcement — Verify that ally_evaluation_limit caps contracts and offers evaluated..
- [x] STRAT-081: `test_overload_source_trauma`: Overload source trauma — Verify that heavy HP damage triggers 'trauma' as the primary overload source..
- [x] STRAT-082: `test_switch_reason_transparency`: Switch reason transparency — Verify that a project switch provides a human-readable reason..

#### `integration/strategy/test_strategic_persistence.py`

- [x] STRAT-083: `test_persistence_boost_prevents_switching`: Persistence boost prevents switching — Verify that the persistence boost prevents switching to a slightly better project..
- [x] STRAT-084: `test_project_lock_prevents_switching`: Project lock prevents switching — Verify that project_lock_until strictly prevents any switches despite critical concerns..
- [x] STRAT-085: `test_interruption_threshold_overridden_by_major_threat`: Interruption threshold overridden by major threat — Verify that a massive threat CAN overcome the interruption threshold..
- [x] STRAT-086: `test_strategic_pipeline_home_threat`: Strategic pipeline home threat — Verify the flow from life event through StrategicConsequenceService to project pivot..
- [x] STRAT-087: `test_resume_restores_valid_objective`: Resume restores valid objective — Verify that brain restores the last active objective when resuming a project. [Strategy M2].
- [x] STRAT-088: `test_resumed_objective_survives_cycle`: Resumed objective survives cycle — Verify that a restored objective doesn't immediately flip back to ProjectRecord.objectives[0] if it matches. [Strategy M2].

#### `integration/strategy/test_strategic_replay_determinism.py`

- [x] SUB-022: `test_world_strategic_registry_deep_isolation`: World strategic registry deep isolation — Verify that WorldStrategicRegistry.copy() performs a deep copy..
- [x] SUB-023: `test_strategic_replay_graph_equality`: Strategic replay graph equality — Verify that replaying from a snapshot yields bit-identical cognition graphs..
- [x] SUB-024: `test_lead_outcome_grounding_verification`: Lead outcome grounding verification — Verify that precise leads correctly ground into world entities..

#### `integration/strategy/test_strategic_resume_objective.py`

- [x] STRAT-089: `test_objective_resume_reliability`: Objective resume reliability — Verify that a suspended objective is resumed correctly..
...
- [x] SOC-067: `test_memory_salience_retention`: Memory salience retention.
...
- [x] STRAT-127: `test_cognition_api_serialization`: Cognition api serialization — Verify that cognitive metrics are correctly serialized for the API..
...
- [x] INFRA-017: Worker pool falls back to inline/local execution when broker transport is unavailable.
- [x] INFRA-018: Worker pool does not require live RabbitMQ/Kafka to execute local simulation behavior.
...
- [x] TOWN-057: `test_sleep_goal_utility_at_night`: sleep utility at night
- [x] TOWN-058: `test_rest_to_sleep_transition`: rest-to-sleep transition
- [x] TOWN-059: `test_routine_service_sleep_bias`: sleep bias
- [x] TOWN-060: `test_routine_service_forced_rest_during_off_hours`: forced off-hours rest
- [x] TOWN-061: `test_routine_service_hunger_bias`: hunger bias
- [x] TOWN-062: `test_home_visit_leads_to_eating`: eating behavior from routine/home visit
- [x] TOWN-063: `test_inn_visit_leads_to_sleeping`: inn visit leads to sleeping
- [x] TOWN-064: `test_biological_decay_and_forced_sleep`: biological decay and forced sleep
- [x] TOWN-065: `test_sleeping_recovery_cycle`: sleeping recovery cycle
- [x] TOWN-066: `test_hunger_reduces_stability`: hunger consequence
- [x] TOWN-067: `test_routine_goal_priority`: routine goal priority
- [x] TOWN-068: `test_routine_disruption_panic`: disruption panic
- [x] TOWN-069: `test_attack_disruption_suppresses_routines`: attack suppresses routine
- [x] TOWN-070: `test_routine_priority_archetype_bias`: archetype-based routine bias
- [x] TOWN-071: `test_life_stage_priority_shift`: life-stage priority shift
...
- [x] COMB-093: `test_ai_boredom_diversification`: boredom diversification effect
- [x] COMB-094: `test_boredom_modifier_applies_multipliers`: boredom modifier law
- [x] COMB-095: `test_life_stage_modifier_early_bracket`: life-stage modifier law
- [x] COMB-096: `test_motive_modifier_biases_explore`: motive modifier explore bias
- [x] COMB-097: `test_motive_modifier_biases_rest`: motive modifier rest bias
- [x] COMB-098: `test_motive_modifier_biases_flee`: motive modifier flee bias
- [x] COMB-099: `test_goal_evaluator_uses_modifiers`: goal evaluator modifier integration
- [x] COMB-100: `test_softmax_distribution`: score-to-choice distribution
- [x] COMB-101: `test_softmax_with_equal_scores`: equal-score handling
...
- [x] SOC-135: `test_locational_trauma_triggers_dread`: locational trauma -> dread
- [x] SOC-136: `test_emotional_bias_on_utility`: emotion changes utility
- [x] SOC-137: `test_emotional_decay`: emotional decay
- [x] SOC-138: `test_narrative_memory_trauma_biasing`: trauma memory biasing
- [x] SOC-139: `test_narrative_memory_victory_confidence`: victory-confidence memory
- [x] SOC-140: `test_narrative_memory_logging`: narrative memory logging
- [x] SOC-141: `test_social_appraisal_with_narrative`: narrative-informed social appraisal
- [x] SOC-142: `test_bravery_modifiers`: bravery modifiers
...
- [x] COMB-102: `test_wound_infliction_massive_hit`: wound infliction
- [x] COMB-103: `test_wound_stat_impact`: wound stat penalties
- [x] COMB-104: `test_scar_permanence`: scar permanence
- [x] COMB-105: `test_scar_decay`: scar decay behavior if preserved
- [x] COMB-106: `test_local_scar_record`: local scar record
- [x] COMB-107: `test_scar_detection`: scar detection
- [x] COMB-108: `test_ai_perception_of_scars`: perception of scars
- [x] COMB-109: `test_stamina_drain_on_attack`: stamina drain on attack
- [x] COMB-110: `test_stamina_decreases_on_move`: stamina drain on move
- [x] COMB-111: `test_stamina_decreases_on_harvest`: stamina drain on harvest
- [x] COMB-112: `test_skill_use_costs_stamina`: stamina cost on skill use
- [x] COMB-113: `test_stamina_regen_resting`: rest stamina regen
- [x] COMB-114: `test_stamina_regen_active`: active regen
- [x] COMB-115: `test_stamina_regen_capped`: regen cap
- [x] COMB-116: `test_exhaustion_penalty_application`: exhaustion penalty
- [x] COMB-117: `test_best_ready_skill_skips_insufficient_stamina`: skill gating by stamina
...
- [x] COMB-118: `test_execution_phase_modifies_proposal_with_aggressive_style`: aggressive action style
- [x] COMB-119: `test_execution_phase_modifies_proposal_with_evasive_style`: evasive action style
- [x] COMB-120: `test_flanking_bonus`: flanking bonus
- [x] COMB-121: `test_no_flanking_bonus_when_facing_attacker`: facing-sensitive flanking exclusion
- [x] COMB-122: `test_shatter_combo`: combo behavior
- [x] COMB-123: `test_ranged_hero_kites_when_adjacent`: ranged kiting
- [x] COMB-124: `test_ranged_skirmisher_kites_when_close`: ranged skirmish spacing
...
- [x] TOWN-072: `test_luck_impacts_loot_modifier`: loot modifier from luck/perception
- [x] TOWN-073: `test_per_based_hidden_discovery`: hidden discovery from perception
- [x] TOWN-074: `test_loot_recovery_consistency`: loot recovery consistency
- [x] TOWN-075: `test_loot_no_duplication`: no duplicated loot
- [x] TOWN-076: `test_corpse_loot_convergence`: corpse loot convergence
- [x] TOWN-077: `test_loot_and_respawn`: loot and respawn interaction
- [x] TOWN-078: `test_loot_tables_exist`: loot table integrity
- [x] TOWN-079: `test_full_bag_aborts_looting`: abort looting when full
- [x] TOWN-080: `test_overweight_aborts_looting`: abort looting when overweight
- [x] TOWN-081: `test_near_weight_limit_penalizes_loot`: weight-limit penalty on looting
...
- [x] STRAT-137: `test_aging_and_death`: aging and death
- [x] STRAT-138: `test_hero_lifecycle_system_permadeath`: hero permadeath
- [x] STRAT-139: `test_permadeath_succession_and_heirlooms`: succession and heirlooms
- [x] STRAT-140: `test_hero_death_creates_scar`: death scar consequences
- [x] STRAT-141: `test_near_death_triggers_survival_consequences`: near-death survival consequences
- [x] STRAT-142: `test_near_death_hardening`: near-death hardening
- [x] STRAT-143: `test_nemesis_recognition_and_fear_bias`: nemesis recognition and fear bias
- [x] STRAT-144: `test_nemesis_milestone_creation`: nemesis milestone creation
- [x] STRAT-145: `test_locational_memory_on_death`: locational memory on death
- [x] STRAT-146: `test_influence_shifts_on_monster_death`: influence shift on monster death
- [x] STRAT-147: `test_influence_shifts_on_hero_death`: influence shift on hero death
...
- [x] STRAT-148: `test_accurate_diagnosis_high_wisdom`: accurate diagnosis at high wisdom
- [x] STRAT-149: `test_misdiagnosis_low_wisdom`: misdiagnosis at low wisdom
...
- [x] STRAT-150: `test_quest_creation`: quest creation baseline
- [x] STRAT-151: `test_quest_advance`: quest progression increments correctly
- [x] STRAT-152: `test_quest_advance_does_nothing_when_completed`: completed quests do not advance further
- [x] STRAT-153: `test_quest_progress_ratio`: progress-ratio computation is correct
- [x] STRAT-154: `test_generate_quest_returns_quest`: quest generator returns valid quest object
- [x] STRAT-155: `test_generate_quest_respects_level`: generated quests respect level banding
- [x] STRAT-156: `test_generate_quest_skips_duplicate`: duplicate quest generation is suppressed
- [x] STRAT-157: `test_generate_quest_gold_scales_with_level`: quest gold reward scales with level
- [x] STRAT-158: `test_generate_explore_quest`: explore-quest generation works
- [x] STRAT-159: `test_hunt_quest_completion_awards_rewards`: hunt-quest completion awards rewards
- [x] STRAT-160: `test_explore_quest_completes_near_target`: explore-quest completes near target
- [x] STRAT-161: `test_gather_quest_advance`: gather-quest progression works
- [x] STRAT-162: `test_dynamic_liberate_quest`: liberate-quest generation/progression works
- [x] STRAT-163: `test_territory_conquest`: territory-conquest quest/world objective behavior is preserved
...
- [x] SUB-123: `test_trait_serialization`: trait serialization is preserved
- [x] STRAT-165: `test_get_traits`: trait retrieval works
- [x] STRAT-166: `test_trait_defs_not_empty`: trait definitions exist and are non-empty
- [x] STRAT-167: `test_with_traits_assigns_traits`: explicit trait assignment works
- [x] STRAT-168: `test_no_traits_by_default`: no-trait default behavior is preserved
- [x] STRAT-169: `test_traits_with_different_race_prefix`: race-prefixed trait handling is preserved
- [x] STRAT-170: `test_empty_traits_returns_zero_bonus`: empty-trait bonus behavior is preserved
- [x] STRAT-171: `test_single_known_trait`: single-trait bonus behavior is preserved
- [x] STRAT-172: `test_multiple_traits_sum`: multiple traits stack/sum correctly
- [x] STRAT-173: `test_unknown_trait_id_ignored`: unknown traits are ignored safely
- [x] STRAT-174: `test_same_trait_compatible`: trait compatibility logic is preserved
- [x] STRAT-175: `test_assigns_between_2_and_4_traits`: random/default trait assignment count is preserved
- [x] STRAT-176: `test_all_assigned_traits_are_valid`: assigned traits are always valid
- [x] SUB-156: `test_all_trait_types_have_definitions`: all trait types have definitions
- [x] STRAT-178: `test_trait_defs_have_all_utility_fields`: trait utility fields are complete
- [x] STRAT-179: `test_trait_defs_have_all_stat_fields`: trait stat fields are complete
...
- [x] PROG-053: `test_place_attachment_instantiation`: place attachment can be instantiated
- [x] PROG-054: `test_place_attachment_navigation`: place attachment influences navigation correctly
- [x] PROG-055: `test_place_attachment_home_navigation`: home-oriented navigation behavior is preserved
- [x] PROG-056: `test_no_home_returns_false`: no-home logic is preserved
- [x] PROG-057: `test_home_sets_home_pos`: home position assignment is preserved
- [x] PROG-058: `test_entity_copy_includes_home_storage`: home storage is preserved during copy
- [x] PROG-059: `test_entity_without_home_storage`: no-home-storage case is preserved
- [x] PROG-060: `test_divergent_home_response`: divergent home response behavior is explicit and preserved where intended
- [x] PROG-061: `test_home_priority_retreat`: home-priority retreat behavior is preserved
- [x] PROG-062: `test_visit_home_upgrade_resolution`: visit-home upgrade resolution is preserved
- [x] PROG-063: `test_home_visit_leads_to_eating`: home visit can lead to eating behavior
...
- [x] WORLD-001: `test_flow_field_basic_navigation`: basic flow-field navigation is preserved
- [x] WORLD-002: `test_flow_field_respects_terrain_cost`: terrain-cost-sensitive navigation is preserved
- [x] WORLD-003: `test_flow_field_smoothing`: flow-field smoothing is preserved
- [x] WORLD-004: `test_flow_field_smoothing_normalization`: smoothing normalization behavior is preserved
- [x] WORLD-005: `test_navigation_uses_flow_field_for_far_town`: far-town navigation uses flow fields
- [x] WORLD-006: `test_navigation_uses_flow_field_for_world_boss`: world-boss navigation uses flow fields
- [x] WORLD-007: `test_road_cost_is_low`: road traversal cost law is preserved
- [x] WORLD-008: `test_prefers_road_over_swamp`: road preference over swamp is preserved
- [x] WORLD-009: `test_each_biome_has_road_network`: biome road-network presence is preserved
- [x] WORLD-010: `test_road_connects_locations`: road connectivity behavior is preserved
- [x] WORLD-011: `test_bridges_placed_over_water`: bridge placement over water is preserved
- [x] WORLD-012: `test_all_four_biomes_have_features`: biome feature presence is preserved
- [x] WORLD-013: `test_difficulty_sets_level_range`: region difficulty sets level range correctly
- [x] WORLD-014: `test_gold_scales_with_difficulty`: difficulty-linked gold scaling is preserved
- [x] WORLD-015: `test_in_region_returns_difficulty`: region difficulty query logic is preserved
- [x] WORLD-016: `test_difficulty_zones_defined`: difficulty-zone definition is preserved
- [x] WORLD-017: `test_lava_only_at_high_difficulty`: lava/high-difficulty coupling is preserved
- [x] WORLD-018: `test_all_terrains_have_names`: terrain naming coverage is preserved
- [x] WORLD-019: `test_all_terrains_have_race_labels`: terrain race-label coverage is preserved
...
- [x] COMB-149: Melee weapons default to range `1`.
- [x] COMB-150: Shortbow and longbow have distinct weapon ranges.
- [x] COMB-151: Clear horizontal line of sight passes.
- [x] COMB-152: Clear diagonal line of sight passes.
- [x] COMB-153: Wall between attacker and target blocks LOS.
- [x] COMB-154: Wall on diagonal path blocks LOS.
- [x] COMB-155: Adjacent tiles are always visible under legacy rule.
- [x] COMB-156: Wall at endpoint does not block LOS.
- [x] COMB-157: Wall at start does not block LOS.
- [x] COMB-158: Adjacent wall counts as cover.
- [x] COMB-159: Open ground gives no cover.
- [x] COMB-160: Melee attack adjacent is valid.
- [x] COMB-161: Melee attack out of range is invalid.
- [x] COMB-162: Ranged attack at valid distance is valid.
- [x] COMB-163: Unarmed entity range is `1`.
- [x] COMB-164: Ranged skill can be selected at distance.
- [x] COMB-165: Melee skill is not selected at ranged distance.
- [x] COMB-166: Starting gear gives warrior sword and ranger bow.
...
- [x] COMB-167: Straight-line path returns expected step count.
- [x] COMB-168: Same start and goal returns empty path.
- [x] COMB-169: Adjacent goal returns single-step path.
- [x] COMB-170: Path routes around walls.
- [x] COMB-171: Fully enclosed goal returns no path.
- [x] COMB-172: Unwalkable goal returns no path.
- [x] COMB-173: Returned path excludes start position.
- [x] COMB-174: Max-node budget can terminate search with no path.
- [x] COMB-175: `next_step()` returns first path tile.
- [x] COMB-176: Occupied tiles are avoided.
- [x] COMB-177: Goal tile can remain reachable even if listed as occupied.
- [x] COMB-178: Road cost is lower than floor.
- [x] COMB-179: Swamp cost is higher than floor.
- [x] COMB-180: Terrain cost registry is used by pathfinding.
- [x] COMB-181: Long-distance movement uses A\*.
- [x] COMB-182: Short-distance movement can use greedy fallback.
...
- [x] COMB-183: Entity with `leash_radius=0` is never beyond leash.
- [x] COMB-184: Entity with no home position is never beyond leash.
- [x] COMB-185: Distance within leash returns false.
- [x] COMB-186: Distance beyond leash returns true.
- [x] COMB-187: Leash multiplier extends allowed chase range.
- [x] COMB-188: Wander handler sends beyond-leash mob to return-to-camp.
- [x] COMB-189: Within-leash mob continues wandering.
- [x] COMB-190: No-leash mob wanders freely.
- [x] COMB-191: Hunt handler abandons chase beyond `1.5x` leash.
- [x] COMB-192: Hunt handler continues chase inside `1.5x` leash.
- [x] COMB-193: Hunt handler gives up after chase timeout.
- [x] COMB-194: Chase ticks reset on combat engagement.
- [x] COMB-195: Return-to-camp heals while returning.
- [x] COMB-196: Returning mob resumes normal behavior after reaching camp.
...
- [x] SOC-145: Entities with the same `cluster_id` and faction can form a group.
- [x] SOC-146: Group requires at least two living members.
- [x] SOC-147: Group leader is selected by highest level.
- [x] SOC-148: Group anchor follows leader position.
- [x] SOC-149: Members receive group ID linkage.
- [x] SOC-150: Group dissolves if leader dies.
- [x] SOC-151: Group removes dead members.
- [x] SOC-152: Group dissolves if membership drops below two.
- [x] SOC-153: Group cohesion decreases with distance from leader.
- [x] SOC-154: Shared group goal biases member goal scoring.
- [x] SOC-155: Distance from leader increases social regrouping bias.
- [x] SOC-156: Active social contracts can instantiate party groups.
- [x] SOC-157: Contract kind maps to shared group goal.
- [x] SOC-158: Contract party dissolution applies contract consequences.
...
- [x] WORLD-023: World maturity increases on schedule.
- [x] WORLD-024: Calamity spawn obeys interval and forced-spawn ticks.
- [x] WORLD-025: Spawned calamity has world-boss identity and role.
- [x] WORLD-026: Calamity uses legendary stats.
- [x] WORLD-027: Calamity receives legendary equipment/loot.
- [x] WORLD-028: Calamity spawn creates bounty quests for heroes.
- [x] WORLD-063: Calamity aura applies local debuffs.
- [x] WORLD-030: Camp reinforcements occur on schedule.
- [x] WORLD-031: Camp reinforcement level increases when camp is full.
- [x] WORLD-032: Faction raids spawn on raid interval.
- [x] WORLD-033: Raid mobs use raid AI state.
- [x] WORLD-034: Raid mobs do not return home.
- [x] WORLD-035: Killing world boss grants fame.
- [x] WORLD-036: Killing world boss grants title.
- [x] WORLD-037: Bounty completion rewards are applied.
...
- [x] WORLD-038: Region contains uses Manhattan distance.
- [x] WORLD-039: Region copy deep-copies locations.
- [x] WORLD-040: Location model preserves type, position, and region ID.
- [x] WORLD-041: Difficulty tier chosen by distance zones.
- [x] WORLD-042: Boundary distances map to expected tier.
- [x] WORLD-043: Empty difficulty zones default safely.
- [x] WORLD-044: Region name selection is deterministic by terrain counter.
- [x] WORLD-045: Resetting name counters restarts name sequence.
- [x] WORLD-046: Every terrain has region names.
- [x] WORLD-047: Every terrain has race label.
- [x] WORLD-048: Difficulty multipliers exist for all tiers.
- [x] WORLD-049: Difficulty multipliers scale upward.
- [x] WORLD-050: All location types have name templates.
- [x] WORLD-051: Expected POI types exist: camp, grove, ruins, dungeon, shrine, boss arena, outpost, watchtower, portal, fishing spot, graveyard, obelisk.
- [x] WORLD-052: Voronoi map coverage is near-total.
- [x] WORLD-053: Region terrain, overlays, details, and town tiles account for map tiles.
- [x] WORLD-054: Regions border each other.
- [x] WORLD-055: Every region owns some territory.
- [x] WORLD-056: `find_region_at()` returns nearest region.
- [x] WORLD-057: Equidistant region lookup returns first region.
- [x] WORLD-058: Empty region list returns `None`.
- [x] WORLD-059: Single region always matches.
...
- [x] SUB-272: Deterministic RNG repeats exactly for same seed/domain/entity/tick.
- [x] SUB-273: RNG domain separation produces different streams for different domains.
- [x] SUB-274: `next_int()` always respects inclusive bounds.
- [x] SUB-275: Spatial hash insert places entity in correct cell.
- [x] SUB-276: Spatial hash radius query includes neighboring cells.
- [x] SUB-277: Spatial hash move removes old cell membership and adds new cell membership.
- [x] SUB-278: Spatial hash remove clears membership.
- [x] SUB-279: Spatial hash behavior is deterministic independent of insertion order where required.
...
- [x] SUB-280: Unauthorized phase read raises.
- [x] SUB-281: Unauthorized phase mutation raises.
- [x] SUB-282: Unauthorized phase emit raises.
- [x] SUB-283: Authorized mutation succeeds only in permitted phase.
- [x] SUB-284: Internal field access is controlled.
- [x] SUB-285: Scheduling phase cannot mutate HP.
- [x] SUB-286: Persistence phase is read-only.
- [x] SUB-287: Phase guard prevents bypassing authoritative action/update application.
- [x] SUB-288: Nested phase access follows the same authorization law.
...
- [x] SUB-289: AI presenter maps structured decision drivers.
- [x] SUB-290: Belief inspection includes apparent state.
- [x] SUB-291: Injury blurring affects API-visible state consistently.
- [x] SUB-292: Stat breakdown service matches real derived stat math.
- [x] SUB-293: Combat trace recording is inspectable.
- [x] SUB-294: AI explainability persists across ticks.
- [x] SUB-295: Scheduler timeline is inspectable.
- [x] SUB-296: Entity inspection behavior matches legacy.
- [x] SUB-297: AI explanation parity is preserved or intentionally divergent.
- [x] SUB-298: Missing continuity data is handled safely.
- [x] SUB-299: Corrupted inspection state does not crash.
...
- [x] SUB-300: Strategic consistency assertion fails on real drift.
- [x] SUB-301: Cognition consistency assertion fails when replay and graph diverge.
- [x] SUB-302: Graph integrity assertion catches broken cognition graph structure.
- [x] SUB-303: Determinism assertion compares actual replay outputs, not superficial success.
- [x] SUB-304: Overload behavior assertion preserves capacity failure semantics.
- [x] SUB-305: Combat arena helper preserves default factions, hostility, tick running, and entity lookup semantics.
- [x] SUB-306: Legacy stat helper preserves expected combat math inputs.
...
- [x] INFRA-071: Simulation step runs when brokers are missing.
- [x] INFRA-072: RabbitMQ missing-package path fails closed or disables cleanly.
- [x] INFRA-073: Kafka missing-package path fails closed or disables cleanly.
- [x] INFRA-074: Engine manager recovery handles missing Kafka.
- [x] INFRA-075: Worker pool RabbitMQ dispatch contract is preserved if broker mode is supported.
- [x] INFRA-076: Kafka recovery can reconstruct from snapshot plus event stream.
- [x] INFRA-077: Live-vs-manual replay produces same world state for loot recovery.
...
- [x] INFRA-078: Every checklist row has a stable ID.
- [x] INFRA-079: Every checklist row has one explicit status: preserved, enhanced, intentionally divergent, unsupported, or not yet checked.
- [x] INFRA-080: Every checked row has at least one proof path.
- [x] INFRA-081: Every checked row names the implementation path that proves it.
- [x] INFRA-082: Every checked row names the test path that proves it.
- [x] INFRA-083: Every intentionally divergent row has a concrete reason.
- [x] INFRA-084: Every intentionally divergent row has a regression or contract test proving the new law.
- [x] INFRA-085: Every unsupported row has a support-boundary note.
- [x] INFRA-086: Unsupported behavior is not marked as intentionally divergent.
- [x] INFRA-087: Partial behavior is not marked as complete.
- [x] INFRA-088: Behavior implemented in a dead or bypassed code path is not marked as complete.
- [x] INFRA-089: Behavior implemented in one path but contradicted by another active path is not marked as complete.
- [x] INFRA-090: The checklist distinguishes implementation detail from RPG semantic law.
- [x] INFRA-091: The checklist keeps smallest necessary logic items instead of collapsing them into subsystem summaries.
- [x] INFRA-092: CI can fail when a checked row has no proof reference.
- [x] INFRA-093: CI can fail when a divergence has no divergence-register entry.
- [x] INFRA-094: CI can fail when a proof path references a missing test file.
- [x] INFRA-095: CI can fail when a checklist item references a removed implementation path.
- [x] INFRA-096: CI can fail when a test marker is unknown.
- [x] INFRA-097: CI can fail when mandatory oracle files are missing.
- [x] INFRA-098: CI can fail when an oracle lacks schema version.
...
- [x] TOWN-103: All active resource acquisition paths share one conservation law.
- [x] TOWN-104: `InteractionSystem` harvest path and any standalone `HarvestSystem` path cannot diverge on capacity rules.
- [x] TOWN-105: `InteractionSystem` loot path and any standalone `LootSystem` path cannot diverge on capacity rules.
- [x] TOWN-106: A resource node charge is not decremented until inventory receipt is proven possible.
- [x] TOWN-107: A ground item is not removed until inventory receipt is proven possible.
- [x] TOWN-108: A corpse loot record is not consumed until inventory receipt is proven possible.
- [x] TOWN-109: Inventory slot capacity is checked before source mutation.
- [x] TOWN-110: Inventory weight capacity is checked before source mutation.
- [x] TOWN-111: Inventory stackability is checked before rejecting for slot pressure.
- [x] TOWN-112: Inventory add failure preserves the source item/node/corpse.
- [x] TOWN-113: Inventory add failure resets or preserves the interaction channel according to an explicit rule.
- [x] TOWN-114: Harvest completion emits both inventory addition and node depletion atomically.
- [x] TOWN-115: Loot completion emits both inventory addition and source removal atomically.
- [x] TOWN-116: Partial channel progress does not create items.
- [x] TOWN-117: Interrupted channel progress does not create items.
- [x] TOWN-118: Interrupted channel progress does not remove source items.
- [x] TOWN-119: Failed capacity check does not remove source items.
- [x] TOWN-120: Failed capacity check does not decrement node charges.
- [x] TOWN-121: Two actors completing the same loot target in the same tick cannot duplicate the item.
- [x] TOWN-122: Two actors completing the same resource-node charge in the same tick cannot duplicate the yield.
- [x] TOWN-123: Two actors completing the same corpse loot in the same tick cannot duplicate corpse rewards.
- [x] TOWN-124: Conflict resolution decides one authoritative winner for contested loot completion.
- [x] TOWN-125: Conflict resolution decides one authoritative winner per limited resource charge when required.
- [x] TOWN-126: Item quantity is preserved through pickup, stacking, selling, crafting, and dropping.
- [x] TOWN-127: Item weight is preserved through pickup, stacking, selling, crafting, and dropping.
- [x] TOWN-128: Item identity/kind is preserved through pickup, stacking, selling, crafting, and dropping.
- [x] TOWN-129: Harvest yield uses the node definition, not caller-provided arbitrary item data.
- [x] TOWN-130: Loot yield uses the ground/corpse source definition, not caller-provided arbitrary item data.
- [x] TOWN-131: Capacity failure reason is structured and observable.
- [x] TOWN-132: Resource source mutation and inventory mutation appear in the same authoritative update/apply transaction.
- [x] TOWN-133: Resource acquisition replay includes both source and inventory state.
- [x] TOWN-134: Resource acquisition replay can prove no duplication across identical seed runs.
- [x] TOWN-135: Resource acquisition replay can prove no item loss under capacity failure.
- [x] TOWN-136: Resource-node cooldown behavior is deterministic.
- [x] TOWN-137: Resource-node recharge behavior is deterministic.
- [x] TOWN-138: Resource-node depletion state survives serialization.
- [x] TOWN-139: Ground item state survives serialization.
- [x] TOWN-140: Corpse loot state survives serialization.
- [x] TOWN-141: Loot/harvest tests include slot pressure.
- [x] TOWN-142: Loot/harvest tests include weight pressure.
- [x] TOWN-143: Loot/harvest tests include stack merge under near-full inventory.
- [x] TOWN-144: Loot/harvest tests include two actors racing for one item.
- [x] TOWN-145: Loot/harvest tests include failure preserving source state.
- [x] TOWN-146: Loot/harvest tests include success mutating inventory and source together.

## Z3. Authoritative pipeline and bypass prevention

- [x] TOWN-147: Every gameplay update enters the same authoritative refinement/apply pipeline.
- [x] TOWN-148: No AI/state system mutates authoritative world state directly during proposal generation.
- [x] TOWN-149: No standalone gameplay system can remove world objects without apply pipeline validation.
- [x] TOWN-150: No standalone gameplay system can add inventory without apply pipeline validation.
- [x] TOWN-151: No standalone gameplay system can change combat HP without combat legality validation.
- [x] TOWN-152: No standalone gameplay system can change position without movement legality validation.
- [x] TOWN-153: No standalone gameplay system can change reputation/social state without social consequence validation.
- [x] TOWN-154: No standalone gameplay system can complete quests without quest lifecycle validation.
- [x] TOWN-155: Every raw update is either accepted, refined, rejected, or marked unsupported.
- [x] TOWN-156: Rejected updates preserve unrelated update domains.
- [x] TOWN-157: Rejected updates expose structured rejection reason.
- [x] TOWN-158: Rejection reasons are replay-visible or log-visible.
- [x] TOWN-159: Merge logic for multiple `StateUpdate`s is deterministic.
- [x] TOWN-160: Merge logic has stable precedence for conflicting entity updates.
- [x] TOWN-161: Merge logic has stable precedence for conflicting world updates.
- [x] TOWN-162: Merge logic does not accidentally drop independent updates.
- [x] TOWN-163: Merge logic detects incompatible updates where required.
- [x] TOWN-164: Apply order is documented.
- [x] TOWN-165: Apply order is tested for cross-domain interactions.
- [x] TOWN-166: Apply order cannot depend on dictionary iteration where ordering matters.
- [x] TOWN-167: Apply order cannot depend on thread completion order where gameplay outcome matters.
- [x] TOWN-168: Pipeline tests include direct old-system calls if such calls remain importable.
- [x] TOWN-169: Pipeline tests prove dead/bypassed systems cannot create different gameplay laws.

## Z4. Movement intention, congestion, and blocked-route behavior

- [x] COMB-202: Movement intentions include pursue.
- [x] COMB-203: Movement intentions include retreat.
- [x] COMB-204: Movement intentions include hold.
- [x] COMB-205: Movement intentions include reposition.
- [x] COMB-206: Movement intentions include intercept.
- [x] COMB-207: Movement intentions include guard.
- [x] COMB-208: Movement intentions include regroup or an explicit intentional-divergence note explains why not.
- [x] COMB-209: Movement intention influences target choice.
- [x] COMB-210: Movement intention influences preferred tile choice.
- [x] COMB-211: Movement intention influences blocked-tile fallback.
- [x] COMB-212: Movement intention influences willingness to wait.
- [x] COMB-213: Movement intention influences willingness to sidestep.
- [x] COMB-214: Movement intention influences willingness to reroute.
- [x] COMB-215: Movement intention influences willingness to break pursuit.
- [x] COMB-216: Blocked movement first checks if target was reached.
- [x] COMB-217: Blocked movement distinguishes temporary occupancy from invalid terrain.
- [x] COMB-218: Blocked movement distinguishes ally-blocked from enemy-blocked.
- [x] COMB-219: Blocked movement distinguishes high-priority actor from low-priority actor.
- [x] COMB-220: Blocked movement can wait when waiting is strategically valid.
- [x] COMB-221: Blocked movement can yield when another actor has higher priority.
- [x] COMB-222: Blocked movement can sidestep when a safe adjacent alternative exists.
- [x] COMB-223: Blocked movement can reroute when the direct step is blocked.
- [x] COMB-224: Blocked movement can replan when route cache/path is stale.
- [x] COMB-225: Blocked movement can regroup when party cohesion matters.
- [x] COMB-226: Blocked movement can abandon route after bounded retry budget.
- [x] COMB-227: Occupancy conflict has one authoritative winner per tile per tick.
- [x] COMB-228: Occupancy conflict losers receive structured reason.
- [x] COMB-229: Occupancy conflict losers do not overlap the winner.
- [x] COMB-230: Occupancy conflict does not weaken occupancy law silently.
- [x] COMB-231: Sidestep avoids known occupied tiles.
- [x] COMB-232: Sidestep avoids invalid terrain.
- [x] COMB-233: Sidestep avoids stepping into lethal/hazardous tiles unless explicitly allowed.
- [x] COMB-234: Retreat sidestep prefers increased distance from threat.
- [x] COMB-235: Pursuit sidestep prefers preserving progress toward target.
- [x] COMB-236: Guard movement prefers preserving guard radius.
- [x] COMB-237: Intercept movement predicts target or path rather than chasing current position only.
- [x] COMB-238: Regroup movement prefers party anchor or leader position.
- [x] COMB-239: Movement fallback is bounded to avoid infinite loops.
- [x] COMB-240: Stuck counters increment only when actual movement fails.
- [x] COMB-241: Stuck counters reset when meaningful movement succeeds.
- [x] COMB-242: Stuck handling can trigger route invalidation.
- [x] COMB-243: Stuck handling can trigger strategic replan when movement is impossible.
- [x] COMB-244: Movement replay can prove no overlapping entities after conflict resolution.
- [x] COMB-245: Movement replay can prove deterministic outcome under same seed.
- [x] COMB-246: Movement tests include one actor blocked by ally.
- [x] COMB-247: Movement tests include one actor blocked by enemy.
- [x] COMB-248: Movement tests include two actors targeting same tile.
- [x] COMB-249: Movement tests include corridor congestion.
- [x] COMB-250: Movement tests include party regroup movement.
- [x] COMB-251: Movement tests include retreat under congestion.
- [x] COMB-252: Movement tests include pursuit under congestion.
- [x] COMB-253: Movement tests include invalid terrain vs occupied terrain distinction.

## Z5. Tactical combat, targeting, and action choice consistency

- [x] COMB-254: Tactical action choice uses only legal candidate actions.
- [x] COMB-255: Tactical action choice does not bypass movement/combat legality.
- [x] COMB-256: Melee attack requires valid adjacency/engagement rule.
- [x] COMB-257: Ranged attack requires valid range rule.
- [x] COMB-258: Ranged attack requires valid line-of-sight or an explicit unsupported note.
- [x] COMB-259: Area attack requires valid target position.
- [x] COMB-260: Area attack affects only entities inside AoE radius.
- [x] COMB-261: AoE friendly-fire behavior is explicit.
- [x] COMB-262: Cover behavior is explicit if ranged combat supports cover.
- [x] COMB-263: Weapon range affects tactical choice.
- [x] COMB-264: Skill range affects tactical choice.
- [x] COMB-265: Skill cost affects tactical choice.
- [x] COMB-266: Readiness/cooldown affects tactical choice.
- [x] COMB-267: Exhaustion affects tactical choice.
- [x] COMB-268: Low HP affects tactical choice.
- [x] COMB-269: Threat level affects tactical choice.
- [x] COMB-270: Target stickiness prevents unrealistic full retarget every tick.
- [x] COMB-271: Target stickiness can break when target invalid/dead/out of range beyond threshold.
- [x] COMB-272: Disengagement has explicit consequence or safe-exit rule.
- [x] COMB-273: Opportunity consequences apply only under legal conditions.
- [x] COMB-274: Anti-stalemate handles chase loops.
- [x] COMB-275: Anti-stalemate handles kite loops.
- [x] COMB-276: Anti-stalemate handles repeated step-back loops.
- [x] COMB-277: Anti-stalemate does not force illegal movement.
- [x] COMB-278: Combat result emits damage trace.
- [x] COMB-279: Combat result emits kill/death consequence when applicable.
- [x] COMB-280: Combat result emits reward/progression consequence when applicable.
- [x] COMB-281: Combat result can trigger social/narrative consequence when applicable.
- [x] COMB-282: Combat result can trigger strategic update when applicable.
- [x] COMB-283: Combat tests cover melee legality.
- [x] COMB-284: Combat tests cover ranged legality.
- [x] COMB-285: Combat tests cover AoE legality.
- [x] COMB-286: Combat tests cover invalid target rejection.
- [x] COMB-287: Combat tests cover dead target rejection.
- [x] COMB-288: Combat tests cover target stickiness break condition.
- [x] COMB-289: Combat tests cover exhaustion/readiness rejection.

## Z6. Deterministic randomness and replay-stable execution

- [x] INFRA-101: Same seed and same inputs produce same final authoritative hash.
- [x] INFRA-102: Same seed and same inputs produce same replay-visible trajectory.
- [x] INFRA-103: Different seeds produce meaningful divergence.
- [x] INFRA-104: Local sequential execution and worker execution produce equivalent gameplay outcomes.
- [x] INFRA-105: Thread scheduling order cannot change gameplay outcome.
- [x] INFRA-106: Work queue order cannot change gameplay outcome except through documented priority.
- [x] INFRA-107: RNG API supports domain separation or another proven call-order-independent scheme.
- [x] INFRA-108: RNG calls are scoped by deterministic context such as domain/entity/tick/sub-id or equivalent.
- [x] INFRA-109: Spawn randomness is isolated from tactical randomness.
- [x] INFRA-110: Tactical randomness is isolated from social randomness.
- [x] INFRA-111: Social randomness is isolated from world-event randomness.
- [x] INFRA-112: Economy/shop randomness is isolated from combat randomness.
- [x] INFRA-113: Quest generation randomness is isolated from movement randomness.
- [x] INFRA-114: Calamity/world-boss randomness is isolated from local action randomness.
- [x] INFRA-115: Adding a new actor cannot perturb unrelated actor decisions unless interaction requires it.
- [x] INFRA-116: Adding a new non-interacting system cannot perturb existing RNG outcomes.
- [x] INFRA-117: Debug/logging/presentation cannot consume gameplay RNG.
- [x] INFRA-118: Tests detect accidental use of global `random` in gameplay code.
- [x] INFRA-119: Tests detect nondeterministic set/dict ordering where it affects gameplay.
- [x] INFRA-120: Replay hash includes all gameplay-relevant domains.
- [x] INFRA-121: Replay hash excludes presentation-only fields.
- [x] INFRA-122: Replay hash includes inventory.
- [x] INFRA-123: Replay hash includes entities and positions.
- [x] INFRA-124: Replay hash includes combat state.
- [x] INFRA-125: Replay hash includes strategic state.
- [x] INFRA-126: Replay hash includes social state.
- [x] INFRA-127: Replay hash includes world resources.
- [x] INFRA-128: Replay hash includes groups/parties.
- [x] INFRA-129: Replay hash includes quests/contracts.
- [x] INFRA-130: Replay hash includes progression.
- [x] INFRA-131: Replay tests include repeated identical runs.
- [x] INFRA-132: Replay tests include sequential vs concurrent execution.
- [x] INFRA-133: Replay tests include save/load continuation.

## Z7. Purpose-driven group and party cooperation

- [x] SOC-160: Party formation can be driven by accepted social contract.
- [x] SOC-161: Party formation can be driven by shared quest or project.
- [x] SOC-162: Party formation can be driven by raid membership.
- [x] SOC-163: Party formation can be driven by escort/expedition/mercenary/revenge contract kind.
- [x] SOC-164: Proximity alone does not create a party unless explicitly modeled as a temporary tactical group.
- [x] SOC-165: Temporary tactical group is distinct from contract party.
- [x] SOC-166: Contract party has founder/leader.
- [x] SOC-167: Contract party has member roles.
- [x] SOC-168: Contract party has shared goal.
- [x] SOC-169: Contract party links back to the contract/obligation that created it.
- [x] SOC-170: Member entity links back to party/group ID.
- [x] SOC-171: Member contract links back to party/group ID where relevant.
- [x] SOC-172: Party anchor follows leader or agreed anchor rule.
- [x] SOC-173: Party cohesion is updated from member positions.
- [x] SOC-174: Party cohesion affects regroup behavior.
- [x] SOC-175: Party cohesion can trigger warnings or replan before dissolution.
- [x] SOC-176: Party dissolves when leader is dead/missing according to explicit rule.
- [x] SOC-177: Party dissolves when membership falls below minimum according to explicit rule.
- [x] SOC-178: Party dissolves when contract is completed/abandoned/failed according to explicit rule.
- [x] SOC-179: Party dissolution updates member group IDs.
...
- [x] SOC-182: Party target propagation occurs inside authoritative tick pipeline, not manual test-only invocation.
- [x] SOC-183: Shared target is valid and alive when assigned.
- [x] SOC-184: Shared target clears when invalid/dead.
- [x] SOC-185: Group focus fire does not override individual legality constraints.
- [x] SOC-186: Group behavior does not teleport or force illegal movement.
- [x] SOC-187: Group formation test covers accepted contract.
- [x] SOC-188: Group formation test covers no contract / proximity-only rejection.
- [x] SOC-189: Group dissolution test covers dead leader.
- [x] SOC-190: Group dissolution test covers scattered members.
- [x] SOC-191: Group dissolution test covers contract abandonment consequence.
- [x] SOC-192: Group coordination test runs through normal kernel tick, not only manual system call.

## Z8. Strategic projects, blockers, leads, and cognition as RPG behavior

- [x] STRAT-184: Strategic project creation is based on current needs/world state.
- [x] STRAT-185: Strategic project retention is bounded by interruption resistance.
- [x] STRAT-186: Strategic project switching requires margin or explicit emergency.
- [x] STRAT-187: Current project has reservation priority.
- [x] STRAT-188: Current objective has continuity priority.
- [x] STRAT-189: Objective derivation can create executable objectives.
- [x] STRAT-190: Objective derivation can create blockers when execution is impossible.
- [x] STRAT-191: Blockers have kind.
- [x] STRAT-192: Blockers have subject/reference.
- [x] STRAT-193: Blockers have severity.
- [x] STRAT-194: Blockers have origin/spawned-from reference where useful.
- [x] STRAT-195: Blockers can be resolved by acquiring missing knowledge.
- [x] STRAT-196: Blockers can be resolved by acquiring missing material.
- [x] STRAT-197: Blockers can be resolved by acquiring missing gold/resource.
- [x] STRAT-198: Blockers can be resolved by finding location/target.
- [x] STRAT-199: Blockers can be misdiagnosed under low cognition.
- [x] STRAT-200: Misdiagnosis is bounded and explainable.
- [x] STRAT-201: Leads have kind.
- [x] STRAT-202: Leads have subject.
- [x] STRAT-203: Leads have certainty.
- [x] STRAT-204: Leads have source trust.
- [x] STRAT-205: Leads can be tested.
- [x] STRAT-206: Tested bad leads are suppressed.
- [x] STRAT-207: Exhausted leads are not retried blindly.
- [x] STRAT-208: Lead retention obeys cognition profile capacity.
- [x] STRAT-209: Concern intake obeys cognition profile capacity.
- [x] STRAT-210: Detour breadth obeys cognition profile capacity.
- [x] STRAT-211: Detour depth obeys cognition profile capacity.
- [x] STRAT-212: Detour overflow suspends or reprioritizes project explicitly.
- [x] STRAT-213: Strategic overload is represented and observable.
- [x] STRAT-214: Strategic state persists across ticks.
- [x] STRAT-215: Strategic state survives serialization.
- [x] STRAT-216: Strategic state appears in replay/fingerprint.
- [x] STRAT-217: Strategic explanation exposes why project/objective changed.
- [x] STRAT-218: Strategic explanation does not mutate strategic state.
- [x] STRAT-219: Strategic tests include high-cognition accurate diagnosis.
- [x] STRAT-220: Strategic tests include low-cognition misdiagnosis.
- [x] STRAT-221: Strategic tests include tested-lead suppression.
- [x] STRAT-222: Strategic tests include detour depth overflow.
- [x] STRAT-223: Strategic tests include current-objective retention.
- [x] STRAT-224: Strategic tests include project-switch margin.

## Z9. Social contracts, trust, reputation, and lived consequence law

- [x] SOC-193: Public reputation and private relationship/bond are separate state.
- [x] SOC-194: Private betrayal can override public reputation.
- [x] SOC-195: Familiarity changes through interaction evidence.
- [x] SOC-196: Trust/sentiment changes through interaction evidence.
- [x] SOC-197: Social source trust changes through fulfilled/failed information.
- [x] SOC-198: Turning points persist as narrative/life-event records.
- [x] SOC-199: Turning points influence later strategic/social appraisal.
...
- [x] SOC-204: Contract appraisal uses trust/private bond.
- [x] SOC-205: Contract appraisal uses greed or reward preference.
- [x] SOC-206: Contract appraisal uses capability/role fit.
- [x] SOC-207: Contract appraisal uses prior trauma/betrayal.
- [x] SOC-208: Accepted contract creates explicit obligation/contract state.
...
- [x] SOC-215: Contract outcome can affect future recruitment decisions.
- [x] SOC-216: Contract outcome can affect strategic directives.
- [x] SOC-217: Social updates are authoritative updates, not direct mutation during appraisal.
- [x] SOC-218: Social appraisal is bounded by social bandwidth/cognition where applicable.
- [x] SOC-219: Social tests include direct betrayal by recruiter.
- [x] SOC-220: Social tests include general betrayal trauma.
- [x] SOC-221: Social tests include high reward overcoming neutral reluctance when legal.
- [x] SOC-222: Social tests include reputation distinct from private trust.
- [x] SOC-223: Social tests include contract honored.
- [x] SOC-224: Social tests include contract broken/abandoned.
- [x] SOC-225: Social tests include party dissolution consequence if contract-backed.

## Z10. Progression, classes, skills, equipment, and growth law

- [x] PROG-064: XP/reward grant is authoritative and traceable to event.
- [x] PROG-065: Level-up thresholds are deterministic.
- [x] PROG-066: Attribute point grant is deterministic.
- [x] PROG-067: Attribute allocation checks available points.
- [x] PROG-068: Attribute allocation checks valid attribute name.
- [x] PROG-069: Attribute allocation applies aptitude multiplier or explicit divergence.
- [x] PROG-070: Attribute caps are enforced.
- [x] PROG-071: Effective stats recompute from base stats plus gear plus traits plus modifiers.
- [x] PROG-072: Effective stats clamp to valid ranges.
- [x] PROG-073: Skill definition includes ID.
- [x] PROG-074: Skill definition includes type/category.
- [x] PROG-075: Skill definition includes target rule.
- [x] PROG-076: Skill definition includes range where relevant.
- [x] PROG-077: Skill definition includes cost/cooldown where relevant.
- [x] PROG-078: Physical skill scaling is defined.
- [x] PROG-079: Magical skill scaling is defined.
- [x] PROG-080: Elemental skill scaling is defined.
- [x] PROG-081: Hybrid skill scaling is defined.
- [x] PROG-082: Skill scaling uses effective stats, not raw stats, where intended.
- [x] PROG-083: Passive skills apply through defined modifier path.
- [x] PROG-084: Active skills require legality checks before applying effects.
- [x] PROG-085: Class selection or assignment is explicit.
- [x] PROG-086: Class starting gear is data-driven or documented.
- [x] PROG-087: Class skill unlocks are deterministic.
- [x] PROG-088: Gear equip validates slot.
- [x] PROG-089: Gear equip validates ownership/inventory.
- [x] PROG-090: Gear equip changes effective stats through authoritative update.
- [x] PROG-091: Gear ranking logic is deterministic.
- [x] PROG-092: Better gear can be selected by equipment service if that behavior is supported.
- [x] PROG-093: Home storage preserves items.
- [x] PROG-094: Shop buy checks gold before adding item.
- [x] PROG-095: Shop buy checks inventory capacity before subtracting gold.
- [x] PROG-096: Shop sell checks item exists before adding gold.
- [x] PROG-097: Crafting checks recipe exists.
- [x] PROG-098: Crafting checks materials.
- [x] PROG-099: Crafting checks gold/cost.
- [x] PROG-100: Crafting checks inventory capacity for output.
- [x] PROG-101: Crafting consumes materials and adds output atomically.
- [x] PROG-102: Progression replay includes XP/level/attributes/skills/gear.
- [x] PROG-103: Progression tests include attribute allocation.
- [x] PROG-104: Progression tests include cap enforcement.
- [x] PROG-105: Progression tests include skill scaling.
- [x] PROG-106: Progression tests include gear equip.
- [x] PROG-107: Progression tests include crafting atomicity.

## Z11. World, region, spawn, calamity, and ecology law

- [x] WORLD-064: World seed controls world generation.
- [x] WORLD-065: World generation is deterministic under same seed.
- [x] WORLD-066: Region assignment is deterministic.
- [x] WORLD-067: Region topology is stable for same seed.
- [x] WORLD-068: Region resource placement is deterministic.
- [x] WORLD-069: Region difficulty/threat appraisal is deterministic.
- [x] WORLD-070: Spawn tables are data-driven or documented as hardcoded intentionally.
- [x] WORLD-071: Spawned entity loadout is valid.
- [x] WORLD-072: Spawned entity faction is valid.
- [x] WORLD-073: Spawned entity role is valid.
- [x] WORLD-074: Spawned entity home/leash fields are valid where needed.
- [x] WORLD-075: Spawn avoids invalid terrain.
- [x] WORLD-076: Spawn avoids occupied tile or uses conflict-safe placement.
- [x] WORLD-077: Camp placement is deterministic.
- [x] WORLD-078: Camp guards spawn near camp.
- [x] WORLD-079: Camp guards have leash/return behavior.
- [x] WORLD-080: Leash gives up chase after explicit condition.
- [x] WORLD-081: Return-to-camp path is legal movement, not teleport, unless intentional.
- [x] WORLD-082: Calamity trigger rule is deterministic.
- [x] WORLD-083: Calamity escalation rule is deterministic.
- [x] WORLD-084: Calamity consequences affect world state authoritatively.
- [x] WORLD-085: World boss spawn rule is deterministic.
- [x] WORLD-086: World boss state appears in replay/fingerprint.
- [x] WORLD-087: Building sabotage affects building state authoritatively.
- [x] WORLD-088: Building damage affects service availability if supported.
- [x] WORLD-089: Building repair restores service availability if supported.
- [x] WORLD-090: World dynamics run even on quiet ticks where required.
- [x] WORLD-091: World dynamics do not depend on presentation/API polling.
- [x] WORLD-092: World state survives serialization.
- [x] WORLD-093: World replay includes resources/buildings/regions/camps/calamities.
- [x] WORLD-094: World tests include same-seed generation.
- [x] WORLD-095: World tests include different-seed divergence.
- [x] WORLD-096: World tests include camp/leash behavior.
- [x] WORLD-097: World tests include calamity/boss behavior if supported.

## Z12. Phase guard, authorization, and mutation boundary law

- [x] SUB-307: Each engine phase declares allowed read domains.
- [x] SUB-308: Each engine phase declares allowed write/update domains.
- [x] SUB-309: Each engine phase declares allowed emit/event domains.
- [x] SUB-310: Unauthorized read is detected or explicitly allowed.
- [x] SUB-311: Unauthorized write is rejected.
- [x] SUB-312: Unauthorized emit is rejected or flagged.
- [x] SUB-313: Phase guard cannot be disabled silently in production/certification profiles.
- [x] SUB-314: Phase guard failure is structured and observable.
- [x] SUB-315: Phase guard failure does not partially mutate world state.
- [x] SUB-316: AI/thought phase cannot write authoritative state directly.
- [x] SUB-317: Presentation phase cannot mutate authoritative state.
- [x] SUB-318: Replay phase cannot mutate authoritative state.
- [x] SUB-319: Metrics/logging phase cannot mutate authoritative state.
- [x] SUB-320: Worker phase cannot bypass authoritative apply.
- [x] SUB-321: Phase guard tests cover allowed read.
- [x] SUB-322: Phase guard tests cover blocked write.
- [x] SUB-323: Phase guard tests cover blocked emit.
- [x] SUB-324: Phase guard tests cover no partial mutation on violation.

## Z13. Spatial index and map authority law

- [x] SUB-327: Spatial index can move entity/object between cells.
- [x] SUB-328: Spatial index query returns current occupants.
- [x] SUB-329: Spatial index does not return removed occupants.
- [x] SUB-330: Spatial index does not duplicate moved occupants.
- [x] SUB-331: Spatial index supports deterministic query ordering when order matters.
- [x] SUB-332: Spatial index stays consistent with authoritative entity positions after apply.
- [x] SUB-333: Spatial index update is atomic with position update.
- [x] SUB-334: Movement legality uses authoritative map/spatial truth.
- [x] SUB-335: Combat range queries use authoritative map/spatial truth where applicable.
- [x] SUB-336: Perception queries use authoritative map/spatial truth where applicable.
- [x] SUB-337: Spawn placement uses authoritative map/spatial truth.
- [x] SUB-338: Spatial index survives serialization or can be rebuilt deterministically.
- [x] SUB-339: Spatial tests include add/query.
- [x] SUB-340: Spatial tests include remove/query.
- [x] SUB-341: Spatial tests include move/query.
- [x] SUB-342: Spatial tests include sync after movement apply.

## Z14. API, inspector, logging, and replay truth surface

- [x] INFRA-134: API state view is derived from authoritative state.
- [x] INFRA-135: API state view does not mutate authoritative state.
- [x] INFRA-136: API exposes supported gameplay state only.
- [x] INFRA-137: API marks unsupported state honestly.
- [x] INFRA-138: Inspector reads authoritative/presenter state only.
- [x] INFRA-139: Inspector does not crash on empty optional gameplay state.
- [x] INFRA-140: Inspector exposes cognition/strategy where supported.
- [x] INFRA-141: Inspector exposes social/contracts where supported.
- [x] INFRA-142: Inspector exposes inventory/equipment where supported.
- [x] INFRA-143: Inspector exposes combat/progression where supported.
- [x] INFRA-144: Logs are structured.
- [x] INFRA-145: Logs include timestamp.
- [x] INFRA-146: Logs include level.
- [x] INFRA-147: Logs include component.
- [x] INFRA-148: Logs include message.
- [x] INFRA-149: Logs include rejection reasons where relevant.
- [x] INFRA-150: Logs include final authoritative hash at shutdown.
- [x] INFRA-151: Metrics count accepted/rejected updates where relevant.
- [x] INFRA-152: Metrics count resource pressure rejection where relevant.
- [x] INFRA-153: Metrics count movement congestion where relevant.
- [x] INFRA-154: Metrics count unsupported behavior attempts where relevant.
- [x] INFRA-155: Replay manifest write is atomic.
- [x] INFRA-156: Replay manifest failure preserves old manifest.
- [x] INFRA-157: Replay finalization respects timeout.
- [x] INFRA-158: Replay includes enough state to debug RPG logic.
- [x] INFRA-159: Replay excludes presentation-only noise.
- [x] INFRA-160: API/inspector/logging tests include missing optional fields.
- [x] INFRA-161: Replay tests include corrupt/write-failure behavior.

## Z15. Safe degraded mode and infrastructure fallback law

- [x] INFRA-162: Broker-disabled mode still runs core RPG simulation.
- [x] INFRA-163: RabbitMQ disabled mode does not import/connect unexpectedly.
- [x] INFRA-164: Kafka disabled mode does not import/connect unexpectedly.
- [x] INFRA-165: Redis disabled/missing mode does not crash RPG core if optional.
- [x] INFRA-166: Local sequential executor preserves gameplay laws.
- [x] INFRA-167: Worker executor preserves gameplay laws.
- [x] INFRA-168: Fallback executor does not change deterministic outcome.
- [x] INFRA-169: Shutdown suspends new work arrival.
- [x] INFRA-170: Shutdown flushes replay/logging within timeout.
- [x] INFRA-171: Shutdown emits final authoritative hash.
- [x] INFRA-172: Failure in optional infrastructure does not corrupt authoritative state.
- [x] INFRA-173: Degraded mode is observable, not silent.
- [x] INFRA-174: Degraded mode tests cover brokerless execution.
- [x] INFRA-175: Degraded mode tests cover shutdown.
- [x] INFRA-176: Degraded mode tests cover optional dependency absence.

## Z16. Small but necessary default/no-op/safety laws

- [x] SUB-343: Empty world tick does not crash.
- [x] SUB-344: Empty world tick advances passive time if required.
- [x] SUB-345: Entity with missing optional strategy state gets safe default.
- [x] SUB-346: Entity with missing optional social state gets safe default.
- [x] SUB-347: Entity with missing optional inventory state gets safe default or explicit invalid-state error.
- [x] SUB-348: Entity with missing required combat state fails loudly before gameplay.
- [x] SUB-349: No-op update preserves state hash except allowed time/metadata changes.
- [x] SUB-350: Copy/clone of state is deep enough to protect authoritative state from worker mutation.
- [x] SUB-351: Frozen snapshot cannot be mutated by AI proposal code.
- [x] SUB-352: Serialization round-trip preserves gameplay state.
- [x] SUB-353: Serialization round-trip rejects unknown critical fields unless intentionally allowed.
- [x] SUB-354: Default values do not create free items/gold/XP.
- [x] SUB-355: Default values do not create hidden contracts/quests.
- [x] SUB-356: Default values do not create invisible movement permissions.
- [x] SUB-357: Default values do not bypass capacity/caps.
- [x] SUB-358: Clamp/floor/ceiling rules are explicit for HP.
- [x] SUB-359: Clamp/floor/ceiling rules are explicit for stamina/readiness.
- [x] SUB-360: Clamp/floor/ceiling rules are explicit for reputation/trust if bounded.
- [x] SUB-361: Clamp/floor/ceiling rules are explicit for cognition metrics if bounded.
- [x] SUB-362: Clamp/floor/ceiling rules are explicit for inventory weight/slots.
- [x] SUB-363: Safe fallback tests cover empty state.
- [x] SUB-364: Safe fallback tests cover no-op update.
- [x] SUB-365: Safe fallback tests cover copy isolation.
- [x] SUB-366: Safe fallback tests cover serialization round-trip.
