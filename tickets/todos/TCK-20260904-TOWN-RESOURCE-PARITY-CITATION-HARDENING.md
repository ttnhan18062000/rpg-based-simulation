...
## Acceptance Criteria
- [ ] All 13 entries (`TOWN-001, 004, 007, 008, 009, 010, 013, 014, 015, 016, 017, 019, 020`) have an
      explicit, evidence-backed disposition: either a confirmed real replacement citation, or an honest
      `missing`/`unsupported` status — none left silently citing a nonexistent `tests_v2/` path.
- [ ] Every citation change goes through `tools/parity_ledger_writer.py`, never a raw YAML edit.
- [ ] Parity index confirmed FRESH after the writes.
- [ ] `rpg_design_roadmap.md`'s "Economic axis check" section updated to record this as resolved.
