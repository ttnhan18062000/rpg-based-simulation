...
Investigation must print all `v2_evidence` values and verify their existence before writing the validator. If many are wrong, the ticket may need to also update matrix entries.
...
## Files Changed
- `src/content/validator.py` or `src/content/matrix_validator.py` (new)
- `tests/unit/content/test_content_usage_evidence.py` (new)
- `tests/unit/content/test_content_usage_matrix.py`
