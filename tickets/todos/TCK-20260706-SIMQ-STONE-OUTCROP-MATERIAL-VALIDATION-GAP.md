...
## Acceptance Criteria
- [ ] Root cause of the material-graph resolution gap confirmed with file:line evidence
- [ ] `stone_outcrop`'s material reference resolves correctly (fix applied to whichever side is
      actually wrong — the resource's reference, the material catalog entry, or the validator's
      graph-building logic)
- [ ] All 5 currently-failing `test_e2e_smoke.py` tests pass
- [ ] Confirmed whether other resources' material references are correctly validated (i.e. this
