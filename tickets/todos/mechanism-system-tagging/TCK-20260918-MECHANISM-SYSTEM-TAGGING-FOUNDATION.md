...
authority: P1
audience: agent
ticket_id: TCK-20260918-MECHANISM-SYSTEM-TAGGING-FOUNDATION
phase: open
date: 2026-09-18
...
## Title
Revive the `system` tier as a declared tag on mechanisms — a managed registry, many-to-many, with
...
**This is a different mechanism, not a retry of that one.** A system becomes a **tag declared on the
...
A tag on the mechanism, not a membership list in the systems registry. A mechanism may belong to
several systems.

**Tag-on-mechanism, not list-in-registry, is deliberate:** a separate membership list is a second
place holding mechanism ids, and a renamed mechanism leaves a dangling reference. A tag cannot
...
- **No orphan system** — every registered system has at least one mechanism tagged into it. A
  declared system with zero members is dead vocabulary and should fail rather than accumulate.

### 4. Untagged mechanisms render as `unassigned`
A mechanism in no system appears explicitly as unassigned, never omitted — the same rule as
`unverified`. A review surface that silently drops untagged mechanisms under-reports exactly what it
exists to show.

### 5. Initial tagging pass — large to small
Tag all 93 mechanisms. **Start with few broad systems that cover everything, then split.**
...
- **Computing anything from system tags.** Priority stays derived from `depends_on`; verification
...
3. Both invariants fail on deliberately invalid fixtures — an unregistered tag, and a registered
   system with no members. **Not** proven by a clean pass on valid data.
4. All 93 mechanisms are tagged, or explicitly listed as `unassigned` with a reason.
5. An untagged mechanism is proven to render by a test that **asserts its presence**, not by checking
   the tagged rows look right — an omission bug is invisible to the latter.
6. Nothing derives a ranking, verdict or priority from a system tag.

## Related Tickets
- `TCK-20260917-MECHANISM-SYSTEM-TIER-FEASIBILITY-INVESTIGATION` — rejected the derived design; this
  supersedes its conclusion **for the tag mechanism only**, not for derivation
...
- `stored_artifacts/TCK-20260917-MECHANISM-SYSTEM-TIER-FEASIBILITY-INVESTIGATION/` — the four derived
  candidate sets and why each failed; read before tagging, since they show what a bad grouping looks
  like
...
1. **A system tag has no mechanical backing, and that is accepted deliberately.** Nothing in the code
...
2. **Tags must remain read-only to every computation.** The moment a ranking or verdict derives from
   a tag, unbacked judgement becomes load-bearing. Systems are a review lens.
...
failed, and the tag approach now adopted. **Do not create a second plan doc for the system tier**;
