...
- [ ] At least one auth mechanism (per-client API key) gates the API surface — extracted to
      `TCK-20260823-HTTP-API-KEY-AUTH`; this epic not closed until that ticket reaches
      `tickets/done/`.
- [ ] HTTP requests are admitted/throttled/shed **per-client**, according to a mode vocabulary
      consistent with the observability layer's existing NORMAL/PRESSURE/DEGRADED/SURVIVAL states
      — extracted to `TCK-20260823-HTTP-PER-CLIENT-ADMISSION-CONTROL`; this epic not closed until
      that ticket reaches `tickets/done/`.
...
(pending — scope-only epic; see `staging_artifacts/TCK-20260817-HTTP-ADMISSION-CONTROL-EPIC/plan.md`
if written, otherwise the shared `investigation.md` serves both child tickets)

## Test Summary
(pending — no direct tests; each child ticket carries its own)

## Files Changed
(pending — no direct implementation by this ticket)

## Completion Summary
(pending — closes once both `TCK-20260823-HTTP-API-KEY-AUTH` and
`TCK-20260823-HTTP-PER-CLIENT-ADMISSION-CONTROL` reach `tickets/done/`)
