...
- Should the index be rebuilt incrementally (on every write) or at run end? **Decision: Both.**
  Incremental `append_entry()` is called from `write_trace()` (capturing `f.tell()` before each
  write) for crash recovery. Full `rebuild()` is called from `close()` for a clean, complete
  sidecar at run end. This resolves the open question.

## Implementation Notes
- `DecisionTraceIndex` created at `src/observability/cognition/tick_index.py` with `rebuild()`,
  `lookup()`, `append_entry()`, `_load()`, `_flush()` methods. All I/O is non-fatal.
- `DecisionTraceWriter.__init__` creates `self._index = DecisionTraceIndex(run_dir)`.
- `write_trace()` captures `offset = self._file.tell()` before writing, then calls
  `self._index.append_entry(tick, offset)` after flush — only first entity per tick is indexed.
- `close()` calls `self._index.rebuild()` after closing the file handle — guarantees clean index.
- `decision_trace_contract.md` updated with a "Tick Index Sidecar" section.
- `docs/parity_ledger/infrastructure.yaml` updated with INFRA-212.
- 7 new tests added to `tests/unit/observability/test_decision_trace.py`.
...
## Files Changed
- `src/observability/cognition/tick_index.py` — New: `DecisionTraceIndex` with `rebuild()`, `lookup()`, `append_entry()`, `_load()`, `_flush()`
- `src/observability/cognition/decision_trace_writer.py` — Added `self._index`, incremental `append_entry()` in `write_trace()`, `rebuild()` in `close()`
- `tests/unit/observability/test_decision_trace.py` — Added 7 new tick-index tests (17 total)
- `docs/observability/decision_trace_contract.md` — Added "Tick Index Sidecar" section
- `docs/parity_ledger/infrastructure.yaml` — Added INFRA-212

## Completion Summary
Implemented `DecisionTraceIndex` (`src/observability/cognition/tick_index.py`) — a tick→byte_offset
sidecar for `decision_trace.jsonl`. The index is updated incrementally on each `write_trace()` call
(crash recovery via `f.tell()` before write + `append_entry()`) and fully rebuilt on `close()` for
a clean complete sidecar at run end. `lookup(tick)` byte-seeks directly to the first entry for the
requested tick and reads forward until the tick changes — O(1) seek, O(k) read for k entities.
7 new AC tests added; 470 total observability tests pass. INFRA-212 added to parity ledger.
TCK-20260619-E22C-REST-API is now unblocked.
