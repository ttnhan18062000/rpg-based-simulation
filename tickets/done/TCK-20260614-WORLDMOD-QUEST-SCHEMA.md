...
- Investigation confirmed: `data/worlds/sandbox_world/world.yaml` uses `quests: []` → migration validator added.

## Implementation Notes
- `data/worlds/sandbox_world/world.yaml` has `quests: []` → migration alias required in WorldSpec
- `src/worldbuilding/cli.py` line 362 uses `spec.quests` (WorldSpec) → update to `quest_definitions`
- `src/worldbuilding/cli.py` line 349 uses `template.quests` (WorldTemplateSpec from recipe.py) → leave unchanged (out of scope)
- `src/worldbuilding/compiler.py` line 311 iterates `spec.quests` → update to `quest_definitions`
- `src/worldbuilding/recipe.py` line 140 outputs `"quests"` key in expanded dict → migration alias handles it; update key to `quest_definitions` for cleanliness

## Test Summary
- Unit: `tests/unit/worldbuilding/test_quest_definition.py`
  - Valid QuestDefinition loads, invalid type raises, source_module defaults None, frozen model, WorldSpec round-trip, migration alias round-trip, serialization
...
## Completion Summary
Added `QuestDefinition` frozen Pydantic model to `src/worldbuilding/schema.py` with 8 typed fields. Replaced `WorldSpec.quests: list[dict]` with `WorldSpec.quest_definitions: List[QuestDefinition]`. Added `_migrate_quests_field` model validator for backward compat with legacy YAML using `quests:` key. Updated compiler to iterate `quest_definitions` with typed field access and location-tag referential warnings. Updated cli.py display, recipe.py expanded dict key, and sandbox_world/world.yaml. Created 24 unit tests covering all acceptance criteria. Added SUBSTRATE-NEW-007 to parity ledger. All 126 unit+integration tests pass.
