...
## Files Changed
- `src/api/server.py` — added `set_quality_hub`/`set_quality_persistence` to top import;
  wired hub into dependency container after `manager.start()` via `manager.kernel.quality_hub`

## Completion Summary
G2 fixed with 4 lines: after manager.start(), grabs hub from manager.kernel.quality_hub
(new property added by WIRE-KERNEL) and calls set_quality_hub()/set_quality_persistence().
The pre-existing shutdown handler now finds a non-None hub and writes quality_report.json.
68 API tests pass.
