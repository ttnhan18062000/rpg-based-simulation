...
## Scope
- [x] Task 1: Update `Makefile` to point `test` and `test-cov` targets to `tests` and `src`.
- [x] Task 2: Update CI validation jobs (if any) to use the new supported test paths.
- [x] Task 3: Migrate automated replay/report generation scripts to depend on `src` by default.
...
- [x] `make test-cov` generates coverage for `src`.
...
- Use `tests` as the authoritative verification suite.
