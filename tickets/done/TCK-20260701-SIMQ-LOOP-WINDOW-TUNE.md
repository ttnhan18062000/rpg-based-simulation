...
Added `--window-size` and `--loop-threshold` CLI flags to `tools/calibrate_simq.py`. Both override
`detection_params.yaml` for the current run only via `ScoringWeights.model_copy()` + `DetectionParams.model_copy()` —
the YAML is never mutated. The print header now shows effective `window_size` and `loop_threshold`.

**Sweep results (2026-07-02, seed 42, 200 ticks):**
| World | Events | w=100 grade | w=150 grade | w=200 grade | w=300 grade |
|---|---|---|---|---|---|
| sandbox_world | 47 | B | B | B | B |
| dungeon_crawl | 287 | — | — | B | B (w=300) |

Zero grade change across all window sizes. sandbox_world produces only 47 scored events — the window
never fills even at window_size=100. dungeon_crawl produces 287 events total but highest single-pillar
count is WORLD=150, COMBAT=65 — loop detection remains dormant.

**Decision: 200/0.70 confirmed correct, no change.** The low event density in 200-tick runs means
loop detection is irrelevant at this timescale. Loop flags are a real-time long-session diagnostic,
not a short-run calibration metric. quality_scoring_contract.md §4.7 updated with this finding.

## Test Summary
13 existing accumulator tests: pass. 2 new tests added (injected window_size, injected loop_threshold): pass.
8 grade regression anchors: pass. 6 hub integration tests: pass. Total: 32/32.

## Files Changed
- `tools/calibrate_simq.py` — added `--window-size`, `--loop-threshold` CLI flags; model_copy patch; updated header print
- `tests/simulation_quality/test_accumulator.py` — added 2 tests for injected detection param overrides
- `docs/simulation_quality/quality_scoring_contract.md` — §4.7 updated with sweep findings and CLI tooling note
- `docs/parity_ledger/infrastructure.yaml` — added INFRA-251

## Completion Summary
Added per-run window/threshold CLI override to calibrate_simq.py (no YAML mutation). Ran sweep
across 4 window sizes on sandbox_world and dungeon_crawl; confirmed 200/0.70 defaults are correct.
Documented decision in quality_scoring_contract.md §4.7. Parity entry INFRA-251 added.
