...
## Request Summary
Switch the default operational entrypoints (CLI, Serve) from the legacy `src` engine to the hardened `src_v2` engine.

## Scope
- [x] Task 1: Implement delegation logic in `src/__main__.py` to point to `src_v2` by default.
...
## Acceptance Criteria
- [x] Running `python3 -m src cli` executes the `src_v2` engine.
- [x] Running `python3 -m src serve` starts the `src_v2` API server.
...
- Non-supported legacy subcommands (like `inspect`) should either be delegated to `src_v2` (if available) or kept in `src` with a warning.
