...
Added a standalone bullet in `.claude/agents/test-scoper.md`'s "Scoping Rules" section (before the
cross-cutting-expansion bullet), generalizing the existing `tools/*.py`-only bare-directory rule
into an unconditional rule for any in-scope directory, citing the retro's own recurrence count as
evidence. The existing `tools/*.py`-specific bullet was left in place unchanged (still correct, now
one instance of the general rule).

## Test Summary
No automated test surface for an agent-definition prompt file. Verified by re-reading the full
file for internal consistency — the new bullet doesn't contradict or duplicate the existing
`tools/*.py` bullet, and sits logically before the cross-cutting-expansion rule it references.

## Files Changed
.claude/agents/test-scoper.md

## Completion Summary
Filed and fixed the same day, per `agent-monitoring/retro/RETRO-2026-W35.md` item 3.
