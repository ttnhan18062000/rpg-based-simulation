...
phase: done
date: 2026-06-23
tags: [test-repair, worldassembly, content, validation]
---
...
## Files Changed
- `data/content/world/terrain.yaml` — added `river` terrain entry (move_cost_multiplier: 1.8, hazard_multiplier: 1.4)
- `tests/unit/core/test_catalog_smoke_simulation.py` — replaced legacy `"quests"` dict with `"quest_definitions"` using QuestDefinition schema fields

## Completion Summary
Both root causes fixed. 180 integration tests pass (content/, content_packs/, scenario_catalog_matrix). Worldassembly unit tests 48 pass. Smoke sim test body passes (teardown error is pre-existing FallbackRestrictedError, tracked by TCK-20260623-FIX-TEST-TEARDOWN). One pre-existing unrelated failure in test_cli_resolve_and_compile_integration (plains_layout module not found) is out of scope.
