...
phase: done
date: 2026-06-23
tags: [test-repair, kernel, pipeline, ItemStack, determinism]
---
...
## Files Changed
- `tests/integration/kernel/test_milestone_a_closure.py:78` — changed source inspection from `Kernel.tick_once` to `Kernel._tick_once_inner`
- `tests/integration/kernel/test_race_conditions_v2.py` — added `GroundItemState` import; replaced `ItemStack("gold_coin", 50)` with `GroundItemState(id=100, item_id="gold_coin", quantity=50, position=(5.0, 5.0))` in ground_items fixture

## Completion Summary
Both fixes are test-only. RC1: phase inspection targeted wrong wrapper method; RC2: test fixture used ItemStack (no .position) where GroundItemState (has .position) is required. 134 kernel tests pass. Remaining failures are pre-existing entity-ID and milestone-C/D issues tracked by separate tickets.
