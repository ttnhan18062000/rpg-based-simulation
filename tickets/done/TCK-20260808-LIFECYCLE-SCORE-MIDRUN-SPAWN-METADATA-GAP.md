...
Traced the real root cause: `_run_for_analysis()` drove the Kernel and discarded it after
computing `health`, so `extract_entity_paths()` could only ever resolve metadata from the
pre-run world snapshot. Real population growth during a run is genuinely multi-source
(`SpawnService`/`BossService`/`RaidService`/`CampService`/`CalamityService`/
`DemographicCycleService`, all feeding `demographic_birth` generically) — not one birth formula,
so the exact "always 10" convergence wasn't chased to full numerical closure (disproportionate
effort for this ticket's own real deliverable). A real, disclosed, out-of-scope connection was
found while tracing `SpawnService`: its own density check reads `EntityRole.MONSTER`, the same
field this session's own `LIFECYCLE-SCORE-WORLD-ARCHETYPE-AWARENESS` ticket already found is
mistagged corpus-wide — flagged for a future ticket, not fixed here.

Fixed by capturing `kernel.state.entities` before `shutdown()`, threading it through
`score_run()`/`extract_entity_paths()` as a fallback metadata source for any entity missing from
the pre-run snapshot. One real, disclosed residual limitation: an entity born AND removed
entirely within the observed window is still unresolvable (present in neither snapshot) — real
data check found none of the 60 previously-`None` entities hit this edge case.

## Test Summary
`pytest tests/tools/test_entity_lifecycle_score.py tests/tools/test_simq_long_run_observation.py
-q` — 30/30 passed (3 new: `final_entities` return-shape check, a real mid-run-spawn metadata
resolution check on `wilderness_survival`, and a `--run-dir`-mode-still-works regression guard).

**Real re-verification** (not just unit tests): re-ran `make simq-long-run-lifecycle-observation`
(2000 ticks, all 6 curated worlds). Before the fix: 21-48% of each world's real population had
`role: None`. After: **0% in all 6 worlds** — full resolution on this real dataset (the 1 disclosed
residual edge case did not occur in this data).

## Files Changed
- `tools/entity_lifecycle_score.py` — `_run_for_analysis()` returns `final_entities`;
  `_entity_metadata()` accepts a raw entities dict; `extract_entity_paths()`/`score_run()`/`main()`
  thread the new post-run fallback through
- `tools/simq_long_run_observation.py` — threads `final_entities` through `observe_world()`
- `tests/tools/test_entity_lifecycle_score.py` — 3 new tests, 2 existing call sites updated for
  the new 3-tuple return
- `docs/simulation_quality/entity_lifecycle_score.md`, `docs/guides/entity_lifecycle_score.md` —
  updated
- `docs/simulation_quality/long_run_observations/*.json` — refreshed with the real post-fix data

## Completion Summary
Fixed a real bug affecting 21-48% of every long-run world's observed population, verified with
real before/after data (not just passing unit tests) showing full resolution across all 6 curated
worlds. Disclosed one residual edge case and one out-of-scope connected finding
(`EntityRole.MONSTER` mistagging's likely effect on `SpawnService`'s own density check) rather
than chasing either to false closure. All 5 of the ticket's own Acceptance Criteria items
satisfied.
