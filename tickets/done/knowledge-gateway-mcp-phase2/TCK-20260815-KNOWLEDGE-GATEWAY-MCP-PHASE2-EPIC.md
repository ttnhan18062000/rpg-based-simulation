...
## Acceptance Criteria
- [x] Only Phase 2 (§20) work is scoped/authorized under this epic; no Phase 3+ deliverable
      (context-packet caching, semantic reuse, durable knowledge records) is claimed as done here.
- [x] The cache schema and migrations extend `knowledge-index/retrieval_cache.db` in place, per
      `cache_migration_plan.md`'s design — no second database, no full-rebuild-and-atomic-swap.
- [x] Cached rows are validated against real evidence fingerprints before being served as a hit,
...
- [x] Only allowlisted source types (Context Search + Graphify results, per
...
- [x] Exact normalized-query reuse works: an identical repeated query (same normalized intent,
      entity IDs, filters, budget class) produces a genuine cache hit on the second call, verified
      by a real test proving the second call bypasses the provider round-trip. (True at unit/
      integration-test granularity with payloads sized to fit under §5's 8KB cap — see Completion
      Summary for the real-corpus caveat this does NOT resolve.)
- [x] This epic's own acceptance-measurement child ticket honestly re-measures the real cache-hit
...
- [x] `docs/plans/knowledge-gateway-mcp-proposal.md` §20's Phase 2 bullets are annotated Done as
...
- [x] This epic is not closed merely because a child ticket's code lands — Phase 2's own bar (all
      ...
      met. (Met in the literal sense specified — see Completion Summary for the honest, load-bearing
      caveat about what "genuinely producing hits" means in practice today.)
...
Architecture-Verify post-Implement) and verified (done-checker) before the next began:

1. **TCK-20260815-KGMCP-P2-CACHE-SCHEMA-MIGRATIONS** — `retrieval_cache_schema_version`,
   `LEVEL1_CACHE_COLUMNS` (21→22 columns after child 3's `migration_003`),
   `migration_001_add_level1_tables()` in `tools/retrieval_cache.py`, creating
   `retrieval_provider_result_cache_rows` additively, in the same `knowledge-index/retrieval_cache.db`
   file — never auto-invoked, never a second database.
2. **TCK-20260815-KGMCP-P2-REDACTION-WRITE-PATH** — `tools/knowledge_gateway_redaction.py`:
   allowlist, redaction+hashing, secret-scan baseline (later expanded 4→10 patterns — see child 3),
   8KB size cap, `redaction_policy_version` stamping, 6 never-cache categories, §9 SQLite
   operational limits, §10 GC-eligibility predicates — pure, directly-testable functions, zero live
   wiring at this point.
3. **TCK-20260815-KGMCP-P2-CACHE-READ-WRITE-WIRING** — `tools/knowledge_gateway_cache.py` wires
   real cache lookup/evidence-validity revalidation/branch-scope enforcement/write into the live
   `knowledge_context` call path for the first time. Security-Review initially BLOCKED on the
   ratified redaction policy's "reviewed and expanded before going live" precondition (the 4-pattern
   baseline had concrete, named gaps); fixed per explicit user decision by expanding to 10 patterns,
   then re-approved. Architecture-Verify also caught and required a real integration-test coverage
   gap (size-cap gate, stampede guard) before approving.
4. **TCK-20260815-KGMCP-P2-BASELINE-RECOMPARISON** — real cold+warm measurement against Phase 0's
   frozen 7-entry corpus, with Architecture Review's explicit DD1 ruling barring any request-shape
   shrinking to force a favorable result. Real result: **0/7 genuine cache hits** — every corpus
   entry's real response payload (10.6–30.5 KB) exceeds `MAX_PAYLOAD_BYTES = 8192`, so
   `check_size_cap()` rejects every write as `oversized_payload` before a row can ever be cached.

## Test Summary
Each child ticket's own Test phase independently re-verified its scoped test suite (never trusting
self-reported numbers): 11, 65 (after the pattern expansion), 297, and 17 new/affected tests
respectively, all passing except one pre-existing, independently-double-confirmed unrelated failure
in child 4 (`test_no_frozen_kgmcp_dependency_edited`, a branch-state artifact from stacked
already-Done sibling diffs, not a regression). No test was weakened or narrowed to force a pass at
any point in this epic — narrowings that did occur (documented in each child's own Implementation
Notes) were reviewer-confirmed corrections of stale, ticket-superseded assumptions.

## Files Changed
- `tools/retrieval_cache.py`, `tools/knowledge_gateway_redaction.py`, `tools/knowledge_gateway_cache.py`
  (new), `tools/knowledge_gateway_mcp.py` — the real cache schema, write-path enforcement, and live
  wiring, per the 4 child tickets above.
- `tools/agent-monitoring/kgmcp_phase2_gateway_runner.py` (new) — the honest recomparison runner.
- `docs/engine/contracts/knowledge_gateway_mcp/cache_migration_plan.md`,
  `redaction_retention_policy.md` — updated in place as each precondition became real/operative;
  `evidence_cache_identity_contract.md` remained untouched (implemented, not redesigned).
- `docs/engine/contracts/knowledge_gateway_mcp/phase2_baseline_recomparison.md` (new) — the honest
  0/7-hits, all-3-thresholds-FAIL results doc.
- `docs/plans/knowledge-gateway-mcp-proposal.md` — all 4 §20 Phase 2 bullets annotated Done.
- `docs/parity_ledger/infrastructure.yaml` — `INFRA-341` through `INFRA-344`, plus in-place citation
  corrections to `INFRA-341`/`INFRA-342` as later child tickets' edits shifted earlier line numbers.
- `tests/tools/test_knowledge_gateway_redaction.py`, `test_retrieval_cache.py`,
  `test_knowledge_gateway_cache.py` (new), `test_kgmcp_phase2_baseline_recomparison.py` (new), plus
  targeted, reviewer-confirmed narrowings to `test_kgmcp_measurement_baseline.py` and
  `test_redaction_retention_policy_doc.py`.
- `tickets/working_log.csv` — 4 rows, one per child ticket.

## Completion Summary
All 4 Phase 2 child tickets landed and closed, each independently architecture-reviewed,
tested, and DoD-verified. Every literal Acceptance Criterion above is met, including the two
most safety-critical ones: cache writes are only ever reachable through the redaction gate
(`evaluate_write_candidate()` returning ALLOW — no bypass, statically guarded), and the ratified
secret-scan baseline was genuinely reviewed and expanded (not merely documented) before the cache
write-path went live, closing a real Security-Review BLOCKED finding rather than routing around it.

**The one honest, load-bearing caveat, surfaced by this epic's own acceptance-measurement child
ticket rather than glossed over:** "a real cache genuinely producing hits" is true at the
unit/integration-test level (a real test proves an identical repeated query bypasses the provider
round-trip when its response fits under the 8KB write-size cap), but the real-corpus measurement
found **0 of 7** realistic Knowledge Gateway MCP queries produce a genuine cache hit in practice —
every one of the 7 frozen corpus entries' actual response payload (10.6–30.5 KB) exceeds
`MAX_PAYLOAD_BYTES = 8192`, so every real write is rejected as `oversized_payload` before a row is
ever cached. This is a materially more severe finding than Phase 1's own honest all-FAIL result
(which was "the cache doesn't exist yet, so of course cold-only timing fails") — here, the cache
mechanism is fully built, correctly gated, and tested, but is currently sized in a way that makes
it structurally inert for the gateway's actual response sizes. §4.1 (latency) and §4.2
(token-reduction) both FAIL as a direct consequence; §4.3 (recall) FAILs for the separate,
already-documented Q2/Q5 routing reason, unchanged from Phase 1.

Per the user's explicit 2026-08-16 closure decision, this epic is closed now with this finding
recorded honestly, mirroring the Phase 1 epic's own precedent of closing on an honest FAIL rather
than deferring closure pending a fix. Scoping and prioritizing a follow-up (raising the size cap,
trimming/compressing response payloads, or a leaner cacheable response mode) is left to a human
reviewer and a separate, later ticket — not assumed, decided, or silently implemented here.
