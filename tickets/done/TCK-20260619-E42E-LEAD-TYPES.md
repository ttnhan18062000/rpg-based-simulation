...
```
## Files Changed
- `src/core/strategic.py` — Added `LeadKind(str, Enum)` with 5 values; migrated `LeadState.kind: str` → `LeadState.kind: LeadKind`
- `src/engine/domain/lead_routing.py` — New `LeadRoutingSystem.resolve_objective_kind()` (E42E-002)
- `src/systems/strategic_systems/detour.py` — Import `LeadKind`, `LeadRoutingSystem`; wire PERSON/CONCEPT routing in `_infer_objective_kind` and `suggest_detours`
- `tests/unit/cognition/test_information_seeking.py` — Added `TestLeadKindEnum` (3 tests) and `TestLeadRoutingSystem` (6 tests); 48 total, all pass
- `docs/simulation/domains/information_contract.md` — Added PERSON/CONCEPT lead routing section
- `docs/parity_ledger/strategic_cognition.yaml` — Added STRAT-232 (LeadKind enum) and STRAT-233 (PERSON/CONCEPT routing)

## Completion Summary
Implemented `LeadKind(str, Enum)` with LOCATION, OBJECT, EVENT, PERSON, CONCEPT values. Migrated `LeadState.kind` type. Created `LeadRoutingSystem` to resolve PERSON → INVESTIGATE and CONCEPT → ASK_INFORMATION objectives. Wired into `DetourSuggestionSystem`. All acceptance criteria tests pass; no regressions introduced. Parity ledger updated with STRAT-232 and STRAT-233.
