...
when:
- [x] All five Scope items above have corresponding child tickets created and linked back to this
  epic's `ticket_id`. Confirmed: all 5 (`NOISE-FILL-SCHEMA`, `COMPILER-NOISE-FILL`,
  `WOLF-DEN-NOISE-MIGRATION`, `NOISE-FILL-DETERMINISM-TEST`, `PROCEDURAL-GENERATOR-KEPT`) exist in
  `tickets/done/` as of 2026-08-22.
- [x] Each child ticket, at its own close, demonstrates its own testable acceptance criteria — the
  golden-hash determinism test (Scope item 4) passes; `wolf_den_near_forest` (Scope item 3) now
  produces per-tile forest/swamp variation instead of a flat rectangular fill, proven by 5 new
  real-content tests including an exact RNG-recomputation proof of the overlap-resolution decision.
...
Scope-only epic — no direct implementation. All 5 child tickets implemented in `SEQUENCE.md`'s
dependency order: `NOISE-FILL-SCHEMA` (schema field), `COMPILER-NOISE-FILL` (compiler consumption,
one Review round found and fixed a real tile-encoding collision bug affecting any region with
y-extent >=256), `NOISE-FILL-DETERMINISM-TEST` (found 3 of 4 planned ACs already satisfied by
tests the prior ticket added, closed only the genuine gap), `PROCEDURAL-GENERATOR-KEPT` (corrected
Scope item 5's own dead-code premise — the class is intentionally preserved, not dead),
`WOLF-DEN-NOISE-MIGRATION` (real content migration, one Review round found a genuine cross-module
overlap consequence affecting 5 real world compositions beyond the originally-scoped intra-module
case, explicitly surveyed and accepted as legality-inert rather than silently shipped).

## Test Summary
No direct tests for this epic itself. Each child ticket carried and passed its own scoped test
suite. Aggregate: schema/compiler tests, a wolf_den-specific real-content test file (5 new tests,
including an exact RNG-recomputation proof of the overlap-resolution decision), and the extended
determinism test — 234 tests passing in the final child ticket's own scoped verification run.

## Files Changed
No files changed directly by this epic. Collectively across all 5 child tickets: `src/worldbuilding/schema.py`,
`src/worldbuilding/recipe.py`, `src/worldassembly/resolver.py`, `src/worldbuilding/compiler.py`,
`data/content/world_modules/wolf_den_near_forest.yaml`, `docs/mechanics/06_worldbuilding_foundation.md`,
`docs/guidelines/intentional_divergences.md`, `docs/world/compiler_contract.md`,
`docs/parity_ledger/substrate.yaml` (SUB-385, SUB-386), `tests/unit/worldbuilding/test_world_compiler.py`,
`tests/unit/worldassembly/test_assembly.py`, `tests/unit/worldbuilding/test_worldspec_schema.py`,
`tests/integration/worldassembly/test_real_content_world_modules.py`,
`tests/integration/worldassembly/test_wolf_den_noise_migration.py` (new).

## Completion Summary
All 5 child tickets tracked by this epic reached `tickets/done/` with their own evidence, satisfying
the epic's own scoped-complete criteria. The batch delivered a fully working, seed-varied organic
terrain mechanism: an inert schema field, a compiler consumption path keyed under a collision-safe
RNG domain (`Domain.INIT`, provably isolated from `Domain.WORLD`'s existing draws), a genuine
determinism regression guard, and the first real content migration (`wolf_den_near_forest`) proving
the mechanism end-to-end — including an honestly-documented decision about both an intra-module and
a previously-latent cross-module region overlap. Two separate Review rounds across the batch each
caught a real, non-hypothetical defect (a tile-encoding collision bug; a missed cross-module content
consequence spanning 5 real shipping worlds) before landing, not after. `WorldProceduralGenerator`
was correctly kept, not deleted, correcting the epic's own original premise. Closed 2026-08-22.
