...
| M3 — finding ingestion | `TCK-20260924-M3-TERRITORY-COMBAT-FINDING-TRIAGE` | **Narrow bar DONE** 2026-09-24 — M3 itself remains a permanent stream with no complete state, but the one hard bar M4 was gated on is met: Territory (5 findings) + Combat (8 findings/questions) triaged against live state, `docs/plans/simulation_semantic_control_plane/finding_triage_log.md`. Caught a stale `ENABLE_COMBAT_ENGAGEMENT` default-OFF premise in both Combat source docs (live: ON since 2026-09-14) — recorded as verified evidence for M4, not classified. Zero Combat rows written (M4's own deliverable); one Territory row's evidence text amended (TERR-01). |
| M4 — Combat slice + cross-domain view | `TCK-20260924-M4-COMBAT-SLICE-CROSS-DOMAIN-VIEW` | **DONE** 2026-09-24. Combat/Conflict mapped (10/12 real Rule IDs, all `PARTIAL`, each for its own independent reason; `CONFLICT-01`/`ECOL-04` stay `UNKNOWN`). First real cross-domain view (`make cross-domain-management-view`) renders both domains with combined raw counts, never a bare percentage. Written comparison records honestly that Combat's higher Rule-level verified-count is not "more done" than Territory. **This closes the roadmap's own bounded M0–M4 proof-of-generalization goal — the mapping itself is not finished**; `rollout_plan.md` Stages D/E/F continue indefinitely. |
