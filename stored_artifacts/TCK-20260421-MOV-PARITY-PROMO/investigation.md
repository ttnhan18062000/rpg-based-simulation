...
## Current State
- Movement parity is currently verified via a standalone script: `tests_v2/parity/movement_oracle/verify_v2_movement.py`.
- It loads expected results from `tests_v2/parity/movement_oracle/results.json`.
...
## Required Changes
1.  **New Test File**: `tests_v2/parity/test_movement_parity.py`.
...
- **Q2**: Should I create `tests_v2/gameplay` directory as mentioned in the task possible affected files?
  - *Answer*: Yes, if `tests_v2/parity/test_movement_parity.py` is better located there. But Milestone 1, Task 1 says `tests_v2/parity/test_movement_parity.py`. I will follow the explicit path provided.
