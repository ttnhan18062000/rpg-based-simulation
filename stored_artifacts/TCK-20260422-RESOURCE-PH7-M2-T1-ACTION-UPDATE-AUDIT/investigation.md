...
## 1. Current Implementation State (`src`)

### 1.1 Already Closed / Hardened
- **Typed Update Buckets**: `StateUpdate` and `EntityUpdate` in `src/core/updates.py` are already well-defined, frozen dataclasses using slots. They support disaggregated update domains (Combat, Social, Inventory, etc.).
...
- **Problem**: There is no `ActionProposal` class in `src`. The simulation lacks an explicit representation of "Intent" (Actor + Verb + Target + Reason).
...
- **Affected Files**: `src/core/updates.py` (needs pairing with Intent), `src/engine/kernel.py` (needs to move routing logic to workers).
...
- **Affected Files**: `src/core/updates.py`.
...
- **Problem**: Workers (currently represented by `src/engine/worker_manager.py` and `src/engine/executor.py`) have access to the `AuthoritativeState` (read-only), but the protocol for producing `EntityUpdate` is still being refined.
...
1. **Task 2**: Implement `src/core/actions.py` with `ActionProposal`.
