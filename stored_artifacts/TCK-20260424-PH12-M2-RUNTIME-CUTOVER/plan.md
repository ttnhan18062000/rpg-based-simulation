...
- If `USE_LEGACY_SRC` is not set or falsy, import and call `src_v2.cli.entry.main()`.
- Else, proceed with legacy `_run_server`, `_run_cli`, etc.

## 2. CLI Argument Alignment
- Audit `src_v2/cli/entry.py` to ensure it handles common flags correctly.
- Add a "Cutover Warning" to `src_v2` output to notify the operator they are using the V2 engine.

## 3. API Server Delegation
- Ensure `src_v2.api.server.create_v2_app` is fully compatible with the expected FastAPI lifecycle.
- Verify that `src_v2` routes are mounted or served correctly when `python -m src serve` is called.
