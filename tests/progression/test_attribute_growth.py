...
def test_hero_ap_grant_on_level_up():
    ...
    entity = (V2EntityBuilder(1)
              .kind("hero")
              .location(0, 0)
              .identity(role=EntityRole.HERO, faction=Faction.HERO_GUILD, evolution_level=1)
              .combat(hp=100, max_hp=100, atk=10, def_stat=5)
              .attributes(strength=5, vitality=5)
              .build())
...
def test_attribute_allocation_and_recalc():
    ...
    entity = (V2EntityBuilder(1)
              .kind("hero")
              .location(0, 0)
              .identity(role=EntityRole.HERO, faction=Faction.HERO_GUILD, evolution_level=2)
              .attributes(strength=5, vitality=5, endurance=5)
              .build())
