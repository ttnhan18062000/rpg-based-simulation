...
def test_warrior_starting_gear():
    ...
    warrior = V2EntityBuilder(entity_id=1).identity(role=0).with_class("WARRIOR").build()
...
def test_mage_starting_gear():
    ...
    mage = V2EntityBuilder(entity_id=1).identity(role=0).with_class("MAGE").build()
...
def test_rogue_starting_gear():
    ...
    rogue = V2EntityBuilder(entity_id=1).identity(role=0).with_class("ROGUE").build()
