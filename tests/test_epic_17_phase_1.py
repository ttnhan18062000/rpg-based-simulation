...
def test_death_escalation_and_replacement(basic_setup):
    ...
    assert len(loop._pending_hero_replacements) == 1
    assert loop._pending_hero_replacements[0]["generation"] == 2
    
    # Progress time to replacement
    world.tick = 60
    loop._process_hero_replacements()
    ...
    assert new_hero.generation == 2
    assert new_hero.display_name != "TestHero"
...
def test_hero_familiarity(basic_setup):
    ...
    assert h1.hero_familiarity[h2.id] == 0.002
    assert h2.hero_familiarity[h1.id] == 0.002
