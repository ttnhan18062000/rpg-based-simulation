...
def test_agents_md_states_authoritative_37_phase_pipeline():
    text=(ROOT / "AGENTS.md").read_text()
    assert "17-phase" not in text and "17 phase" not in text
    assert "37-phase" in text and "docs/engine/authoritative_pipeline.md" in text


def test_agents_md_pipeline_note_matches_live_engine_doc():
    assert "37 phases" in (ROOT / "docs" / "engine" / "authoritative_pipeline.md").read_text()
