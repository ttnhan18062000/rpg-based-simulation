...
def test_recruitment_cost_discount_with_trust():
    ...
    # Low trust from candidate toward recruiter:
    # candidate.social.trust_history[recruiter.id] = 0.0
    # Expected multiplier: 1.5
    cand_low = (
        V2EntityBuilder(2)
        .kind("HERO")
        .location(1.0, 0.0)
        .social(trust_history={1: 0.0})
        .build()
    )

    cost_low = SocialAppraisalSystem.calculate_recruitment_cost(
        recruiter,
        cand_low,
    )

    # High trust from candidate toward recruiter:
    # candidate.social.trust_history[recruiter.id] = 1.0
    # Expected multiplier: 0.5
    cand_high = (
        V2EntityBuilder(3)
        .kind("HERO")
        .location(2.0, 0.0)
        .social(trust_history={1: 1.0})
        .build()
    )

    cost_high = SocialAppraisalSystem.calculate_recruitment_cost(
        recruiter,
        cand_high,
    )
...
def test_recruitment_acceptance_logic():
    ...
    # Candidate trusts recruiter/source entity 2.
    candidate = (
        V2EntityBuilder(1)
        .kind("HERO")
        .location(0.0, 0.0)
    ...
    # Low payout + low trust -> reject/cancel.
    candidate_low = (
        V2EntityBuilder(1)
        .kind("HERO")
        .location(0.0, 0.0)
        .social(trust_history={2: 0.2})
        .build()
    )
