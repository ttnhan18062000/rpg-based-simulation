...
from src.core.models.enums import ActionType, DamageType, EnemyTier
...
def _make_entity(
    ...
    identity = IdentityAspect(faction=faction)
    spatial = SpatialAspect(pos=Vector2(*pos))
    combat = CombatAspect(hp=hp, max_hp=hp, atk=atk, def_=def_, spd=spd)
    mind = MindAspect()
    ...
    return Entity(
        id=eid, kind=kind, next_act_at=0.0,
        identity=identity,
        spatial=spatial,
        combat=combat,
        progression=prog,
