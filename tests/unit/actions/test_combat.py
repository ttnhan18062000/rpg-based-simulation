...
class TestEntityWithAttributes:
    ...
    def test_entity_has_stamina(self):
        ...
        assert e.stats.progression.stamina == 60
        assert e.stats.progression.max_stamina == 60
        assert e.stats.progression.stamina_ratio == 1.0

    def test_stamina_ratio_partial(self):
        e = _make_entity(1, stamina=50)
        e.stats.progression.stamina = 25
        assert abs(e.stats.progression.stamina_ratio - 0.5) < 0.01
...
class TestStaminaMechanics:
    ...
    def test_stamina_decreases_on_attack(self):
        ...
        e.stats.progression.stamina = max(0, e.stats.progression.stamina - 3)
        assert e.stats.progression.stamina == 47

    def test_stamina_decreases_on_move(self):
        e = _make_entity(1, stamina=50)
        e.stats.progression.stamina = max(0, e.stats.progression.stamina - 1)
        assert e.stats.progression.stamina == 49

    def test_stamina_decreases_on_harvest(self):
        e = _make_entity(1, stamina=50)
        e.stats.progression.stamina = max(0, e.stats.progression.stamina - 2)
        assert e.stats.progression.stamina == 48

    def test_stamina_cannot_go_below_zero(self):
        e = _make_entity(1, stamina=1)
        e.stats.progression.stamina = max(0, e.stats.progression.stamina - 5)
        assert e.stats.progression.stamina == 0

    def test_stamina_regen_resting(self):
        e = _make_entity(1, stamina=50)
        e.stats.progression.stamina = 20
        # Resting regen: +5 per tick
        regen = 5
        e.stats.progression.stamina = min(e.stats.progression.stamina + regen, e.stats.progression.max_stamina)
        assert e.stats.progression.stamina == 25
    ...
    def test_stamina_regen_active(self):
        ...
        regen = 1
        e.stats.progression.stamina = min(e.stats.progression.stamina + regen, e.stats.progression.max_stamina)
        assert e.stats.progression.stamina == 21

    def test_stamina_regen_capped(self):
        e = _make_entity(1, stamina=50)
        e.stats.progression.stamina = 49
        regen = 5
        e.stats.progression.stamina = min(e.stats.progression.stamina + regen, e.stats.progression.max_stamina)
        assert e.stats.progression.stamina == 50
...
class TestSkillUsage:
    ...
    def test_skill_use_costs_stamina(self):
        ...
        cost = si.effective_stamina_cost(sdef.progression.stamina_cost)
        e.stats.progression.stamina -= cost
        assert e.stats.progression.stamina == 50 - cost
        assert e.stats.progression.stamina < 50
    ...
    def test_self_skill_heals(self):
        ...
        e.stats.combat.hp = 50  # half HP
        sdef = SKILL_DEFS.get("second_wind")
        assert sdef is not None
        hp_mod = getattr(sdef, 'hp_mod', 0.0) or 0.0
        heal = int(e.stats.combat.max_hp * hp_mod) if hp_mod > 0 else 0
        e.stats.combat.hp = min(e.stats.combat.hp + heal, e.stats.combat.max_hp)
        assert e.stats.combat.hp > 50  # healed some
    ...
    def test_skill_damage_formula(self):
        ...
        raw_dmg = int(attacker.stats.combat.atk * power)
        dmg = max(raw_dmg - defender.stats.combat.def_ // 2, 1)
        defender.stats.combat.hp -= dmg
        assert defender.stats.combat.hp < 100
        assert dmg > 0
...
class TestDamageCalculatorStrategy:
    ...
    def test_physical_resolve_uses_atk_def(self):
        ...
        assert ctx.combat.atk_power == attacker.stats.combat.atk
        assert ctx.combat.def_power == defender.stats.combat.def_
        assert ctx.train_action == "attack"
        # STR=10 → 1.0 + 10*0.02 = 1.20
        assert abs(ctx.combat.atk_mult - 1.20) < 0.001
        # VIT=10 → 1.0 + 10*0.01 = 1.10
        assert abs(ctx.combat.def_mult - 1.10) < 0.001
    ...
    def test_magical_resolve_uses_matk_mdef(self):
        ...
        ctx = calc.resolve(attacker, defender)
        assert ctx.combat.atk_power == attacker.stats.combat.matk
        assert ctx.combat.def_power == defender.stats.combat.mdef
        assert ctx.train_action == "magic_attack"
        # SPI=15 → 1.0 + 15*0.02 = 1.30
        assert abs(ctx.combat.atk_mult - 1.30) < 0.001
        # WIS=12 → 1.0 + 12*0.01 = 1.12
        assert abs(ctx.combat.def_mult - 1.12) < 0.001
    ...
    def test_physical_no_attributes_defaults_mult_to_1(self):
        ...
        calc = PhysicalDamageCalculator()
        ctx = calc.resolve(attacker, defender)
        assert ctx.combat.atk_mult == 1.0
        assert ctx.combat.def_mult == 1.0
    ...
    def test_damage_context_dataclass(self):
        ...
        assert ctx.combat.atk_power == 10
        assert ctx.combat.def_power == 5
        assert ctx.train_action == "attack"
    ...
    def test_physical_higher_str_higher_atk_mult(self):
        ...
        assert ctx_high.combat.atk_mult > ctx_low.combat.atk_mult
...
class TestSkillEffects:
    ...
    def test_self_buff_applies_effect(self):
        ...
        assert e.effects[0].combat.atk_mult == 1.2
        assert e.effects[0].combat.def_mult == 1.5
    ...
    def test_buff_modifies_effective_stats(self):
        ...
        base_atk = e.stats.combat.atk
        base_def = e.stats.combat.def_
        e.effects.append(skill_effect(atk_mod=0.3, def_mod=0.4, duration=3))
        assert e.stats.combat.atk > base_atk
        assert e.stats.combat.def_ > base_def
    ...
    def test_hp_per_tick_dot(self):
        ...
        e.stats.combat.hp = max(0, e.stats.combat.hp + eff.combat.hp_per_tick)
        assert e.stats.combat.hp == 45

    def test_hp_per_tick_regen(self):
        from src.core.gameplay.effects import skill_effect
        e = _make_entity(1, hp=50)
        e.stats.combat.hp = 30
        eff = skill_effect(hp_per_tick=10, duration=3, source="regen")
        e.effects.append(eff)
        e.stats.combat.hp = min(e.stats.combat.hp + eff.combat.hp_per_tick, e.stats.combat.max_hp)
        assert e.stats.combat.hp == 40
    ...
    def test_multiple_effects_stack(self):
        ...
        assert e.stats.combat.atk == int(10 * 1.2 * 1.3)
    ...
    def test_skill_effect_factory_debuff_type(self):
        ...
        assert eff.combat.atk_mult == 0.85
