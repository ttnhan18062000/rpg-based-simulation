...
def test_entity_to_full_schema_no_crash():
    ...
    eb.identity.faction(Faction.HERO_GUILD)
    eb.identity.role(EntityRole.HERO)
...
def test_entity_to_full_schema_minimal():
    ...
    schema = entity.to_full_schema()
    assert schema.id == 2
    assert schema.progression.hero_class == "none"
