...
from src.core.strategic import ProjectState, ProjectStatus, ObjectiveState, ObjectiveStatus, BlockerState, LeadState, LeadCertainty
from src.engine.tactical import TacticalDecisionSystem
from src.systems.strategic import StrategicIntelligenceSystem

def create_mock_entity(id, pos=(0,0), hp=100, faction="HERO_FACTION"):
    return EntityState(
        id=id,
        kind="ACTOR",
        position=pos,
        identity=IdentityComponent(faction=faction, role=EntityRole.HERO),
        combat=CombatComponent(hp=hp, max_hp=100, atk=10, range=1, alive=hp > 0, tactical_role="VANGUARD"),
        readiness=100.0,
        active=True,
...
def test_strategic_inventory_detour():
    ...
    h1 = replace(h1, latest_intent_results=[IntentResult(transaction_id="t1", accepted=False, reason="INVENTORY_FULL", source_kind="NODE", source_id=50)])
...
def test_wound_infliction():
    ...
    assert "WOUND_INFLICTED" in combat_up.trace
...
def test_contract_outcome_consequences():
    ...
    assert any(d.kind == "combat" and d.target == "1" for d in strat_up_b.directives_add_or_update)
