...
def test_routine_service_sleep_bias():
    ...
    # Active hour (12:00)
    biases = RoutineService.calculate_routine_biases(entity, 12)
...
def test_routine_service_forced_rest_during_off_hours():
    ...
    # Off hour (2:00 AM)
    biases = RoutineService.calculate_routine_biases(entity, 2)
    
    assert GoalType.SLEEP in biases
    assert biases[GoalType.SLEEP] == 2.5 # Night bonus
    assert biases[GoalType.COMBAT] == 0.4 # Night penalty
