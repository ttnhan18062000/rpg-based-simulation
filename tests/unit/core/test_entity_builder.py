...
class TestEntityBuilderBasic:
    ...
    def test_ai_state_sets_state(self):
        rng = _FakeRNG()
        entity = EntityBuilder(rng, 1).mind.ai_state(AIState.GUARD_CAMP).build()
        assert entity.mind.ai_state == AIState.GUARD_CAMP

    def test_faction_sets_faction(self):
        rng = _FakeRNG()
        entity = EntityBuilder(rng, 1).identity.faction(Faction.GOBLIN_HORDE).build()
        assert entity.identity.faction == Faction.GOBLIN_HORDE

    def test_tier_sets_tier(self):
        rng = _FakeRNG()
        entity = EntityBuilder(rng, 1).identity.tier(3).build()
...
class TestEntityBuilderStats:
    ...
    def test_with_base_stats(self):
        ...
        assert entity.stats.combat.def_ == 10
        assert entity.stats.combat.spd == 15
        assert entity.stats.combat.luck == 5
        assert abs(entity.stats.crit_rate - 0.1) < 0.001
        assert entity.stats.progression.level == 3
        assert entity.stats.progression.xp_to_next == 300
...
class TestEntityBuilderChaining:
    ...
    def test_full_hero_chain(self):
        ...
        assert entity.stats.combat.hp_regen > 1.0
    ...
    def test_full_mob_chain(self):
        ...
        entity = (
            ...
            .at(Vector2(30, 40))
            .home(Vector2(30, 40))
            .identity.faction(Faction.GOBLIN_HORDE)
            .mind.ai_state(AIState.GUARD_CAMP)
            .identity.tier(2)
    ...
    def test_each_with_method_returns_self(self):
        ...
        assert b.mind.ai_state(AIState.IDLE) is b
        assert b.identity.faction(Faction.HERO_GUILD) is b
        assert b.identity.tier(0) is b
