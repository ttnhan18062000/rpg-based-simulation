...
class TestRecalcDerivedStats:
    ...
    def test_creation_mode_adds_bonuses(self):
        ...
        stats = CombatAspect(max_hp=50, hp=50, atk=10, def_=3, spd=10)
    ...
    def test_delta_mode_strips_old_adds_new(self):
        from src.core.aspects.combat import CombatAspect
        stats = CombatAspect(max_hp=50, hp=50, atk=10)
    ...
    def test_default_attrs_add_bonuses(self):
        from src.core.aspects.combat import CombatAspect
        stats = CombatAspect(max_hp=20, hp=20, atk=5)
...
class TestEntityIntegration:
    def test_entity_copy_includes_home_storage(self):
        ...
        e = EntityBuilder(rng, 1).kind("hero").with_inventory().with_home_storage(30).build()
    ...
    def test_entity_without_home_storage(self):
        ...
        assert e.inventory is None or e.inventory.home_storage is None
        e2 = e.copy()
        assert e2.inventory is None or e2.inventory.home_storage is None
...
class TestTrainWithStats:
    def test_training_updates_stats_on_increment(self):
        ...
        attrs = entity.attributes # uses StatsProxy shim
...
class TestEngagementLock:
    def test_engaged_ticks_on_entity(self):
        ...
        assert e.mind.engaged_ticks == 0
        e.mind.engaged_ticks = 3
        e2 = e.copy()
        assert e2.mind.engaged_ticks == 3

    def test_last_reason_on_entity(self):
        from src.platform.rng import DeterministicRNG
        rng = DeterministicRNG(42)
        e = EntityBuilder(rng, 1).kind("hero").build()
        assert e.mind.last_reason == ""
        e.mind.last_reason = "Heading to store"
        e2 = e.copy()
        assert e2.mind.last_reason == "Heading to store"
