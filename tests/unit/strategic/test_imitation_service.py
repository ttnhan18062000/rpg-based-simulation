...
from src.content.schema import SpeciesDefinition
...
def species_catalog():
    ...
    repo.species = {
        "test_species_high": SpeciesDefinition(
        ...
        "test_species_low": SpeciesDefinition(
            id="test_species_low",
...
def build_entity(entity_id: int, species_id):
    builder = V2EntityBuilder(entity_id).kind("hero").location(0, 0)
    if species_id is not None:
        builder = builder.identity(properties={"species_id": species_id})
    return builder.build()


def test_imitation_scaling_reads_intelligence_tier_high_vs_low(species_catalog):
    high_entity = build_entity(1, "test_species_high")
    low_entity = build_entity(2, "test_species_low")
...
def test_imitation_scaling_missing_species_or_tier_fails_safe(species_catalog):
    no_species_entity = build_entity(1, None)
    assert (
        RoleModelImitationService.compute_imitation_fidelity(no_species_entity)
        == RoleModelImitationService.DEFAULT_FIDELITY
    )

    unresolved_species_entity = build_entity(2, "does_not_exist")
    assert (
        RoleModelImitationService.compute_imitation_fidelity(unresolved_species_entity)
        == RoleModelImitationService.DEFAULT_FIDELITY
    )
