...
def test_hero_trading(system, context):
    ...
    with patch("src.core.gameplay.item_registry.ITEM_REGISTRY") as mock_reg:
        # Mock templates
        iron_t = MagicMock()
        iron_t.item_type = ItemType.WEAPON
        iron_t.combat.atk_bonus = 10
        iron_t.combat.def_bonus = 0
        iron_t.combat.matk_bonus = 0
        iron_t.combat.mdef_bonus = 0
        iron_t.combat.max_hp_bonus = 0
        iron_t.crit_rate_bonus = 0.0
        iron_t.combat.evasion_bonus = 0.0
        iron_t.name = "Iron Sword"
        
        steel_t = MagicMock()
        steel_t.item_type = ItemType.WEAPON
        steel_t.combat.atk_bonus = 20
        steel_t.combat.def_bonus = 0
        steel_t.combat.matk_bonus = 0
        steel_t.combat.mdef_bonus = 0
        steel_t.combat.max_hp_bonus = 0
        steel_t.crit_rate_bonus = 0.0
        steel_t.combat.evasion_bonus = 0.0
