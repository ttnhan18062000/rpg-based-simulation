...
def test_bravery_modifiers(context):
    ...
    hero.stats.combat.atk = 10
    hero.stats.combat.spd = 10
    
    # Default bravery 0.5 -> mult 1.0
    assert hero.stats.combat.atk == 10
    
    # Brave Bonus (> 0.8)
    hero.mind.emotional_state["bravery"] = 0.9
    assert hero.stats.combat.atk == 11 # 10 * 1.1
    assert hero.stats.combat.spd == 11
    
    # Fear Factor (< 0.3)
    hero.mind.emotional_state["bravery"] = 0.2
    assert hero.stats.combat.atk == 9 # 10 * 0.9
    assert hero.stats.combat.spd == 9
    assert hero.stats.combat.def_ == 0 # 0 * 0.9 = 0

def test_regional_suppression(context):
    hero = Entity(id=1, kind="hero", faction=Faction.HERO_GUILD)
    hero.current_region_id = "forest"
    ...
    hero.stats.combat.atk = 100
    # atk should be 100 * 0.8 = 80
    assert hero.stats.combat.atk == 80
