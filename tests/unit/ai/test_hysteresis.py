...
from src.ai.goals.scorers import FleeGoal, CombatGoal
...
def _make_hero(
    ...
    stats = Stats(hp=hp, max_hp=max_hp, atk=10, def_=5, spd=10, gold=100)
    inv = Inventory(items=[], max_slots=12, max_weight=100.0)
    entity = Entity(
        id=eid, kind="hero", pos=Vector2(5, 5),
        stats=stats, faction=Faction.HERO_GUILD,
        inventory=inv,
    )
    entity.mind.ai_state = ai_state
    entity.mind.last_goal = last_goal
    entity.mind.goal_committed_at = goal_committed_at
    return entity
...
class TestCooldownPenalty:
    ...
    def test_cooldown_penalty_applied_to_abandoned_goal(self):
        ...
        hero.mind.goal_cooldowns = {"combat": 25}  # cooldown until tick 25
        world.add_entity(hero)
    ...
    def test_cooldown_does_not_affect_non_cooled_goals(self):
        ...
        world = _make_world(tick=20)
        hero = _make_hero()
        hero.mind.goal_cooldowns = {"combat": 25}
        world.add_entity(hero)
        ctx = _make_ctx(hero, world)
