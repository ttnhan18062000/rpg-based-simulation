...
def test_contract_appraisal_trust():
    ...
    contract = ContractService.create_recruitment_contract("c1", 99, 1, daily_pay=5, tick=100)
    
    state = AuthoritativeState(entities={1: friend}, tick=100, seed=1)
    
    should_accept, reason = SocialAppraisalSystem.appraise_contract(friend, contract, state)
...
def test_contract_appraisal_greed():
    ...
    should_accept, reason = SocialAppraisalSystem.appraise_contract(desperate, contract, state)
    assert should_accept is True
    assert reason == "FAIR_COMPENSATION"

def test_contract_appraisal_rejection():
    # Scenario: Low trust stranger offers low pay
    enemy = create_mock_entity(1, sentiment=-0.6)
    ...
    should_accept, reason = SocialAppraisalSystem.appraise_contract(enemy, contract, state)
...
def test_contract_lifecycle_resolution():
    ...
    assert b_upds[0].sentiment_delta == 0.1
    ...
    assert b_upds[0].sentiment_delta == -1.0
