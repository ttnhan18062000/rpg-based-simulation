...
def create_mock_entity(id, gold=10, hp=100, sentiment=0.0):
    ...
    entity = replace(entity, 
        inventory=replace(entity.inventory, gold=gold),
        combat=replace(entity.combat, hp=hp)
    )
    return entity
...
def test_appraisal_betrayal_rejection():
    ...
    candidate = create_mock_entity(1, gold=0)
    candidate = replace(candidate, social=replace(candidate.social, betrayal_count=1))
    
    # Recruiter (99) is specifically distrusted due to past betrayal
    candidate = replace(candidate, social=replace(candidate.social, 
        bonds={99: SocialBond(target_id=99, sentiment=-1.0)}
    ))
...
def test_offer_expiration_logic():
    ...
    e1 = replace(e1, strategic=replace(e1.strategic, contracts={"c_expired": contract}))
