"""AC #3 hand-orchestrated metamorphic validation for race hostility.
...
`race_relations.yaml` lives entirely outside both). This test bypasses that pipeline
entirely and hand-orchestrates the same validation `MetamorphicRuleEngine.evaluate_rules()`
provides: physically swap the on-disk `data/content/social/race_relations.yaml` wolf<->human
...
RACE_RELATIONS_PATH = "data/content/social/race_relations.yaml"
SCENARIO_ID = "race_hostility_metamorphic_check"
EXPERIMENT_ID = "race_hostility_metamorphic_check_experiment"
SEEDS = [201, 202, 203]
TICKS = 200

SCENARIO_DIR = f"data/scenarios/{SCENARIO_ID}"
EXPERIMENT_DIR = f"data/experiments/{EXPERIMENT_ID}"
LAB_RUN_BASELINE = "data/lab_runs/race_hostility_baseline"
LAB_RUN_HIGH = "data/lab_runs/race_hostility_high"
...
def test_race_hostility_increase_does_not_decrease_combat_engagement_rate():
    with open(RACE_RELATIONS_PATH, "r", encoding="utf-8") as f:
    ...
    scenario_spec = ScenarioSpec(
        ...
        name="Race Hostility Metamorphic Check",
        world_id="unit_faction_tension",
        scenario_type="sandbox",
        intent={"primary_goal": "race_hostility_metamorphic_check"},
        expected_behavior={},
    ...
    try:
        # Baseline variant: no wolf<->human race_relations entries at all.
        with open(RACE_RELATIONS_PATH, "w", encoding="utf-8") as f:
        ...
        baseline_orchestrator.run_lab(EXPERIMENT_ID, lab_run_id="race_hostility_baseline")
        baseline_engagements = _count_combat_engagements("race_hostility_baseline")
        ...
        high_orchestrator.run_lab(EXPERIMENT_ID, lab_run_id="race_hostility_high")
        high_engagements = _count_combat_engagements("race_hostility_high")
        ...
        assert result.status == "PASSED", (
            f"Increasing wolf<->human race hostility decreased combat_engagement_rate: "
        ...
        assert restored_content == original_content, (
            "data/content/social/race_relations.yaml was not restored to its authored state "
