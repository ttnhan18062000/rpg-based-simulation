...
def test_calamity_spawning(basic_setup):
    ...
    assert boss.stats.combat.hp >= 500, f"Boss stats too low: {boss.stats}. Kind: {boss.kind}, BossFlag: {boss.identity.is_world_boss}"
    assert boss.stats.combat.atk >= 100 # 20 base * 5x mult
...
def test_calamity_bounty_generation(basic_setup):
    ...
    hero.stats.progression.level = 20  # Minimum for calamity_hunter quest
...
def test_calamity_kill_rewards(basic_setup):
    ...
    boss.stats.combat.hp = 1 # One-shot for testing
    ...
    assert hero.stats.combat.atk > 0 
