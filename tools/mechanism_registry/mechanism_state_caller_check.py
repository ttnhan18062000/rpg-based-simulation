...
Four checks, in the order peer named them (cheapest payoff first):
  ...
  4. declared `skeleton` whose implementing file's real body looks substantial, not a stub --
     weaker signal, labeled low-confidence in the report.
...
def _is_stub_body(path: Path) -> bool:
    """A crude 'this looks like a real stub' check: strip blank lines, comments, and docstring
    delimiters, then see if anything but pass/... /raise NotImplementedError/class or def headers
    remains."""
    try:
        lines = path.read_text(encoding="utf-8").splitlines()
    except OSError:
    ...
    for line in lines:
        ...
        if stripped in ("pass", "...", '"""', "'''"):
            continue
        if re.match(r"^(class|def|from|import|@)\b", stripped):
            continue
        if "NotImplementedError" in stripped:
            continue
        substantive += 1
    return substantive < 8
...
def check_mechanism(mechanism: dict, source_files: List[Path]) -> List[Finding]:
    ...
    for rel_path in implemented_by:
        ...
        if not defining_file.is_file():
            continue
        symbols = _symbol_names(defining_file)
        ...
        if not _is_stub_body(defining_file):
            all_stub = False
    ...
    if state == "skeleton" and not all_stub:
        findings.append(Finding(
            mid, "skeleton_not_stub", state,
            f"declared skeleton but {implemented_by}'s own body does not look like a stub "
            f"(8+ substantive non-boilerplate lines)",
            "low",
        ))
