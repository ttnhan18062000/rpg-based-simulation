...
# own tools/start_search_mcp.sh), so a git-worktree session's $REPO_ROOT/.venv never has the
# knowledge deps installed. Checking the absolute shared path first makes every worktree's MCP
