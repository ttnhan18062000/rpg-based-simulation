...
class ActionSystem(System):
    ...
    def _try_loot_chest(self, context: SystemContext, entity: Any, pos: Any) -> None:
        ...
        for chest in world.treasure_chests.values():
            if chest.spatial.pos == pos and chest.is_available:
                # Check guard
                ...
                # Generate loot
                loot_table = CHEST_LOOT_TABLES.get(chest.identity.tier, [])
                loot_items = []
    ...
    def _handle_use_skill(self, context: SystemContext, entity: Any, skill_id: str, target_id: int | None = None) -> None:
        ...
        if not sdef or sdef.skill_type != SkillType.ACTIVE:
            print(f"DEBUG_ACTION_SKILL: Invalid sdef for {skill_id}")
            return
        ...
        print(f"DEBUG_ACTION_SKILL: Entity {entity.id} used {skill_id} on {len(targets)} targets")
        ...
        targets = self._resolve_skill_targets(context, entity, sdef, target_id)
        if not targets:
            print(f"DEBUG_SKILL: Entity {entity.id} used {skill_id} but found 0 TARGETS")
        else:
            print(f"DEBUG_SKILL: Entity {entity.id} using {skill_id} hit {len(targets)} targets: {[t.id for t in targets]}")
    ...
    def _resolve_skill_targets(self, context: SystemContext, entity: Any, sdef: Any, primary_target_id: int | None = None) -> list[Any]:
        ...
        from src.core.gameplay.faction import Faction
        world = context.world
        reg = context.identity  # FactionRegistry is passed as 'identity' in SystemContext
        targets = []
        ...
        primary_target_id = primary_target_id or entity.combat_target_id
    ...
    def _find_nearest_hostile(self, context: SystemContext, entity: Any) -> Any:
        ...
        best_dist = 999
        best_target = None
        for oid in world.spatial_index.query_radius(entity.spatial.pos, entity.combat.vision_range):
            other = world.entities.get(oid)
            ...
            if d < best_dist:
                ...
                best_target = other
        if best_target:
            print(f"DEBUG_PICK: Entity {entity.id} CHOSE {best_target.id} (dist={best_dist})")
        return best_target
