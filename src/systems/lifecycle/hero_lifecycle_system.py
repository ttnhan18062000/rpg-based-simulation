...
class HeroLifecycleSystem(System):
    ...
    def _tick_hero_trading(self, ctx: SystemContext, tick: int) -> None:
        ...
        def item_pwr(template: ItemTemplate) -> int:
            return (template.combat.atk_bonus + template.combat.def_bonus + template.combat.matk_bonus + template.combat.mdef_bonus + 
                    template.combat.max_hp_bonus // 5 + int(template.crit_rate_bonus * 100) + 
                    int(template.combat.evasion_bonus * 100))
