...
class CombatAction:
    ...
    def apply(self, proposal: ActionProposal, world: WorldState) -> None:
        ...
        logger.info(
            "TICK_ACTION_COMBAT: [%d]%s(L%d) hit [%d]%s(L%d) for %d dmg. Def HP: %d/%d",
        ...
        if defender.combat.alive:
            ...
            if defender.combat.hp_ratio < 0.15:
                ...
                logger.info("TICK_ACTION_COMBAT: [%d] %s survived near-death, MaxHP increased to %d",
        ...
        if not defender.combat.alive:
            ...
            if hasattr(world, "corpse_nodes") and (defender.identity.role == "hero" or defender.identity.is_world_boss):
                ...
                logger.info("Tick %d: Entity %d died -> Created CorpseNode #%d at %s", tick, defender.id, node_id, defender.spatial.pos)
            else:
