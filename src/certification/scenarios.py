...
class ArenaInjector:
    ...
    def build_mortality_test(state: AuthoritativeState) -> AuthoritativeState:
        ...
        boss = (V2EntityBuilder(3)
            .kind("MONSTER")
            .location(10.0, 10.0)
            .identity(evolution_level=10, faction=Faction.MONSTER_HORDE)
            .combat(hp=1000, max_hp=1000)
            .combat(atk=500, def_stat=100)
            .combat(readiness=100.0)
            .build())
    ...
    def build_social_test(state: AuthoritativeState) -> AuthoritativeState:
        ...
        hero1 = (V2EntityBuilder(8)
            ...
            .combat(hp=100, max_hp=100)
            .combat(atk=10, def_stat=10)
            .social(bonds={9: SocialBond(target_id=9, familiarity=0.8)})
            .combat(readiness=100.0)
            .build())
...
class GameplayInjector:
    ...
    def build_harvest_test(state: AuthoritativeState, required_ticks: int = 3) -> AuthoritativeState:
        ...
        actor = (V2EntityBuilder(1)
            .kind("actor")
            .location(0.0, 1.0)
            .interaction(target_node_id=100, progress=0)
