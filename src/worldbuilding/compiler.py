...
# than hardcoded here, so designers can retune without a code change. These module-level
# fallbacks match the shipped data file exactly and are used only if that file is missing or
# malformed, so world compilation never crashes on a bad/absent data file.
...
def get_action_style_for_bravery(bravery: float) -> int:
    ...
    the real thresholds in personality_bias.yaml. Both entity-construction paths previously left
