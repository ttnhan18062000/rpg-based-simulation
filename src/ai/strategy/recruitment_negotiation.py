...
class RecruitmentNegotiationService:
    ...
    def create_offer(
        ...
        terms.append(ContractTermRecord(
            term_id="payout_share",
            value=share,
            is_negotiable=True
        ))
        
        # Role Expectation
        role = "vanguard" if project.metadata.get("risk", 0) < 0.5 else "support"
        terms.append(ContractTermRecord(
            term_id="preferred_role",
            value=role,
            is_negotiable=True
        ))
        ...
        return RecruitmentOfferRecord(
            ...
            contract_id=project.project_id, # Link to project originally? No, project might not be contract yet.
            kind=kind,
            founder_id=actor.id,
            candidate_id=candidate_id,
            status=OfferStatus.PENDING,
            terms=terms,
            expires_at=ctx.snapshot.tick + 100 # 10 ticks = 1 hour, so 10 hours approx
        )
    ...
    def evaluate_offer(
        ...
        actor = ctx.actor
        founder = ctx.snapshot.entities.get(offer.founder_id)
        if not founder:
            return False
            
        # 1. Base Willingness from Relationship
        willingness = 0.0
        bond = actor.mind.social.known_bonds.get(offer.founder_id)
        if bond:
