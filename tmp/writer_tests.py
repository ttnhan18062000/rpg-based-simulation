...
from src.core.enums import ActionType, AIState, EntityRole, HeroClass
...
def basic_setup():
    ...
    # Add some buildings
    b1 = Building(building_id="inn_1", name="Inn", pos=Vector2(5, 5), durability=100.0, max_durability=100.0)
    b2 = Building(building_id="shop_1", name="Shop", pos=Vector2(6, 6), durability=100.0, max_durability=100.0)
...
def test_repair_action(basic_setup):
    ...
    hero = Entity(1, pos=Vector2(5, 6))
    hero.stats.gold = 20.0
    ...
    assert b.durability == 100.0
    assert hero.stats.gold == 10.0

def test_monument_spawning_on_permadeath(basic_setup):
    loop, world, config = basic_setup
    # Mock a hero dying permanently
    hero = Entity(1, pos=Vector2(5, 5))
    hero.display_name = "Legend"
    hero.stats.level = 20
    hero.hero_class = HeroClass.WARRIOR
    hero.death_count = config.death_tier_max - 1
    hero.home_pos = Vector2(0, 0)
    world.add_entity(hero)
...
def test_monument_buff_application(basic_setup):
    ...
    # Spawn new hero
    new_hero = Entity(2, pos=Vector2(0,0))
    new_hero.stats.atk = 10
    
    loop._apply_monument_buffs(new_hero)
    assert new_hero.stats.atk == 11 # 10 * 1.1
...
def test_defeat_condition(basic_setup):
    ...
    b3 = Building(building_id="b3", name="B3", pos=Vector2(0,0), durability=0.0, max_durability=100.0)
...
path = "tests/test_epic_19_phase_3.py"
