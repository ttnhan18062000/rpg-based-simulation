...
- `engine/world_loop.py`
  Atomic checklist:
- [ ] Action proposals are typed intents, not direct world mutation.
...
- [ ] World mutation happens after proposal generation, not inside worker thought code.
- [ ] Action application supports partial rejection without corrupting unrelated update domains.
- [ ] Conflict resolution preserves one authoritative outcome per tick.
- [ ] Worker decision-making is decoupled from authoritative application.
- [ ] Replay and observability consume authoritative results rather than defining them.
...
- `core/models/combat.py`
  Atomic checklist:
- [ ] Manhattan distance is the shared spatial metric for movement and combat range where claimed.
- [ ] Cardinal/tile movement and occupancy legality are explicit.
- [ ] Occupied-tile movement is rejected or redirected rather than silently overlapped.
...
- [ ] Quiet ticks still advance passive world consequences.
...
- [ ] Tactical choice is a bounded choice among legal actions, not a geometry exploit.
...
- `core/gameplay/items/items.py`
  Atomic checklist:
- [ ] Looting is a channeled state with progress, interruption, and completion semantics.
- [ ] Harvesting is a channeled state tied to nearby resource-node legality and harvest duration.
- [ ] Loot/harvest can abort because of inventory slot pressure.
- [ ] Loot/harvest can abort because of inventory weight pressure.
- [ ] Inventory state tracks both slots and weight/carry burden.
- [ ] Ground items, node yields, and inventory additions/removals are authoritative side effects.
- [ ] Town return is a real gameplay state, not a cosmetic teleport.
- [ ] Shop visits resolve bounded buy/sell behavior using inventory/gold truth.
...
- [ ] Inn/home/class-hall visits have distinct progression or recovery semantics.
- [ ] Building interactions are explicit gameplay slices, not generic proximity triggers.
...
- `systems/social/knowledge_propagation_system.py`
  Atomic checklist:
- [ ] Strategic state is first-class and survives across ticks (directives, projects, objectives, concerns, blockers, obligations, contracts, offers, leads, candidate zones, hypotheses).
- [ ] Current project/objective continuity is explicit and bounded.
...
- [ ] Blockers are inferred from project/objective state and can be accurate or misdiagnosed under bounded cognition.
...
- [ ] `test_belief_refresh_captures_apparent_state`: belief refresh from apparent state
- [ ] `test_belief_decay_lifecycle`: belief decay lifecycle
...
- [ ] `test_sleep_goal_utility_at_night`: sleep utility at night
- [ ] `test_rest_to_sleep_transition`: rest-to-sleep transition
- [ ] `test_routine_service_sleep_bias`: sleep bias
- [ ] `test_routine_service_forced_rest_during_off_hours`: forced off-hours rest
- [ ] `test_routine_service_hunger_bias`: hunger bias
...
- [ ] `test_regional_suppression`: regional suppression
- [ ] `test_strategic_pivot_on_regional_danger`: pivot on regional danger
- [ ] `test_region_fatigue_biasing`: region fatigue biasing
- [ ] `test_region_consequence_record`: region consequence record
...
- [ ] `test_near_death_triggers_survival_consequences`: near-death survival consequences
- [ ] `test_near_death_hardening`: near-death hardening
...
- [ ] `test_role_bias_influence`: role bias affects decisions as expected
- [ ] `test_role_aware_tactical_biases`: tactical behavior reflects role-aware biasing
