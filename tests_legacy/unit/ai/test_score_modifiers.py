...
from src_legacy.ai.goals.base import GoalScore, GoalEvaluator
from src_legacy.core.models.enums import AIState, GoalType

def test_boredom_modifier_applies_multipliers():
    from src_legacy.ai.goals.base import BoredomModifier
    
    # Setup
    actor = MagicMock()
    actor.mind.decision.boredom_multipliers = {GoalType.COMBAT: 0.5, GoalType.EXPLORE: 1.2}
    ctx = MagicMock()
    ctx.actor = actor
...
def test_goal_evaluator_uses_modifiers():
    from src_legacy.ai.goals.base import ScoreModifier
    ...
    ctx.actor.mind.decision.boredom_multipliers = {}
    
    from src_legacy.ai.goals import base
    from src_legacy.ai.goals.base import GoalScorer
    
    class SimpleScorer(GoalScorer):
        @property
        def name(self): return GoalType.COMBAT
        @property
