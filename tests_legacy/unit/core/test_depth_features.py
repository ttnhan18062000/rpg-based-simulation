...
from src_legacy.core.gameplay.effects import well_rested_effect, StatusEffect, EffectType
from src_legacy.core.models.enums import HeroClass
from src_legacy.core.gameplay.items.items import _item_power
from src_legacy.core.gameplay.attributes import Attributes
...
def test_attribute_synergy_xp_mult():
    ...
    mult = e.progression.xp_mult
    assert abs(mult - 1.30) < 0.001
    
    # Add Well-Rested (* 1.2)
    e.combat.effects.append(well_rested_effect())
    # 1.30 * 1.2 = 1.56
    assert abs(e.progression.xp_mult - 1.56) < 0.001
