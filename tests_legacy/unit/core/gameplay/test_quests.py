...
from src_legacy.core.gameplay.quests import (
...
class _FakeRNG:
    ...
    def next_float(self, domain: object, entity_id: int, tick: int) -> float:
        val = self._values[self._idx % len(self._values)]
        self._idx += 1
        return (val % 100) / 100.0

    def next_bool(self, domain: object, entity_id: int, tick: int, probability: float = 0.5) -> bool:
