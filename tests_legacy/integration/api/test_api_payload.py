...
class TestStaticEndpoint(unittest.TestCase):
    ...
    def test_world_state_response_has_dynamic_fields(self):
        """WorldStateResponse schema should have dynamic state for buildings/resources/chests."""
        fields = set(WorldStateResponse.model_fields.keys())
        for f in ("buildings", "resource_nodes", "treasure_chests"):
            self.assertIn(f, fields)
        # regions is still purely static for now
