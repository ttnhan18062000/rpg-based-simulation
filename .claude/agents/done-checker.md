...
3. **Ticket is updated and in `tickets/inprogress/`**
   - Status must be `INPROGRESS` or ready to move to DONE.
   - Files Changed, Implementation Notes, Test Summary sections are filled in.

4. **Staging artifacts complete in `staging_artifacts/{ticket_id}/`**
   - `plan.md`, `investigation.md`, `test_plan.md` must exist and be non-empty.
