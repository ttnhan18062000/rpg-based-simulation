...
def test_json_logging_format():
    ...
    cmd = ["python3", "-m", "src_v2", "--json-logs", "cli", "--ticks", "1"]
...
def test_telemetry_disabled_flag():
    ...
    cmd = ["python3", "-m", "src_v2", "cli", "--ticks", "1", "--log-level", "INFO"]
...
def test_logging_context_injection():
    ...
    cmd = ["python3", "-m", "src_v2", "--json-logs", "cli", "--ticks", "10", "--log-level", "INFO"]
    result = subprocess.run(cmd, capture_output=True, text=True)
    ...
    kernel_logs = [l for l in json_lines if l.get("component") == "src_v2.engine.kernel"]
...
def test_replay_artifact_integrity():
    ...
    cmd = ["python3", "-m", "src_v2", "cli", "--ticks", "5", "--replay", replay_dir]
