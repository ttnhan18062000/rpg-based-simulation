...
def test_broker_disabled_flag():
    ...
    cmd = ["python3", "-m", "src_v2", "cli", "--ticks", "1", "--workers", "4"]
    result = subprocess.run(cmd, env=env, capture_output=True, text=True)
...
""")
    ...
    try:
        cmd = ["python3", "-m", "src_v2", "--config", yaml_path, "cli", "--ticks", "1"]
        ...
        cmd = ["python3", "-m", "src_v2", "--config", yaml_path, "cli", "--ticks", "1", "--workers", "9"]
        result = subprocess.run(cmd, capture_output=True, text=True)
...
def test_safe_import_isolation():
    ...
    """
    # Verify that src_v2.engine.kernel doesn't fail on import
    import src_v2.engine.kernel
    assert src_v2.engine.kernel is not None
