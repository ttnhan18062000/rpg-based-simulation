...
from src_v2.core.state import EntityState, IdentityComponent, CombatComponent, AuthoritativeState, TaskComponent
...
def test_retreat_behavior():
    ...
    update = TacticalDecisionSystem.evaluate_entity_intent(state, attacker)
    assert update.task.work_kind_set == "ENTITY_MOVE"
    assert update.task.payload_set["reason"] == "RETREAT"
