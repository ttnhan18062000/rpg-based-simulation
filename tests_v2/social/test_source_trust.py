...
from src_v2.core.strategic import StrategicComponent, SourceTrustEntry
...
class TestSocialContracts:
    ...
    def test_honored_contract_boosts_trust(self):
        from src_v2.systems.social import SocialContract
...
class TestFamiliarity:
    ...
    def test_positive_interaction_builds_familiarity(self):
        entity = EntityState(id=1, kind="hero", position=(5.0, 5.0))
        bond = SocialAppraisalSystem.update_familiarity(
            observer=entity, subject_id=42,
            interaction_quality=1.0, current_tick=100
        )
        assert bond.familiarity > 0
        assert bond.sentiment > 0
        assert bond.interaction_count == 1

    def test_cha_modifier_scales_gain(self):
        entity = EntityState(id=1, kind="hero", position=(5.0, 5.0))
        bond_low = SocialAppraisalSystem.update_familiarity(
            entity, 42, 1.0, 100, cha_modifier=0.5
        )
        bond_high = SocialAppraisalSystem.update_familiarity(
            entity, 42, 1.0, 100, cha_modifier=2.0
        )
        assert bond_high.familiarity > bond_low.familiarity
