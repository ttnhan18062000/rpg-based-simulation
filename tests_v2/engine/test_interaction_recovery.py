import pytest
from src_v2.core.state import AuthoritativeState, EntityState, InventoryComponent, InteractionComponent, ResourceNodeState
from src_v2.core.updates import StateUpdate, EntityUpdate, InteractionUpdate, InventoryUpdate
...
def test_weight_pressure_enforcement():
    ...
    inv = InventoryComponent(items=["WOOD", "WOOD"], current_weight=4.0, max_weight=5.0)
...
def test_channeled_looting_one_shot():
    ...
    inv = InventoryComponent(items=[], current_weight=0.0, max_weight=100.0)
    ...
    assert "GOLD" in refined.entity_updates[1].inventory.items_added

def test_town_resolution_sell_on_entry():
    # Setup state: Entity with materials entering town
    inv = InventoryComponent(items=["WOOD", "ORE"], current_weight=7.0, max_weight=100.0)
