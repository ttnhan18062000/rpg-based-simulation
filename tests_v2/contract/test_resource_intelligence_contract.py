...
from src_v2.core.updates import StateUpdate, EntityUpdate, IdentityUpdate, InventoryUpdate
...
def test_law_of_necessity_blocker_generation(kernel):
    ...
    kernel._state = replace(kernel._state, town_tiles={(0,0)})
...
def test_law_of_discovery_blocker_resolution(kernel):
    ...
    from src_v2.core.strategic import BlockerState
    ...
    update = StateUpdate(
        entity_updates={
            1: EntityUpdate(
                entity_id=1,
                inventory=InventoryUpdate(items_added=["iron_ore"])
            )
        }
    )
