...
from src_v2.core.updates import StateUpdate, EntityUpdate, InventoryUpdate, IdentityUpdate
...
def test_shop_sell_price_enforcement():
    ...
    assert sorted(e_upd.inventory.items_removed) == ["wood", "wood"]
...
def test_shop_junk_auto_sell():
    ...
    assert sorted(e_upd.inventory.items_removed) == ["iron_sword", "wood"]
...
def test_blacksmith_material_consumption():
    ...
    assert sorted(e_upd.inventory.items_removed) == ["iron_ore", "iron_ore", "wood"]
    assert e_upd.inventory.items_added == ["steel_sword"]
