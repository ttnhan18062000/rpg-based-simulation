...
from src_v2.core.state import AuthoritativeState, EntityState, IdentityComponent, InventoryComponent, ResourceNodeState, StrategicComponent
...
from src_v2.core.strategic import LeadState
    ...
    return AuthoritativeState(
        ...
        entities={
            1: EntityState(
                ...
                inventory=InventoryComponent(gold=100, items=["iron_ore", "wood"])
            )
        },
        resource_nodes={
...
def test_full_resource_progression_loop(profile, initial_state):
    ...
    kernel.tick_once()
    hero = kernel.state.entities[1]
    assert "steel_sword" in hero.inventory.items
