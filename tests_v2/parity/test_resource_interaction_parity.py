...
from src_v2.core.state import AuthoritativeState, EntityState, ResourceNodeState, InteractionComponent, InventoryComponent
from src_v2.core.updates import StateUpdate, EntityUpdate, InteractionUpdate
from src_v2.engine.interaction import InteractionSystem

# Constants
RESULTS_PATH = "tests_v2/parity/interaction_oracle/results.json"
...
def get_v2_setup(scenario_name: str, oracle: Dict[str, Any]):
    ...
    actor = EntityState(
        ...
        inventory=InventoryComponent(max_slots=10, current_slots_used=0)
    )
    ...
    if scenario_name == "harvest_done":
        ...
        entities[1] = EntityState(
            id=1, kind="hero", position=(1.0, 1.0),
            inventory=InventoryComponent(max_slots=10, current_slots_used=10) # Full
        )
...
def test_resource_interaction_parity_with_oracle(oracle):
    ...
    if scenario_name == "harvest_tick_1":
        ...
        assert "WOOD" in e_upd.inventory.items_added
