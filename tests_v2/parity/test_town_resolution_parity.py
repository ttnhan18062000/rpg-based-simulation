...
from src_v2.core.state import AuthoritativeState, EntityState, InventoryComponent, IdentityComponent
...
def load_oracle():
    with open("tests_v2/parity/town_oracle/results.json", "r") as f:
        return json.load(f)
...
def test_town_resolution_scenario_parity(scenario):
    ...
    actual_removed = sorted(inv_upd.items_removed) if inv_upd else []
    expected_removed = sorted([i for i in expected["items_removed"] if i != "gold"])
    assert actual_removed == expected_removed, f"Removed items parity failed in {scenario_id}"
    
    actual_added = sorted(inv_upd.items_added) if inv_upd else []
    expected_added = sorted([i for i in expected["items_added"] if i != "gold"])
    assert actual_added == expected_added, f"Added items parity failed in {scenario_id}"
