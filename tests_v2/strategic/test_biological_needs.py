...
from src_v2.core.state import EntityState, BiologicalComponent, AuthoritativeState
...
def test_routine_action_execution():
    ...
    assert upd_sleep.biological is not None
    assert upd_sleep.biological.sleep_debt_delta == -20.0
    assert upd_sleep.biological.last_sleep_tick_set == 500
    
    # 2. Test EAT
    upd_eat = SimulationDomainLogic.execute_action(ent, {"action": "EAT"}, current_tick=600)
    assert upd_eat.biological is not None
    assert upd_eat.biological.hunger_delta == -40.0
    assert upd_eat.biological.last_meal_tick_set == 600
