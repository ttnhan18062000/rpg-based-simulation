import pytest
from src_v2.core.state import AuthoritativeState, EntityState, InteractionComponent, InventoryComponent, ResourceNodeState
...
def test_interaction_channeling_success():
    ...
    assert ent_upd.inventory.items_added == ["herb"]
...
def test_interaction_inventory_pressure():
    ...
    entity = EntityState(id=10, kind="hero", position=(0,0), 
                         inventory=InventoryComponent(max_slots=1, current_slots_used=1, items=["old_item"]))
    state = AuthoritativeState(tick=10, seed=42, 
                               entities={10: entity}, 
                               resource_nodes={1: node})
