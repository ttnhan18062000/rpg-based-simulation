...
# Plan — TCK-20260817-ENGINE-LIVENESS-HEALTH-EPIC

Scope-only epic. Plan is the ticket file plus `docs/plans/engine_liveness_health_epic.md` — no
code change, no `create-tickets` run yet. When this epic is chosen for action, write a fresh
proposal document scoped to its items only and run `create-tickets` against it, producing
investigated child tickets in `tickets/todos/engine-liveness-health/`.
