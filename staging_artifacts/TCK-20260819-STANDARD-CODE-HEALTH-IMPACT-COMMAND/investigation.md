...
1. **`graphify-out/`'s dependency-graph edge data** — exists, but **no ready-made CLI verb finds
   "dependents of path X."** `graphify --help` was checked directly: the CLI offers `path "A" "B"`
   (shortest path between two *named* nodes) and `explain "X"` (one node's immediate neighbors),
   nothing that takes a file path and returns its direct/transitive dependents. **Real
   implementation requirement, not just wiring**: this command needs to load
   `graphify-out/graph.json` directly and do its own graph traversal (BFS over edges whose target
   is the file's node, filtered to import/use edge types) — the epic doc's "composable" framing
   undersells this piece slightly.
