...
- **A — Adopt the World Evolution Bible's 2,400-ticks/day as sole authority; migrate raid's 100-tick alias
  ...
  (Recommended.) The Bible is already the documented, cited-elsewhere authority (`rpg_design_roadmap.md`
...
- **A — Migrate age representation to fantasy-year units once Decision 1 lands, per the proposal's own §9.2
  direction, rather than inventing new tick counts.** (Recommended, and blocked on Decision 1.) Doesn't pick
...
- **A — Generalize movement's existing readiness-cost pattern (`base_cost × modifier / rate`) as the shared
  ...
  set.** (Recommended.) Reuses a real, tested pattern rather than inventing one from nothing — matches the
...
Present Decisions 1-4 to the plan owner (options + recommendation, as above) for accept/redirect/defer, then
record the outcome in `rpg_design_roadmap.md`'s "Temporal axis" section (or a new `docs/mechanics/` chapter,
per this ticket's own Open Question on authoritative-home) before moving to `tickets/done/`.
