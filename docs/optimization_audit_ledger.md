...
| OPT-001 | CandidateSelector O(1) dirty filtering | `src/engine/candidate_selector.py` | `tests/unit/optimization/test_candidate_selector.py` | PROVEN | Unit / Regression |
| OPT-002 | Full-Scan O(N) compliance elimination | `src/engine/pipeline.py` | `tests/unit/optimization/test_full_scan_compliance.py` | PROVEN | Unit / Regression |
...
| OPT-004 | DirtyDependencyGraph hierarchical tracking | `src/core/dirty.py` | `tests/unit/optimization/test_dirty_dependency_graph.py` | PROVEN | Unit / Regression |
| OPT-005 | StateUpdateCompactor & ApplyPath parity | `src/engine/compactor.py` | `tests/unit/optimization/test_compactor.py` | PROVEN | Unit / Differential |
| OPT-006 | Spatial Movement candidate narrowing | `src/engine/pipeline_phases/movement.py` | `tests/unit/optimization/test_movement_candidate_selector.py` | PROVEN | Unit / Regression |
| OPT-007 | OccupancySnapshot immutable spatial read model | `src/engine/occupancy_snapshot.py` | `tests/unit/optimization/test_occupancy_snapshot.py` | PROVEN | Unit / Regression |
| OPT-008 | MovementPlanCache multi-step path caching | `src/engine/movement_cache.py` | `tests/unit/optimization/test_movement_plan_cache.py` | PROVEN | Unit / Regression |
| OPT-009 | WorldIndexService dirty-domain spatial indexes | `src/engine/world_index.py` | `tests/unit/optimization/test_world_index_service.py` | PROVEN | Unit / Regression |
| OPT-010 | CacheInvalidationPolicy centralized rules | `src/engine/world_index.py` | `tests/unit/optimization/test_cache_invalidation_policy.py` | PROVEN | Unit / Regression |
| OPT-011 | StrategicWorkQueue bounded cognition | `src/systems/strategic_systems/work_queue.py` | `tests/unit/optimization/test_strategic_work_queue.py` | PROVEN | Unit / Regression |
