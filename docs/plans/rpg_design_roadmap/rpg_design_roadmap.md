...
1. **Social/Reputation/Political — no Mechanics Bible chapter, the widest blast-radius gap found.**
   ...
   Corrected the initial framing (only 94 of `social_narrative.yaml`'s 276 entries are genuinely social
   content, not all 276) and found the real gap is fragmentation — three partial contracts already exist,
   but the foundational `SocialComponent`/`SocialBond`/`RelationshipService` state has zero contract
...
2. **Spatial index — the "spatial-reach" half of the pair this roadmap's own M2 section names alongside
   temporal timing.** Two P0 `substrate.yaml` entries (`SUB-325`/`SUB-326`) are `status: missing` with
   `test_path: null`, cited only by an unverifiable "proven via exhaustive checklist audit" note — not yet
   investigated.
