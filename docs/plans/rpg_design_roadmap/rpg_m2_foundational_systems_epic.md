...
1. **Idea 43 — Seed `population_cohorts` at world-compile time.** Cited by 5 later ideas; also the single
   ...
   would need non-trivial rework once that migration lands. **Cadence-coupled rate finding (code-verified,
   2026-08-31 — see the parent roadmap's "§9.3/§9.4 conflicts — code-verified" entry):**
   `PopulationCohort.birth_rate`/`mortality_rate` are defined and coded as "per 200-tick cycle" — the rate's
   real-world meaning is tied directly to `DemographicCycleService`'s evaluation cadence
   (`COHORT_INTERVAL=200`), not a calendar-independent duration unit (e.g. births/year). This is exactly the
   fields this ticket seeds at world-compile time, so seed values chosen now inherit that ambiguity; either
   state the intended real-world meaning explicitly when choosing seed numbers, or flag it as a known gap the
   future calendar-authority migration ticket must resolve — don't let a compile-time seed value quietly
   imply a fantasy-calendar meaning nobody decided.
