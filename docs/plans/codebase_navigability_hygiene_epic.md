...
3. `tests/unit/domains/` vs. flat domains-subpackage test dirs (`tests/unit/campaigns/`,
   `tests/unit/faction/`, etc.) is a directory-naming inconsistency, not a coverage gap — coverage
   exists for all 19 `domains` subpackages, just inconsistently located, risking an agent
   wrongly concluding some subpackages are untested.
...
- Standardize `tests/unit/domains/` vs. flat placement on one convention; move files accordingly.
...
- A documented decision exists on whether `src/observability/mining/`'s classes are distinct or
  duplicative.
- One consistent convention for domains test-directory placement, applied repo-wide.
