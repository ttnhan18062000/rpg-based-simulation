...
> **Status note (2026-07-02, re-verified 2026-08-22):** the write side ("Core concept" below) shipped as E22 — `decision_trace.jsonl`, tick-index sidecar, REST endpoints (`docs/observability/decision_trace_contract.md`). Remaining live scope is the read side: ring buffer, Chronicle surprise signal, social-memory annotation. (Audit item P1-H, previously listed here, is `RESOLVED` per `docs/plans/audit_fix_plan.md:194,736` — `TCK-20260627-P1H-GOAL-RUNNERUP` — dropped from remaining scope.) Before scheduling any of it, read the SimQ interaction and process-isolation constraints in `docs/plans/archive/observability_process_isolation.md` §4 — the ring buffer is a behavior-feedback feature (durable cognitive state, anchor recalibration required). The shipped writer's hot-path IO violation is remediated by `TCK-20260702-OBSISO-TRACE-ASYNC`, confirmed DONE (closed with the `TCK-20260702-OBSISO-EPIC` on 2026-08-04) — this claim was momentarily inaccurate only during a 2026-08-04 investigation window earlier the same day the epic finished closing, and is genuinely true as of today. Grepped `src/strategy/`/`src/cognition/` for `IntentionSnapshot`/`decision_trace`/`RingBuffer`: zero hits — the read side remains fully unbuilt, exactly as this note already said.
