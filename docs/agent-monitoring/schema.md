...
| `seq` | int | No | 1-based call order *within one continuous invocation* of the writing script. **Corrected guarantee (`TCK-20260915-EVENT-SEQ-INTEGRITY`): `seq` is unique and gap-free only within one continuous execution, NOT globally unique or contiguous per `run_id` across all time** — a real corpus scan found 71 runs with a duplicate `seq` and 46 with gaps, mostly by design: a `run_id` re-invoked (a legitimate re-run/reopen, `TCK-20260915-DUPLICATE-RUN-RECORDS`) restarts its own local numbering at 1 each time, since `seq` is computed from the writing invocation's own in-memory event count, not a persistent global counter; a resumed pause continues numbering via `seq_offset.py`'s own lookup (`TCK-20260728-MONITORING-PAUSE-RESUME-SEQ-COLLISION`) rather than restarting, which can leave a large apparent "gap" below the resumed run's own first `seq` that is not evidence of lost events. Never assume `seq` alone disambiguates two events for the same `run_id` — pair it with `execution_id` when one is present. **Exceptions:** the advisory context-packet shadow-call site (TCK-20260729-SHADOW-PACKET-CALL-SITE, see Provenance note below) emits `seq <= 0` values; the advisory shadow-reviewer call sites (TCK-20260904-SHADOW-REVIEWER-LOGGING, see the Shadow-reviewer-event field family section below) emit `seq <= -100` values. Both are deliberately disjoint from the positive range and from each other — never assume `seq >= 1` when reading `context-packet-wrapper` or `*-shadow` rows. |
...
| `summary` | string | No | One sentence describing what the agent did and the key finding. Empty string = agent did not provide a summary (prompt quality signal). Max 200 chars, enforced at write time by a `truncateSummary()` helper (`TCK-20260915-EVENT-SUMMARY-TRUNCATION`, extended to all sibling workflows by `TCK-20260915-SIBLING-WORKFLOW-SUMMARY-TRUNCATION-MARKERS`) — a deliberate convention (every agent prompt in these files already instructs "one sentence, <=200 chars"), not a schema-level limit; there is no storage-layer cap (summaries up to 1,055 chars are already stored from before this helper existed). A real cut is never silent: a summary over 200 chars is truncated to 196 chars plus a trailing `' […]'` marker (4 chars, total 200), so a reader can always tell a truncated summary from a complete one that happens to end near the boundary. This is now uniform across all 4 workflow files that write events — `implement-ticket.js` (including the ArchVerify/Security-Review shadow-call sites that build event objects directly), `create-tickets.js`, `simq-audit.js`, and `implement-epic.js`'s own single direct-assignment site — each defines its own local copy of the same helper (no shared module exists between these standalone scripts), applied at every summary-producing call site in the file, not only a central `pushEvent()` where one exists. Historical summaries truncated before either fix (silent `.slice(0, 200)`, no marker) are not recoverable and were not rewritten. |
| `status` | string | No | `ok` \| `failed` \| `blocked` \| `skipped` |
| `tool_call_count` | int | Yes | Total number of tool calls made by this agent. Computed deterministically by `record_events.py` at write time from `tools.jsonl` (counts entries matching `run_id` + `seq`) — see below. Since `TCK-20260904-COST-PROXY-EPIC-TICKETS`, computed for `implement-ticket` (all real call sites), `implement-epic` (all 4 real top-level call sites), and `create-tickets` (4 of 7 real call sites — `comprehend`, `structure`, `write-sequence`, `link-epic`; `monitoring-write` and the 2 `pipeline()`-fan-out sites `investigate:`/`write:` per-item are excluded and stay `null`/absent, see below). `null` for runs produced before this field was added, and for the excluded `create-tickets` sites above. **Authoritative source (`TCK-20260915-TOOL-CALL-COUNT-MISMATCH`): `tools.jsonl`'s own real, `(run_id, seq)`-filtered row count is authoritative, not this field.** `tool_call_count` is a write-time SNAPSHOT of that count, not an independently-measured value — it goes stale exactly when `tools.jsonl` itself was incomplete or misattributed at the moment the event was written: before `TCK-20260719-COST-PROXY-WRITE-PATH` (no ground-truth computation existed at all — pre-dates this field's real meaning) and during the pre-`TCK-20260824-SIDECAR-CROSS-SESSION-SCOPE` cross-session-contamination window (see below) both produce a real, permanent, unbackfilled mismatch between the two. When the two disagree, trust a fresh `tools.jsonl` scan over the recorded field. |
...
**`agent` is the pipeline phase's agent literal, not necessarily the real caller (`TCK-20260911-AGENT-TOOLS-FRONTMATTER-WAVE-2`).** The sidecar is written once per phase, before that phase's top-level `agent()` call — every tool call made anywhere during that phase's window gets tagged with the same `agent` literal, including calls made by a nested `Agent()` spawn the top-level agent itself makes. This makes `tools.jsonl`'s `agent` field a poor signal for "which tools does this specific registered agent role actually invoke" when the answer needs to be caller-accurate rather than phase-accurate — confirmed empirically: a caller-level re-count from Claude Code's own subagent transcripts found `architecture-reviewer` at 5 real `Edit` calls against `tools.jsonl`'s 545. `tools/agent-monitoring/agent_tool_usage_baseline.py` (phase-attributed, sourced from this file) remains valid for its original purpose of sizing a `tools:` frontmatter candidate scope from broad usage patterns; `tools/agent-monitoring/subagent_tool_audit.py` (caller-attributed, sourced from `~/.claude/projects/<repo-slug>*/<session>/subagents/agent-<id>.{meta.json,jsonl}` instead of this file) is the correct tool when the question is specifically about one registered agent's own real tool calls — e.g. evaluating whether a wave of `tools:` scoping caused a real denial. See `docs/agent-monitoring/README.md`'s "Agent Tool-Usage Baseline" section and `docs/guidelines/subsystem_ownership_lifecycle.md`'s Tools frontmatter rollout row.
