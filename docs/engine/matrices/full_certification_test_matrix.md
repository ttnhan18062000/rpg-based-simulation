...
Verifies the engine's concurrent work protocol: deterministic packet identity, commit ordering, fallback behavior, and bit-identical equivalence between local and concurrent execution paths.
...
| `test_no_retry_on_failure` | Failed worker result | No local retry for same entity in same tick | Duplicate work attempts |
