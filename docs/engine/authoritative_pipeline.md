...
> [!IMPORTANT]
> **The Singular Bottleneck Law**: All state transitions must pass through this pipeline. No system, worker, or external process may mutate the `AuthoritativeState` directly. All changes must be represented as a `StateUpdate` and refined through these 32 phases.
...
| 9 | `adventure_decision` | Enhanced RPG: routes adventure/quest decision logic (`ENABLE_ADVENTURE_ROUTING`). |
| 10 | `action_routing` | Resolves combat, skills, and tactical ability interactions (`TOWN-149`). |
| 11 | `position_swaps` | Resolves adjacent position exchange contracts and mutual passing (`COMB-028`). |
| 12 | `movement_routing` | Calculates movement steps, terrain costs, and obstacle avoidance (`COMB-046`). |
| 13 | `combat_engagement` | Enhanced RPG: resolves full combat engagement sequences (`ENABLE_COMBAT_ENGAGEMENT`). |
| 14 | `interaction_routing` | Routes interaction intents to domain handlers (`TOWN-165`). |
| 15 | `interaction_enforcement` | Enforces harvesting, chest opening, and node usage rules. |
| 16 | `building_sabotage` | Resolves damage to buildings and town structures (`LEG-RPG-006`). |
| 17 | `town_resolution` | Updates regional influence, taxes, and town-level state (`TOWN-147`). |
| 18 | `world_dynamics` | Resolves world-wide generation, respawns, and weather effects (`INFRA-001`). |
| 19 | `world_emergence` | Enhanced RPG: emergent world events and narrative triggers (`ENABLE_WORLD_EMERGENCE`). |
...
| 23 | `evolution` | Applies entity evolution and stat boosts. |
| 24 | `progression_conversion` | Enhanced RPG: converts progression points to levels/skills. |
...
| 27 | `occupancy_resolution` | Resolves spatial occupancy conflicts between entities. |
| 28 | `lifecycle` | Commits death and lifecycle status changes. |
| 29 | `groups` | Updates group membership and party composition. |
| 30 | `active_contracts` | Enforces active social and economic contract obligations. |
| 31 | `expired_offers` | Clears stale trade and social offers past their TTL. |
| 32 | `capacity_enforcement` | Final inventory and carrying-capacity enforcement pass. |
