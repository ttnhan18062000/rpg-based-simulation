...
# Signal Truth Contract

## 1. Purpose
This document defines the authoritative operational law for the engine runtime signals and governor. It closes the gap between architectural presence and operational truth, ensuring that monitoring, degradation, and recovery are based on real, bounded accounting rather than estimates or placeholders.
...
- **Out-of-Scope**: Full lifecycle hardening (see `concurrent_integrity_contract.md`), Bounded concurrency trust (see `bounded_concurrency_contract.md`).
...
## 9. Verification
The single-process path must operate under these truthful signals without oscillation or semantic drift.
