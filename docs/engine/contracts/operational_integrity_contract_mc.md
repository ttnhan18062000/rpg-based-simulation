...
## Purpose
This document defines the final operational lifecycle laws for the RPG-based simulation engine (V2). It ensures that startup, persistence, runtime status, and shutdown behave as trustworthy contract paths, maintaining the non-authoritative boundary for all observational work.

## Scope of Milestone C
- Hardening of the replay lifecycle (staging, rotation, manifest, pressure).
- Completion of startup profile and operational flag validation.
- Implementation of deterministic shutdown sequencing with real timeouts.
- Finalization of the runtime status truth-audit.
...
4. **Warning-Only Feasibility**: Hardware class vs. tick budget mismatches MUST be treated as warnings, not rejections (Reserved for certification in M9).
...
At the end of Milestone C, the engine operational lifecycle is considered "Finished Law." Any future failure in startup, persistence, or shutdown will be treated as a contract violation (bug) rather than a known infrastructure gap.
