...
> `src/core/buildings.py` (does not exist — real file is `src/town/buildings.py`) and a `Building`
> dataclass shape that doesn't match the real `BuildingRegistry` (static templates, uppercase type
> constants) + `BuildingState` (`src/core/state.py`) split. Real, current, authoritative
> replacement: `docs/simulation/town_contract.md` (`status: authoritative`,
> `last_verified: 2026-06-12`, sourced from the real `src/town/` directory). Kept for historical
> reference only.
