...
> `src/core/world_state.py`, `src/core/buildings.py`, `src/core/monuments.py`,
> `src/actions/raid.py`, `src/actions/repair.py` — none exist. Only `src/engine/world_dynamics.py`
> is real. Real, current coverage: `docs/mechanics/05_world_evolution.md`. No single current
> `docs/` reference doc replaces the resilience/building-durability portion — a disclosed gap,
> out of scope. Kept for historical reference only.
