...
> [!WARNING]
> **ARCHIVED 2026-08-10 — `TCK-20260809-STALE-DOCS-AI-BRAIN-ARCHITECTURE-AUDIT`.** 8 of 11 cited
> `src/` file paths do not exist (`src/actions/combat.py`, `src/actions/damage.py`,
> `src/actions/base.py`, `src/ai/states.py`, `src/core/grid.py`, `src/engine/world_loop.py`,
> `src/core/models.py`, `src/ai/perception.py`) — `src/actions/` and `src/ai/states.py` do not
> exist anywhere in the tree. The doc's own "95% certified" self-claim (below) and its link to
> `docs/compliance/gap_analysis.md` are themselves stale — not re-verified as part of this
> ticket's own doc-only scope. Real, current coverage: `docs/mechanics/02_combat_laws.md`,
> `docs/mechanics/damage_formula_contract.md`. No single current `docs/` reference doc replaces
> the progression/XP portion — a disclosed gap, out of scope. Kept for historical reference only.
