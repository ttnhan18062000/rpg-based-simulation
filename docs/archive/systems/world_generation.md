...
> [!WARNING]
> **ARCHIVED 2026-08-10 — `TCK-20260809-STALE-DOCS-AI-BRAIN-ARCHITECTURE-AUDIT`.** Cites
> `src/core/grid.py`, `src/core/resource_nodes.py`, `src/core/regions.py`,
> `src/systems/terrain_detail.py`, `src/core/faction.py` — none exist (only `src/core/enums.py`
> and `src/api/engine_manager.py` are real). Real, current, authoritative replacement:
> `docs/world/generator_contract.md` (`status: authoritative`, `last_verified: 2026-08-08`,
> sourced from the real `src/worldgeneration/` directory). Also see
> `docs/mechanics/06_worldbuilding_foundation.md` and `docs/mechanics/regional_sovereignty.md` for
> the declarative content-catalog terrain system. Kept for historical reference only.
...
## 2. World Dynamics & Aging

While the map starts as a static grid, it evolves through two primary systems:
...
| `name` | str | Display name (unique per terrain) |
| `terrain` | Material | Primary biome terrain |
| `owner_faction` | Faction | Current strategic owner (None if contested or Hero-held) |
| `center` | Vector2 | Voronoi seed position (Home of the **Stronghold**) |
| `radius` | int | Effective radius for influence calculation |
| `difficulty` | int | 1–4 (proportional to distance from Town) |
| `locations` | list[Location] | Sub-locations (camps, ruins, etc.) |
