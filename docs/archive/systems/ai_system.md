...
> [!WARNING]
> **ARCHIVED 2026-08-10 — `TCK-20260809-STALE-DOCS-AI-BRAIN-ARCHITECTURE-AUDIT`.** Describes a
> "PACKETIZATION" kernel phase and `MindAspect`/`ActionResults` classes that do not exist in the
> current codebase or in `docs/engine/kernel.md`'s real phase table (INIT/SCHEDULING/COLLECTION/
> RESOLUTION/CLEANUP/ADVANCEMENT/PERSISTENCE). `WorkerPacket` (`src/core/worker_protocol.py`) is
> real, but the overall phase-cycle framing is not. Real, current documentation:
> `docs/engine/kernel.md` (phases), `docs/engine/contracts/tactical_contract.md` (tactical AI
> decisions), `docs/systems/strategic_cognition.md` (strategic layer, still current). Kept for
