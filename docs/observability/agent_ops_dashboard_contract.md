...
`gate_failure_breakdown` (from a `runs.jsonl` row with neither `final_status`
nor `status` set) is sanitized to the string `"unknown"` at this API boundary
...
`self._events_all`/`self._tools_all` — the raw, ungrouped `runs.jsonl`/
`events.jsonl`/`tools.jsonl` lists, previously local-only to `_rebuild`
(`tools_all` specifically as of TCK-20260818-STANDARD-KGMCP-CACHE-ATTRIBUTION-
AND-SKILL-USAGE-DASHBOARD, which needed it for `skill_usage`) — since
