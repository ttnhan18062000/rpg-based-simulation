...
1. **Log hook near-misses, not just hook fires.** When the grep-pattern hook doesn't match a command that arguably should have triggered it, that's exactly the signal needed to know the advisory approach is or isn't working. *(Closed by TCK-20260708-AGENT-GATE-ENFORCEMENT-HARDENING — resolved via hard-blocking the two named cases directly rather than near-miss logging; see `docs/plans/archive/agent_infrastructure/idea_agent_gate_determinism.md`'s "Enforcement: nudge vs. block" section — archived, shipped.)*
