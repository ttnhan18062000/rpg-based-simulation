...
**Monitoring failure is designed to fail silently.** *(enforcement)* The hard rule "monitoring write failure must never fail the workflow" is a sound resilience call, but it means the audit trail's own completeness is unguaranteed — a hook that fails leaves no trace that it failed. *(Partially closed by TCK-20260708-AGENT-GATE-ENFORCEMENT-HARDENING: a missing monitoring write at Finalize now produces a `failed`-status event and a persistent `WARNING` in the returned `message` — a trace now exists — while remaining intentionally non-blocking, per CLAUDE.md's Hard Rule. The underlying resilience trade-off described here is otherwise unchanged.)*
...
1. **Log hook near-misses, not just hook fires.** When the grep-pattern hook doesn't match a command that arguably should have triggered it, that's exactly the signal needed to know the advisory approach is or isn't working. *(Correction, 2026-07-20 orchestration audit: this recommendation is NOT closed. TCK-20260708-AGENT-GATE-ENFORCEMENT-HARDENING's own Out of Scope section explicitly states "The context-scan grep/find advisory hook ... do not escalate it" — that ticket hard-blocked two different, unrelated cases instead (a missing-monitoring-write trace at Finalize, made loud-but-non-blocking; and a parity-ledger cross-reference miss, made hard-blocking as `PARITY_INCOMPLETE`). Confirmed still live in this session: the grep/find and Agent-investigation `PreToolUse` hooks in `.claude/settings.json` still only emit `additionalContext`, with no deny/block mechanism. This recommendation remains open.)*
2. **Add a cost proxy to `tools.jsonl`.** Even a rough per-call estimate (agent count × phase) closes the one gap the project's own docs already flag. *(Closed by TCK-20260708-AGENT-COST-OBSERVABILITY — `cost_proxy_score` now computed in `writeMonitoring` and surfaced as a spend-by-phase/spend-by-agent breakdown in `make agent-monitoring-retro`; see `docs/plans/archive/agent_infrastructure/idea_agent_cost_observability.md` — archived, shipped.)*
...
4. **Prune or scope the skill catalog.** Confirm which of the 14 skills have ever fired against this codebase; retire the rest so the catalog reflects what's actually maintained. *(Partially closed by TCK-20260705-SIX-SKILLS-INVESTIGATION: investigated 6 named zero-invocation skills, all scored correctly redundant, none retired. Repo now has 16 skills, not 14. Two skills flagged by this audit, `frontend-design` and `prompt-builder`, were outside that ticket's scope and have never been re-evaluated — confirmed still zero-invocation as of the 2026-07-20 orchestration audit. This recommendation remains partially open.)*
