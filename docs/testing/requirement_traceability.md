...
| **Progression Correctness** | XP-to-level formula matches Mechanics Bible §01. Attribute growth on level-up is correct. Evolution point conversion is bounded. Breakthrough events trigger at the right thresholds. | `tests/unit/progression/test_leveling.py` `tests/unit/progression/test_attribute_growth.py` `tests/unit/progression/test_evolution.py` `tests/unit/progression/test_breakthroughs.py` `tests/integration/domains/progression/test_phase6_progression_conversion_phase.py` | `v2_contract`, `legacy_characterization` | P1 |
