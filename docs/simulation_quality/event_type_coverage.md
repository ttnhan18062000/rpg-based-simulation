...
**Last updated:** 2026-09-06 (`TCK-20260906-ENTITY-EVOLVED-EVENT-GAP` — added `entity_evolved`, a
real gap independent of any of M9's 32 tracked ideas: `EvolutionSystem` (`src/engine/evolution.py`,
species evolution at level thresholds 10/25/50, e.g. GOBLIN->GOBLIN_WARRIOR) writes `kind_set` on
every progression-touched entity but never emitted an observability event for it — confirmed via
grep, zero hits for `entity_evolved` anywhere in `src/` prior to this ticket. Wired into both the
live `ProgressionShaper` push path (`event_shapers.py`) and the flag-gated `event_extractor.py`
rollback path, diffing `kind_set`/`entity.kind` against the prior tick's `kind` (not a bare
non-None check — `kind_set` is always written, unchanged when no evolution occurred this tick).
Registered with `ProgressionScorer` as a new `species_evolution` signal (+10,
`config/simulation_quality/scoring_weights.yaml`) — PROGRESSION's own Question already names
"evolving their playstyle" as part of what this pillar should track. `scored` count 85→86. Not yet
calibrated against a real corpus run — no shipped calibration profile's own goblin/wolf/hero
entities are confirmed to reach level 10 within existing tick budgets; flagged as a real, disclosed
gap for a future calibration ticket, not fabricated here.)

**Previously updated:** 2026-09-06 (`TCK-20260906-SIMQ-CALIBRATION-AND-COMPLETENESS` — attempted to
...
|---|---|---|
| scored | 86 | +1 `entity_evolved` (TCK-20260906-ENTITY-EVOLVED-EVENT-GAP, a real gap independent of M9's own 32 tracked ideas). +1 `route_new_query` (TCK-20260906-SIMQ-PILLAR-MAPPING-AND-RULES, a genuine gap missed by every prior audit of this doc). +1 `world_hard_law_violation` (TCK-20260805-SIMQ-HARDLAW-BRIDGE-COVERAGE-GAP) |
| translation_gap | 0 | — |
...
| `entity_evolved` | event_shapers (ProgressionShaper) | ProgressionScorer | 0 | `EvolutionSystem` (`src/engine/evolution.py`) transforms `kind` at level thresholds 10/25/50 — new, `TCK-20260906-ENTITY-EVOLVED-EVENT-GAP`. `event_extractor.py`'s own block is the flag-gated (`_push_shapers_phase2_active`) rollback path, not the live source. Not yet observed in any real calibration run. |
