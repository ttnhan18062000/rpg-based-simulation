...
|---|---|---|
| 13 of 14 `include_router()` calls (`history`, `search`, `behavior`, `decisions`, `scenarios`, `campaigns`, `chronicle`, `economy`, `manifest`, `map_routes`, `static_routes`, `stats_routes`, `quality_routes`) | `require_api_key` | Ordinary REST sub-routers |
