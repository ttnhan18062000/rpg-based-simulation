...
- **Agent Monitoring** — stat tiles (total runs, done, gate fails, average
  ...
  only when non-empty), and a **Skill usage** subsection (a stat tile for
  total invocations plus a per-skill magnitude bar chart). A **KGMCP cache
  efficiency** subsection (color-coded verdict callout, hit/write/reuse-rate
  stat tiles, per-ticket/per-agent tables) also lived here from
  TCK-20260818-STANDARD-KGMCP-CACHE-ATTRIBUTION-AND-SKILL-USAGE-DASHBOARD
  through TCK-20260910-RETRIEVAL-CACHE-ACCESS-LOG-CHAIN-REMOVAL, which removed
  it entirely once the Knowledge Gateway MCP deprecation
  (`TCK-20260907-KGMCP-DEPRECATION-EPIC`/`TCK-20260908-KGMCP-DELETE-ARCHIVED-
  GATEWAY`) had left it permanently reporting zero.
