...
That compliance rate only confirms the hand-orchestration-path fix directly:
`compute_tool_safety_metrics()` sees only the orchestrating run's own
`tools.jsonl` rows, so when Investigate is properly delegated to
`Agent(subagent_type: "investigator")`, that sub-agent's own
search-before-grep sequence never reaches this metric at all — a
"non-compliant" delegated pair may just be an orchestrator-level incidental
grep, not evidence the agent-dispatched-path fix (`investigator.md`) has
regressed (`TCK-20260824-RETRO-METRIC-CAVEATS`).
