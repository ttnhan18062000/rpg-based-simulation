...
1. **The D0–D18 domain map is broader than the project's own discipline allows.** It is a *target vocabulary* presented at a size that invites Principle 7's failure ("precise, invisible, load-bearing for nothing"). It needs an explicit admission budget, not just an admission test.
   *Outcome 2026-09-20:* the owner chose to build the clean map in full, ahead of implementation, so features can be planned against a long roadmap. The direction doc answers this concern with a reading rule instead of a trim — presence on the map is not a commitment to build, and every entry still has to pass the admission test and Principle 7.
...
| A2 Open Instability | **Agree with an important modification** | Distinguish **stabilisers** (may fail: trauma decay, war exhaustion, migration) from **invariants** (must never fail: atomic conservation, determinism, the `HardLawMonitor` per-tick checks). "The simulation must permit systemic resistance to fail" must not be read as licence to weaken invariants. **Both points were settled 2026-09-20, sharpened 2026-09-21**: the stabiliser/invariant boundary is now in the direction doc, and scoring was redefined to detect logic failure (hard and soft) rather than misfortune — with collapse itself carrying no polarity in either direction; only the causal validity of the process that produced it does. |
...
| Hybrid progression | **Agree, blocked** | Not visible until the combat gate is addressed: the decision-driven attack path fires 0–2 times per 1000–2000 ticks. The wrong-hostility half was fixed on 2026-09-20 (#222), which *reduced* real combat volume by 84.8%/95.9% in the sampled worlds — so the gate is sharper now. **Resolved 2026-09-20:** the "should the strategic layer point entities at combat at all" question is answered — hybrid: keep ambient/incidental combat, add a small number of genuine reasons to fight (predation, named threats, territory defence). See the direction doc's decisions log. |
...
| D14 Places & settlements | **Necessary — and the weakest layer; recommend top priority.** *Confirmed 2026-09-20: the owner made this the first structural gap.* | Place is immutable at runtime; camps only compiled; no tiers, abandonment, colonisation, place history or naming. Breaks the vision's own headline example (a goblin camp becoming a spider ruin). |
...
| D16 Magic & supernatural | **Important long-term; essentially greenfield** | No mana, no spells. **Resolved 2026-09-20** (superseding this row's own original framing): the owner decided magic is its own domain, required to wire outward into combat/interaction/body/ecology/places/belief/information. The rule-injection alternative (§Q9) is kept as reasoning, not as an active recommendation — see §Q9's own annotation. |
...
6. **SimQ grade anchors versus Open Instability — resolved 2026-09-20, sharpened 2026-09-21.** Scoring's job is logic failure (hard and soft), not misfortune; collapse carries no polarity of its own — only the causal validity of the process behind it does. The open engineering question is narrower: today's pillars and grade anchors reward activity and health, so they will need revisiting to match that definition, and mechanic changes already cause anchor staleness independently.
...
| Magic as a spell catalogue | ~~Magic as rule-injection (§Q9)~~ — **superseded 2026-09-20**: magic is its own domain by owner decision. The surviving constraint from this row: whatever shape magic takes, it must reuse, feed or affect existing domain mechanisms rather than becoming an isolated parallel simulation, or it fails Principle 7/8 the same way a pure spell catalogue would. |
...
| 2 | ~~A2 Open Instability vs SimQ grade anchors and health scoring~~ | **Resolved 2026-09-20, sharpened 2026-09-21** | Scoring detects **logic failure only** — hard (impossible behaviour: duplicated movement, invalid/erroring actions, conservation or determinism breaches) and soft (reasoning that doesn't follow its own rules). Destroyed countries, broken economies, ruined lands and collapsed populations carry **no polarity of their own, in either direction** — only whether the causal process that produced them was itself valid does. Scoring must never push the world toward *or* away from any particular outcome |
...
> **Outcome, 2026-09-20: all 13 resolved.** 12 of the 13 were adopted exactly as proposed below, in two passes — the vocabulary changes first, then the domain-map and measurement changes once the owner decided to build the clean taxonomy ahead of implementation. **Item 9 was adopted with a real modification**, not literally as written: of the five proposed domain additions, only two (movement/navigation, law/crime/enforcement) actually became domains — content authoring & world assembly, observation & legibility, and evaluation/SimQ instead joined item 8's cross-cutting reclassification, per the owner's own preferred structure (see the direction doc's §15 correction note). Everything else, including the standalone-magic decision, is a separate change tracked in its own annotation (§Q9, §F/D16), not part of this 13-item list. They are folded into [`core_rpg_design_direction.md`](core_rpg_design_direction.md) §7/§9/§10/§11, and listed in its §15. The list below is kept as the original reasoning.
...
3. **Promote Place to a mutable first-class entity.** *Confirmed 2026-09-20: the owner made this the first structural gap to close.* This is the highest-leverage single structural change in the repository: it is the weakest layer (§F/D14), it blocks the vision's own headline example, and it is additive rather than a rewrite. Without it, no scenario involving ruins, founding, abandonment, colonisation or settlement growth is reachable.
...
9. **Magic as rule-injection, not a subsystem.** *Superseded 2026-09-20 — the owner decided magic is its own domain, wired outward, rather than rule-injection into existing domains (see the direction doc §4 and decisions log). The reasoning below is kept because it stays useful as a constraint: whatever magic's own domain does, it should reuse, feed, or affect existing domain mechanisms rather than forming an isolated parallel simulation — the actual risk this idea was trying to head off, not the specific "no standalone domain" structure.* Original idea: rather than a spell engine, let magical content instantiate *existing* rules with unusual parameters — fire magic uses the fire/hazard rules; necromancy is a spawn rule keyed to corpses and mass-death regions; corruption is a hazard kind plus adaptation.
...
*(Cross-reference fixed 2026-09-20: this section originally cited "the direction doc §29" — that section number belongs to `rpg-direction.md`, the original owner input document, not to `core_rpg_design_direction.md`, which has no §29. The alignment-pass request is real, it's just sourced from a different document than this one originally said.)*

`rpg-direction.md`'s own closing section asks for a **repository-to-target alignment pass** (its sections A–I) as the natural next artifact, not a milestone plan. Much of it is drafted here (§F, §H, §I, §J, §K, §O). What is missing and worth doing deliberately:
...
- [`simulation-rule-world-law-design-preparation.md`](simulation-rule-world-law-design-preparation.md) — the methodology companion for the next phase (2026-09-21)
