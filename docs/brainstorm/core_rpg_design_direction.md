...
| | |
|---|---|
| **Settled** | The 8 principles (§2). The five agreed decisions (§6). The definition of realism and the legitimacy of RPG abstractions (§4). Hybrid progression (§5). The structural, rule and entity vocabularies, and the target domain map (§7). Power plurality without a conversion framework (§8). Depth measurement and the counterforce requirement (§9, §11). The runtime status vocabulary's ten-term list and its two defined values, `STARVED`/`REACH-LIMITED` (§10 — its other eight values are reserved, not yet defined; see §10's own status note). All decisions in §16 |
| **Still open** | The *soft failure* definition for quality scoring (§6/A2) — deliberately left to sharpen later. How today's scoring pillars are reconciled with outcome-neutral scoring (§6/A2). Defining or pruning §10's eight undefined runtime-status values — deferred to the Semantic Control Plane's M4 milestone. Everything about sequencing: what gets built, in what order, and when. No milestone, wave or ticket has been agreed, and none should be inferred from this document |
...
See `docs/plans/status_axis_model.md` for how this vocabulary relates to the Mechanism Registry's `state`/`verified.verdict` axes and the semantic control plane's Rule realization axis.

**Status note (2026-09-24).** Only `STARVED` and `REACH-LIMITED` above carry a real prose definition; the other eight values (`MISSING`, `DESIGNED`, `EXPERIMENTAL`, `OFF`, `DORMANT`, `LIVE`, `DEPRECATED`, `REPLACED`) are aspirational, undefined beyond their name and list position, unenforced by any validator, and carried by zero real mechanism rows today (confirmed by `docs/plans/status_axis_model.md`'s own cross-axis audit). Treat this list as a *reserved vocabulary*, not a settled, ready-to-use taxonomy — defining or pruning the remaining eight is deliberately deferred until the Semantic Control Plane's M4 milestone (a cross-domain view) actually needs specific terms, so the definitions are evidence-driven rather than speculative.
...
| 2026-09-21 | **World Rule Catalog started.** Foundational Batch 01 (Identity, State Ownership, Causality) drafted, pending external review | `docs/world_rules/README.md` |
| 2026-09-21 | **History / Provenance added as a fourth foundational/cross-cutting Rule family, scope only.** Names what §7's "History, significance & world reaction" cross-cutting layer already implied for provenance/persistence/compression/significance-fading semantics; no rules drafted, no change to build order or the §7 domain map/cross-cutting table itself | `docs/world_rules/foundations/history-provenance.md` |
