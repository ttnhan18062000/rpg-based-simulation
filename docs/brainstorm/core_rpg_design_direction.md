...
See `docs/plans/status_axis_model.md` for how this vocabulary relates to the Mechanism Registry's `state`/`verified.verdict` axes and the semantic control plane's Rule realization axis. **Status note (2026-09-24).** Only `STARVED` and `REACH-LIMITED` above carry a real prose definition; the other eight values (`MISSING`, `DESIGNED`, `EXPERIMENTAL`, `OFF`, `DORMANT`, `LIVE`, `DEPRECATED`, `REPLACED`) are aspirational, undefined beyond their name and list position, unenforced by any validator, and carried by zero real mechanism rows today (confirmed by `docs/plans/status_axis_model.md`'s own cross-axis audit). Treat this list as a *reserved vocabulary*, not a settled, ready-to-use taxonomy — defining or pruning the remaining eight is deliberately deferred until the Semantic Control Plane's M4 milestone (a cross-domain view) actually needs specific terms, so the definitions are evidence-driven rather than speculative.
