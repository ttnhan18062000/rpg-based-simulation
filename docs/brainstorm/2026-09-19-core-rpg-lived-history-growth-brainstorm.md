...
- **W1.1's combat-derived experience kinds** (`SURVIVED_NEAR_DEATH`, `KILLED_NOTABLE`, `SURVIVED_HUNT`) needed hostility unification first, so that entities don't accumulate lived history — and later epithets and feuds — from fights the authored world says shouldn't happen. The main path is now fixed; the remaining sweep (7+ instances) should be triaged before combat-derived history is built on it.
- **The combat gate is now sharper, not looser.** With the false fights removed, the corpus has far less real combat than the earlier 181–2177 opportunity-attack figures implied. Whether the strategic layer should point entities at combat more often (the design question #219 left open) is now the decisive question for every combat-driven scenario link.
...
|---|---|
| `progression-starvation-chain` (its P0 attack-path investigation is closed, see §3.1); hostility unification landed 2026-09-20 (#222); `TCK-20260919-RAW-LEGACY-FACTION-ENUM-HOSTILITY-SWEEP` (P1, open) | L0. Without the *right* combat, A3/A4/A11/E4 never fire, or fire from the wrong fights, however much else is built. The design answer to #219 ("should the strategic layer point entities at combat more?") is where W3.1/W3.5 can enter. |
