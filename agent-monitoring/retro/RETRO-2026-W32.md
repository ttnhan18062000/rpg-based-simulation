...
- **Combat work this week is clean**: 17 `combat`-tagged runs, 100% DONE, 0 gate failures — spans
  the full chain from `TCK-20260809-COMBAT-ATTACK-LEGALITY-ALWAYS-FALSE-INVESTIGATION` through
  `TCK-20260809-COMBAT-PACING-READINESS-MOVEMENT-DECOUPLE` and the 3 test-isolation hotfixes found
  along the way. No rework, no gate blocks.
- **This week's slow/outlier runs are dominated by real, deep, hand-orchestrated ("claude" agent)
  investigation work**, not process inefficiency: `TCK-20260809-COMBAT-ATTACK-LEGALITY-ALWAYS-
  FALSE-INVESTIGATION` (145 min, 4.8x tier median), `-ACTIONSTYLE-WIRING` (115 min, 3.8x),
  `-PERSONALITY-RACE-CORRELATION` (110 min, 3.7x), `-OUTCOME-FLEE-VS-FIGHT-PERSONALITY` (105 min,
  3.5x) all ran long because they involved live `Kernel.tick_once()` probes against real compiled
  worlds to verify root causes and fixes with real data rather than delegating to a sub-agent —
  the same discipline the earlier `TCK-20260808-LEVEL-UP-GATED-PROGRESSION-CASCADE-DEAD` outlier
  (500 min, 16.7x) also reflects. Expected, not a red flag.
- **Search-Before-Grep compliance is 46.2% (18/39) this week** — real, below-50% gap against
  CLAUDE.md's hard rule. Worth a closer look at which Investigate-phase runs skip `search_docs`/
  `graphify query` first; not root-caused here.
- **A stray non-canonical phase name, `Investigate-Deepen`, was logged once this week**
  (`TCK-20260808-LEVEL-UP-GATED-PROGRESSION-CASCADE-DEAD`'s own closure) — a warn-only drift, not
  registered in `tools/agent-monitoring/vocabulary.py`'s `WORKFLOW_PHASES`. Harmless (matches the
  same warn-only class already accepted for the `"claude"` agent literal before its own
  registration fix), but if this phase name gets reused going forward it should be registered
  properly rather than left ad hoc.
- **2 unsafe `parity_index.py build` invocations against the real repo path** flagged by the Tool
  Safety Audit this week — not from this session's own work (parity updates here were direct YAML
  edits, the established workflow pattern, not `parity_index.py build` calls); worth checking
  which run triggered these.
