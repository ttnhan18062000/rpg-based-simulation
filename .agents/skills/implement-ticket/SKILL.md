...
| `await writeSidecar(seq, phase, agent)` — appears immediately before every `agent()` dispatch | **Mandatory, not optional.** Run its exact `python3 -c` body yourself via Bash before every corresponding `Agent(...)` call: write `{"run_id": tid, "seq": <events.length + 1 + seqOffset>, "phase": "<Name>", "agent": "<agentType>", "execution_id": ..., "provider": "claude"}` to `.claude/current_run`. Forgetting this is the single most common hand-orchestration gap (see `TCK-20260807-CURRENT-RUN-SIDECAR-HAND-ORCHESTRATION-GAP`) — it silently zeroes `tool_call_count`/`cost_proxy_score` for the phase and makes the Tool Safety Audit's `search_before_grep` check blind to it. A `PreToolUse` hook now nudges when `tickets/inprogress/` is active and the sidecar's `run_id` is empty, but the write itself is still the orchestrating agent's own responsibility, not automated. |
