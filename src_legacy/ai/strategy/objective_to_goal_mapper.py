...
class ObjectiveToGoalMapper:
    ...
    def get_tactical_biases(cls, ctx: AIContext, objective_override: ObjectiveRecord | None = None) -> dict[GoalType, float]:
        ...
        confidence = 1.0
        # Use safe access as ConcernRecord might not have all attributes [TCK-20260415-HARDENING]
        leads = getattr(obj, "leads", [])
        if leads:
            # Take strongest lead as the driver
            confidence = max((l.certainty for l in leads), default=1.0)
