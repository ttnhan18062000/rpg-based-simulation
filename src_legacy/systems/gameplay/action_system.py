...
from src_legacy.core.models.enums import AIState, ActionType, Element, GoalType, EmotionType, HeroClass, StrategicStatus
...
from src_legacy.core.gameplay.classes import SKILL_DEFS, SkillTarget, SkillType
from src_legacy.core.logic.event_interpreter import EventInterpreterService
...
from src_legacy.core.logic.memory_salience import MemorySalienceService
from src_legacy.core.gameplay.effects import StatusEffect, EffectType
from src_legacy.core.models.strategy import StrategicState
from src_legacy.actions.eat import EatAction
from src_legacy.actions.sleep import SleepAction
from src_legacy.core.gameplay.attributes import train_attributes
...
if TYPE_CHECKING:
    ...
    from src_legacy.core.models.config import SimulationConfig
    from src_legacy.core.world.factions import FactionRegistry
    from src_legacy.systems.infrastructure.base import SystemContext
    from src_legacy.utils.event_emitter import EventEmitter
...
class ActionSystem(System):
    ...
    def _apply_updates(cls, world: WorldState, actor: Entity, updates: list[IntentUpdate], proposal: ActionProposal, emit: EventEmitter | None = None) -> None:
        ...
        for up in updates:
            ...
            if isinstance(up, MindUpdate):
                ...
                if milestones:
                    from src_legacy.core.aspects.mind import SocialNarrative, InterpretedEvent
                ...
                for existing in strat.projects:
                    if existing.project_id == p.project_id:
                        if existing.status != p.status:
                            from src_legacy.utils.metrics import SIM_STRATEGIC_PROJECT_STATUS
                ...
                for existing in strat.contracts:
                    if existing.contract_id == ct.contract_id:
                        if existing.status != ct.status and ct.status == StrategicStatus.ABANDONED:
                            from src_legacy.utils.metrics import SIM_STRATEGIC_CONTRACT_BREACHES
    ...
    def _get_use_skill_updates(cls, world: WorldState, config: SimulationConfig, rng: DeterministicRNG, faction_reg: FactionRegistry | None, entity: Entity, skill_id: str, target_id: int | None = None, proposal: ActionProposal | None = None, emit: Callable | None = None, ignore_skill_check: bool = False) -> list[IntentUpdate]:
        ...
        if sdef.radius > 0:
            ...
            from src_legacy.core.models.types import LocationTarget
        ...
        from src_legacy.actions.combat import DamageResolutionService, CombatAftermathService
        for t in targets:
    ...
    def _get_looting_updates(world: WorldState, entity: Entity, pos: Any) -> list[IntentUpdate]:
        ...
        for nid, node in list(world.corpse_nodes.items()):
            if node.pos == pos:
                ...
                from src_legacy.core.aspects.mind import InterpretedEvent, LootNarrative
    ...
    def _process_social_interpretation(cls, world: WorldState, actor: Entity, updates: list[IntentUpdate], proposal: ActionProposal, rng: DeterministicRNG | None = None) -> None:
        from src_legacy.actions.base import CombatTraceUpdate, SpatialUpdate
        for up in updates:
