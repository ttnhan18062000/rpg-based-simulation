...
from src_legacy.core.state import InventoryComponent, ItemStack, EntityState, EquipSlot
...
class InventoryService:
    ...
    def can_add_item(inventory: InventoryComponent, item_id: str, quantity: int) -> bool:
        ...
        if existing_stack:
            # For simplicity, assume infinite stacking for now or check stack_size if defined
            pass
        else:
    ...
    def can_add_items(inventory: InventoryComponent, item_stacks: List[ItemStack]) -> bool:
        ...
        # Create a temporary list of items to simulate additions
