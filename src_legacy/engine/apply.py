...
from src_legacy.core.state import AuthoritativeState, EntityState, InventoryComponent
...
class ApplyPath:
    ...
    def apply_generation(
        ...
        from src_legacy.world.regional_sovereignty import RegionalSovereigntyService
                    ...
                    if intent.splash_radius > 0:
                        ...
                        for other_id, other_ent in prior_state.entities.items():
                            ...
                            if dist <= intent.splash_radius:
                                # LoS Check for Splash: Center to target tile
                                if LegalityServiceV2.has_line_of_sight(impact_pos, other_ent.position, prior_state):
        ...
        for e_id, entity in prior_state.entities.items():
            ...
            if ent_upd:
                ...
                if is_now_dead:
                    from src_legacy.core.state import CorpseState
                    from src_legacy.engine.legality import LegalityServiceV2
    ...
    def _apply_entity_update(entity: EntityState, update: EntityUpdate) -> EntityState:
        ...
        if update.interaction:
            if update.interaction.reset:
                from src_legacy.core.state import InteractionComponent
        ...
        if update.identity:
            ...
            if update.identity.skills_learned:
                 new_skills = set(new_identity.learned_skills)
                 new_skills.update(update.identity.skills_learned)
                 new_identity = replace(new_identity, learned_skills=new_skills)
        ...
        if update.attributes or update.equipment:
            # Recalculate derived stats from base attributes + equipment
            derived = LevelingService.recalculate_combat_stats(new_attributes)
            
            # PH8 Equipment Stats Application
            total_atk_bonus = 0
            total_def_bonus = 0
            total_max_hp_bonus = 0
            
            if new_equipment.slots:
                from src_legacy.core.registry import Registry
                for slot, item_id in new_equipment.slots.items():
                    if item_id:
                        item_data = Registry.get_item(item_id)
                        if item_data:
                            total_atk_bonus += getattr(item_data, "atk_bonus", 0)
                            total_def_bonus += getattr(item_data, "def_bonus", 0)
                            total_max_hp_bonus += getattr(item_data, "hp_bonus", 0)
                            
            new_combat = replace(
                new_combat,
                max_hp=derived["max_hp"] + total_max_hp_bonus,
                atk=derived["atk"] + total_atk_bonus,
                def_stat=derived["def_stat"] + total_def_bonus,
        ...
        if update.social:
            from src_legacy.social.relationships import RelationshipService
            new_social = RelationshipService.process_update(entity.social, update.social)

        if update.biological:
            from src_legacy.core.state import BiologicalComponent
        ...
        if update.lifecycle:
            from src_legacy.core.state import LifecycleComponent
        ...
        if update.combat:
            ...
            from src_legacy.core.state import CombatComponent
        ...
        if update.navigation:
            current_nav = entity.navigation
            new_nav = replace(
                current_nav,
                target=update.navigation.target_set if update.navigation.target_set is not None else current_nav.target,
                path=update.navigation.path_set if update.navigation.path_set is not None else current_nav.path,
                mode=update.navigation.mode_set if update.navigation.mode_set is not None else current_nav.mode,
                moved_recently=update.navigation.moved_recently_set if update.navigation.moved_recently_set is not None else current_nav.moved_recently,
                last_failure_reason=update.navigation.failure_reason if update.navigation.failure_reason is not None else current_nav.last_failure_reason
            )
            entity = replace(entity, navigation=new_nav)

        if update.task:
            from src_legacy.core.state import TaskComponent
            new_task = replace(
                entity.task,
        ...
        if update.quest:
            from src_legacy.core.quests import QuestState, QuestStatus
            from src_legacy.quests.service import QuestService
        ...
        from src_legacy.progression.evolution import EvolutionService
        evo_kind = EvolutionService.check_evolution(final_kind, curr_lvl)
        if evo_kind:
            final_kind = evo_kind
            # Refresh gear for evolved form
            from src_legacy.core.state import EquipmentComponent, EquipSlot
