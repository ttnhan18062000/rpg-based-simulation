...
from src_legacy.core.updates import WorldUpdate, EntityUpdate, CombatUpdate
...
class WorldDynamicsSystem:
    ...
    def resolve_dynamics(state: AuthoritativeState, update: StateUpdate, generator: EntityGenerator) -> StateUpdate:
        ...
        # 3.6 Ambient Spawning (PH9)
        from src_legacy.systems.spawn_system import SpawnSystem
        ambient_update = SpawnSystem.resolve_ambient_spawns(state, update, generator)
        ...
        update = update.replace(
            ...
            entities_add=ambient_update.entities_add + calamity_update.entities_add + raid_update.entities_add
        )

        # 4. Regional Transformations (Type Shifting)
        from src_legacy.world.transformation import TransformationService
