...
class SimulationDomainLogic:
    ...
    def execute_brain(
        ...
        from src_legacy.engine.cognition import SensoryFilter, AppraisalSystem
    ...
    def execute_action(
        ...
        from src_legacy.core.updates import BiologicalUpdate, CombatUpdate, RewardUpdate, LifecycleUpdate
        ...
        if action == "SLEEP":
            ...
            if target and context:
                ...
                # PH5: Authoritative Legality Enforcement
                legal, reason = LegalityServiceV2.verify_attack_legality(entity, target, context)
                if not legal:
                    # Reject action but still consume readiness (failed attempt cost)
                    return {entity.id: EntityUpdate(
                        entity_id=entity.id,
                        readiness_delta=-100.0,
                ...
                # PH7: Betrayal Detection
                betrayal_social = None
                betrayal_strat = None
                if entity.identity.faction == target.identity.faction and entity.id != target.id:
                    from src_legacy.systems.social_legacy import SocialAppraisalSystem
                    betrayal_social, betrayal_strat = SocialAppraisalSystem.process_betrayal(
                        target, entity.id, salience=0.8, current_tick=context.tick
                    )
                ...
                # Merge grudge with betrayal social
                final_social = SocialUpdate(grudge_delta={entity.id: combat_up.damage_taken / target.combat.max_hp})
                if betrayal_social:
                     # Merge SocialUpdate logic (trust_delta, bond_updates, etc)
                     final_social = replace(final_social,
                        trust_delta=betrayal_social.trust_delta,
                        bond_updates=betrayal_social.bond_updates,
                        betrayal_increment=betrayal_social.betrayal_increment
                     )
