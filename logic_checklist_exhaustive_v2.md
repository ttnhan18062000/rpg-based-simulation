...
## Latest V2 coverage update — 2026-04-28

This update marks only atomic checklist items that are fully implemented and backed by visible V2 source/test evidence. Phase-level or partially implemented laws remain unchecked.

Coverage before this update:

- Total checklist items: 1647
- Completed items: 351
- Incomplete / unproven items: 1296
- Coverage: 21.31%
...
- Total checklist items: 1647
- Completed items: 534
- Incomplete / unproven items: 1113
- Coverage: 32.42%
- Newly completed atomic items: 183
- Coverage change: +11.11 percentage points
...
- [x] Movement intentions exist as semantic modes (pursue, retreat, hold, reposition, intercept, guard, regroup). <!-- VERIFIED v2: MovementMode plus tactical/movement tests cover these semantic modes. -->
...
- [x] Loot/harvest can abort because of inventory slot pressure. <!-- VERIFIED v2: ResourceTransactionResolver rejects capacity failure and tests preserve source. -->
- [x] Loot/harvest can abort because of inventory weight pressure. <!-- VERIFIED v2: inventory weight enforcement and resource conservation tests cover this. -->
- [x] Inventory state tracks both slots and weight/carry burden.
- [x] Ground items, node yields, and inventory additions/removals are authoritative side effects. <!-- VERIFIED v2: resource transfer intents are resolved by authoritative pipeline. -->
...
- [x] Party/group cooperation is purpose-driven, not just proximity clustering. <!-- VERIFIED v2: proximity-only group rejected; contract-backed party accepted. -->
...
- [x] RNG domain separation produces different streams for different domains. <!-- VERIFIED v2: DeterministicRNG has per-domain streams and tests cover reproducibility/isolation. -->
- [ ] `next_int()` always respects inclusive bounds.
- [x] Spatial hash insert places entity in correct cell. <!-- VERIFIED v2: SpatialHashV2 contract tests cover insert/query. -->
- [x] Spatial hash radius query includes neighboring cells. <!-- VERIFIED v2: SpatialHashV2 radius query tests cover neighboring cells. -->
- [x] Spatial hash move removes old cell membership and adds new cell membership. <!-- VERIFIED v2: SpatialHashV2 move/update tests cover old/new membership. -->
- [x] Spatial hash remove clears membership. <!-- VERIFIED v2: SpatialHashV2 remove tests cover membership clearing. -->
...
- [ ] Every checklist row has a stable ID.
- [ ] Every checklist row has one explicit status: preserved, enhanced, intentionally divergent, unsupported, or not yet checked.
- [ ] Every checked row has at least one proof path.
- [ ] Every checked row names the implementation path that proves it.
- [ ] Every checked row names the test path that proves it.
- [ ] Every intentionally divergent row has a concrete reason.
- [ ] Every intentionally divergent row has a regression or contract test proving the new law.
- [ ] Every unsupported row has a support-boundary note.
- [ ] Unsupported behavior is not marked as intentionally divergent.
- [ ] Partial behavior is not marked as complete.
- [ ] Behavior implemented in a dead or bypassed code path is not marked as complete.
- [ ] Behavior implemented in one path but contradicted by another active path is not marked as complete.
- [ ] The checklist distinguishes implementation detail from RPG semantic law.
- [ ] The checklist keeps smallest necessary logic items instead of collapsing them into subsystem summaries.
- [ ] CI can fail when a checked row has no proof reference.
- [ ] CI can fail when a divergence has no divergence-register entry.
- [ ] CI can fail when a proof path references a missing test file.
- [ ] CI can fail when a checklist item references a removed implementation path.
- [ ] CI can fail when a test marker is unknown.
- [ ] CI can fail when mandatory oracle files are missing.
- [ ] CI can fail when an oracle lacks schema version.
- [ ] CI can fail when an oracle lacks seed/config metadata.
- [ ] CI can fail when unsupported behavior is silently accepted as success.

## Z2. Resource conservation and inventory pressure law

- [ ] All active resource acquisition paths share one conservation law.
- [x] `InteractionSystem` harvest path and any standalone `HarvestSystem` path cannot diverge on capacity rules. <!-- VERIFIED v2: ResourceTransferIntent/ResourceTransactionResolver and conservation tests cover this atomic law. -->
- [x] `InteractionSystem` loot path and any standalone `LootSystem` path cannot diverge on capacity rules. <!-- VERIFIED v2: ResourceTransferIntent/ResourceTransactionResolver and conservation tests cover this atomic law. -->
- [x] A resource node charge is not decremented until inventory receipt is proven possible. <!-- VERIFIED v2: ResourceTransferIntent/ResourceTransactionResolver and conservation tests cover this atomic law. -->
- [x] A ground item is not removed until inventory receipt is proven possible. <!-- VERIFIED v2: ResourceTransferIntent/ResourceTransactionResolver and conservation tests cover this atomic law. -->
- [x] A corpse loot record is not consumed until inventory receipt is proven possible. <!-- VERIFIED v2: ResourceTransferIntent/ResourceTransactionResolver and conservation tests cover this atomic law. -->
- [x] Inventory slot capacity is checked before source mutation. <!-- VERIFIED v2: ResourceTransferIntent/ResourceTransactionResolver and conservation tests cover this atomic law. -->
- [x] Inventory weight capacity is checked before source mutation. <!-- VERIFIED v2: ResourceTransferIntent/ResourceTransactionResolver and conservation tests cover this atomic law. -->
- [ ] Inventory stackability is checked before rejecting for slot pressure.
- [x] Inventory add failure preserves the source item/node/corpse. <!-- VERIFIED v2: ResourceTransferIntent/ResourceTransactionResolver and conservation tests cover this atomic law. -->
- [x] Inventory add failure resets or preserves the interaction channel according to an explicit rule. <!-- VERIFIED v2: ResourceTransferIntent/ResourceTransactionResolver and conservation tests cover this atomic law. -->
- [x] Harvest completion emits both inventory addition and node depletion atomically. <!-- VERIFIED v2: ResourceTransferIntent/ResourceTransactionResolver and conservation tests cover this atomic law. -->
- [x] Loot completion emits both inventory addition and source removal atomically. <!-- VERIFIED v2: ResourceTransferIntent/ResourceTransactionResolver and conservation tests cover this atomic law. -->
- [x] Partial channel progress does not create items. <!-- VERIFIED v2: ResourceTransferIntent/ResourceTransactionResolver and conservation tests cover this atomic law. -->
- [x] Interrupted channel progress does not create items. <!-- VERIFIED v2: ResourceTransferIntent/ResourceTransactionResolver and conservation tests cover this atomic law. -->
- [x] Interrupted channel progress does not remove source items. <!-- VERIFIED v2: ResourceTransferIntent/ResourceTransactionResolver and conservation tests cover this atomic law. -->
- [x] Failed capacity check does not remove source items. <!-- VERIFIED v2: ResourceTransferIntent/ResourceTransactionResolver and conservation tests cover this atomic law. -->
- [x] Failed capacity check does not decrement node charges. <!-- VERIFIED v2: ResourceTransferIntent/ResourceTransactionResolver and conservation tests cover this atomic law. -->
- [ ] Two actors completing the same loot target in the same tick cannot duplicate the item.
- [ ] Two actors completing the same resource-node charge in the same tick cannot duplicate the yield.
- [ ] Two actors completing the same corpse loot in the same tick cannot duplicate corpse rewards.
- [ ] Conflict resolution decides one authoritative winner for contested loot completion.
- [ ] Conflict resolution decides one authoritative winner per limited resource charge when required.
- [ ] Item quantity is preserved through pickup, stacking, selling, crafting, and dropping.
- [ ] Item weight is preserved through pickup, stacking, selling, crafting, and dropping.
- [ ] Item identity/kind is preserved through pickup, stacking, selling, crafting, and dropping.
- [ ] Harvest yield uses the node definition, not caller-provided arbitrary item data.
- [ ] Loot yield uses the ground/corpse source definition, not caller-provided arbitrary item data.
- [ ] Capacity failure reason is structured and observable.
- [x] Resource source mutation and inventory mutation appear in the same authoritative update/apply transaction. <!-- VERIFIED v2: ResourceTransferIntent/ResourceTransactionResolver and conservation tests cover this atomic law. -->
- [ ] Resource acquisition replay includes both source and inventory state.
- [ ] Resource acquisition replay can prove no duplication across identical seed runs.
- [ ] Resource acquisition replay can prove no item loss under capacity failure.
- [ ] Resource-node cooldown behavior is deterministic.
- [ ] Resource-node recharge behavior is deterministic.
- [ ] Resource-node depletion state survives serialization.
- [ ] Ground item state survives serialization.
- [ ] Corpse loot state survives serialization.
- [x] Loot/harvest tests include slot pressure. <!-- VERIFIED v2: ResourceTransferIntent/ResourceTransactionResolver and conservation tests cover this atomic law. -->
- [x] Loot/harvest tests include weight pressure. <!-- VERIFIED v2: ResourceTransferIntent/ResourceTransactionResolver and conservation tests cover this atomic law. -->
- [ ] Loot/harvest tests include stack merge under near-full inventory.
- [ ] Loot/harvest tests include two actors racing for one item.
- [x] Loot/harvest tests include failure preserving source state. <!-- VERIFIED v2: ResourceTransferIntent/ResourceTransactionResolver and conservation tests cover this atomic law. -->
- [x] Loot/harvest tests include success mutating inventory and source together. <!-- VERIFIED v2: ResourceTransferIntent/ResourceTransactionResolver and conservation tests cover this atomic law. -->

## Z3. Authoritative pipeline and bypass prevention

- [x] Every gameplay update enters the same authoritative refinement/apply pipeline. <!-- VERIFIED v2: authoritative refinement/apply pipeline resolves accepted/rejected resource and action updates. -->
- [ ] No AI/state system mutates authoritative world state directly during proposal generation.
- [x] No standalone gameplay system can remove world objects without apply pipeline validation. <!-- VERIFIED v2: authoritative refinement/apply pipeline resolves resource/movement/action updates before mutation. -->
- [x] No standalone gameplay system can add inventory without apply pipeline validation. <!-- VERIFIED v2: authoritative refinement/apply pipeline resolves resource/movement/action updates before mutation. -->
- [ ] No standalone gameplay system can change combat HP without combat legality validation.
- [ ] No standalone gameplay system can change position without movement legality validation.
- [ ] No standalone gameplay system can change reputation/social state without social consequence validation.
- [ ] No standalone gameplay system can complete quests without quest lifecycle validation.
- [x] Every raw update is either accepted, refined, rejected, or marked unsupported. <!-- VERIFIED v2: authoritative refinement/apply pipeline resolves accepted/rejected resource and action updates. -->
- [x] Rejected updates preserve unrelated update domains. <!-- VERIFIED v2: authoritative refinement/apply pipeline resolves accepted/rejected resource and action updates. -->
- [x] Rejected updates expose structured rejection reason. <!-- VERIFIED v2: authoritative refinement/apply pipeline resolves accepted/rejected resource and action updates. -->
- [x] Rejection reasons are replay-visible or log-visible. <!-- VERIFIED v2: authoritative refinement/apply pipeline resolves accepted/rejected resource and action updates. -->
- [ ] Merge logic for multiple `StateUpdate`s is deterministic.
- [ ] Merge logic has stable precedence for conflicting entity updates.
- [ ] Merge logic has stable precedence for conflicting world updates.
- [ ] Merge logic does not accidentally drop independent updates.
- [ ] Merge logic detects incompatible updates where required.
- [ ] Apply order is documented.
- [ ] Apply order is tested for cross-domain interactions.
- [ ] Apply order cannot depend on dictionary iteration where ordering matters.
- [ ] Apply order cannot depend on thread completion order where gameplay outcome matters.
- [ ] Pipeline tests include direct old-system calls if such calls remain importable.
- [ ] Pipeline tests prove dead/bypassed systems cannot create different gameplay laws.

## Z4. Movement intention, congestion, and blocked-route behavior

- [x] Movement intentions include pursue. <!-- VERIFIED v2: MovementMode, sidestep/yield/regroup logic, and tactical movement regressions cover this. -->
- [x] Movement intentions include retreat. <!-- VERIFIED v2: MovementMode, sidestep/yield/regroup logic, and tactical movement regressions cover this. -->
- [x] Movement intentions include hold. <!-- VERIFIED v2: MovementMode, sidestep/yield/regroup logic, and tactical movement regressions cover this. -->
- [x] Movement intentions include reposition. <!-- VERIFIED v2: MovementMode, sidestep/yield/regroup logic, and tactical movement regressions cover this. -->
- [x] Movement intentions include intercept. <!-- VERIFIED v2: MovementMode, sidestep/yield/regroup logic, and tactical movement regressions cover this. -->
- [x] Movement intentions include guard. <!-- VERIFIED v2: MovementMode, sidestep/yield/regroup logic, and tactical movement regressions cover this. -->
- [x] Movement intentions include regroup or an explicit intentional-divergence note explains why not. <!-- VERIFIED v2: MovementMode, sidestep/yield/regroup logic, and tactical movement regressions cover this. -->
- [x] Movement intention influences target choice. <!-- VERIFIED v2: MovementMode, sidestep/yield/regroup logic, and tactical movement regressions cover this. -->
- [x] Movement intention influences preferred tile choice. <!-- VERIFIED v2: MovementMode, sidestep/yield/regroup logic, and tactical movement regressions cover this. -->
- [x] Movement intention influences blocked-tile fallback. <!-- VERIFIED v2: MovementMode, sidestep/yield/regroup logic, and tactical movement regressions cover this. -->
- [ ] Movement intention influences willingness to wait.
- [x] Movement intention influences willingness to sidestep. <!-- VERIFIED v2: MovementMode, sidestep/yield/regroup logic, and tactical movement regressions cover this. -->
- [ ] Movement intention influences willingness to reroute.
- [ ] Movement intention influences willingness to break pursuit.
- [ ] Blocked movement first checks if target was reached.
- [ ] Blocked movement distinguishes temporary occupancy from invalid terrain.
- [ ] Blocked movement distinguishes ally-blocked from enemy-blocked.
- [x] Blocked movement distinguishes high-priority actor from low-priority actor. <!-- VERIFIED v2: MovementMode, sidestep/yield/regroup logic, and tactical movement regressions cover this. -->
- [ ] Blocked movement can wait when waiting is strategically valid.
- [x] Blocked movement can yield when another actor has higher priority. <!-- VERIFIED v2: MovementMode, sidestep/yield/regroup logic, and tactical movement regressions cover this. -->
- [x] Blocked movement can sidestep when a safe adjacent alternative exists. <!-- VERIFIED v2: MovementMode, sidestep/yield/regroup logic, and tactical movement regressions cover this. -->
- [ ] Blocked movement can reroute when the direct step is blocked.
- [ ] Blocked movement can replan when route cache/path is stale.
- [x] Blocked movement can regroup when party cohesion matters. <!-- VERIFIED v2: MovementMode, sidestep/yield/regroup logic, and tactical movement regressions cover this. -->
- [ ] Blocked movement can abandon route after bounded retry budget.
- [ ] Occupancy conflict has one authoritative winner per tile per tick.
- [x] Occupancy conflict losers receive structured reason. <!-- VERIFIED v2: MovementMode, sidestep/yield/regroup logic, and tactical movement regressions cover this. -->
- [x] Occupancy conflict losers do not overlap the winner. <!-- VERIFIED v2: MovementMode, sidestep/yield/regroup logic, and tactical movement regressions cover this. -->
- [x] Occupancy conflict does not weaken occupancy law silently. <!-- VERIFIED v2: MovementMode, sidestep/yield/regroup logic, and tactical movement regressions cover this. -->
- [x] Sidestep avoids known occupied tiles. <!-- VERIFIED v2: MovementMode, sidestep/yield/regroup logic, and tactical movement regressions cover this. -->
- [x] Sidestep avoids invalid terrain. <!-- VERIFIED v2: MovementMode, sidestep/yield/regroup logic, and tactical movement regressions cover this. -->
- [ ] Sidestep avoids stepping into lethal/hazardous tiles unless explicitly allowed.
- [ ] Retreat sidestep prefers increased distance from threat.
- [ ] Pursuit sidestep prefers preserving progress toward target.
- [ ] Guard movement prefers preserving guard radius.
- [ ] Intercept movement predicts target or path rather than chasing current position only.
- [x] Regroup movement prefers party anchor or leader position. <!-- VERIFIED v2: MovementMode, sidestep/yield/regroup logic, and tactical movement regressions cover this. -->
- [ ] Movement fallback is bounded to avoid infinite loops.
- [ ] Stuck counters increment only when actual movement fails.
- [ ] Stuck counters reset when meaningful movement succeeds.
- [ ] Stuck handling can trigger route invalidation.
- [ ] Stuck handling can trigger strategic replan when movement is impossible.
- [ ] Movement replay can prove no overlapping entities after conflict resolution.
- [ ] Movement replay can prove deterministic outcome under same seed.
- [x] Movement tests include one actor blocked by ally. <!-- VERIFIED v2: MovementMode, sidestep/yield/regroup logic, and tactical movement regressions cover this. -->
- [x] Movement tests include one actor blocked by enemy. <!-- VERIFIED v2: MovementMode, sidestep/yield/regroup logic, and tactical movement regressions cover this. -->
- [x] Movement tests include two actors targeting same tile. <!-- VERIFIED v2: MovementMode, sidestep/yield/regroup logic, and tactical movement regressions cover this. -->
- [ ] Movement tests include corridor congestion.
- [x] Movement tests include party regroup movement. <!-- VERIFIED v2: MovementMode, sidestep/yield/regroup logic, and tactical movement regressions cover this. -->
- [x] Movement tests include retreat under congestion. <!-- VERIFIED v2: MovementMode, sidestep/yield/regroup logic, and tactical movement regressions cover this. -->
- [x] Movement tests include pursuit under congestion. <!-- VERIFIED v2: MovementMode, sidestep/yield/regroup logic, and tactical movement regressions cover this. -->
- [ ] Movement tests include invalid terrain vs occupied terrain distinction.

## Z5. Tactical combat, targeting, and action choice consistency

- [ ] Tactical action choice uses only legal candidate actions.
- [x] Tactical action choice does not bypass movement/combat legality. <!-- VERIFIED v2: combat legality/regression and tactical tests cover this atomic law. -->
- [x] Melee attack requires valid adjacency/engagement rule. <!-- VERIFIED v2: combat legality/regression and tactical tests cover this atomic law. -->
- [x] Ranged attack requires valid range rule. <!-- VERIFIED v2: combat legality/regression and tactical tests cover this atomic law. -->
- [x] Ranged attack requires valid line-of-sight or an explicit unsupported note. <!-- VERIFIED v2: combat legality/regression and tactical tests cover this atomic law. -->
- [ ] Area attack requires valid target position.
- [ ] Area attack affects only entities inside AoE radius.
- [ ] AoE friendly-fire behavior is explicit.
- [ ] Cover behavior is explicit if ranged combat supports cover.
- [x] Weapon range affects tactical choice. <!-- VERIFIED v2: combat legality/regression and tactical tests cover this atomic law. -->
- [ ] Skill range affects tactical choice.
- [ ] Skill cost affects tactical choice.
- [x] Readiness/cooldown affects tactical choice. <!-- VERIFIED v2: combat legality/regression and tactical tests cover this atomic law. -->
- [ ] Exhaustion affects tactical choice.
- [x] Low HP affects tactical choice. <!-- VERIFIED v2: combat legality/regression and tactical tests cover this atomic law. -->
- [x] Threat level affects tactical choice. <!-- VERIFIED v2: combat legality/regression and tactical tests cover this atomic law. -->
- [ ] Target stickiness prevents unrealistic full retarget every tick.
- [ ] Target stickiness can break when target invalid/dead/out of range beyond threshold.
- [x] Disengagement has explicit consequence or safe-exit rule. <!-- VERIFIED v2: combat legality/regression and tactical tests cover this atomic law. -->
- [x] Opportunity consequences apply only under legal conditions. <!-- VERIFIED v2: combat legality/regression and tactical tests cover this atomic law. -->
- [ ] Anti-stalemate handles chase loops.
- [x] Anti-stalemate handles kite loops. <!-- VERIFIED v2: combat legality/regression and tactical tests cover this atomic law. -->
- [ ] Anti-stalemate handles repeated step-back loops.
- [ ] Anti-stalemate does not force illegal movement.
- [x] Combat result emits damage trace. <!-- VERIFIED v2: combat legality/regression and tactical tests cover this atomic law. -->
- [x] Combat result emits kill/death consequence when applicable. <!-- VERIFIED v2: combat legality/regression and tactical tests cover this atomic law. -->
- [x] Combat result emits reward/progression consequence when applicable. <!-- VERIFIED v2: combat legality/regression and tactical tests cover this atomic law. -->
- [ ] Combat result can trigger social/narrative consequence when applicable.
- [ ] Combat result can trigger strategic update when applicable.
- [x] Combat tests cover melee legality. <!-- VERIFIED v2: combat legality/regression and tactical tests cover this atomic law. -->
- [x] Combat tests cover ranged legality. <!-- VERIFIED v2: combat legality/regression and tactical tests cover this atomic law. -->
- [ ] Combat tests cover AoE legality.
- [x] Combat tests cover invalid target rejection. <!-- VERIFIED v2: combat legality/regression and tactical tests cover this atomic law. -->
- [x] Combat tests cover dead target rejection. <!-- VERIFIED v2: combat legality/regression and tactical tests cover this atomic law. -->
- [ ] Combat tests cover target stickiness break condition.
- [ ] Combat tests cover exhaustion/readiness rejection.

## Z6. Deterministic randomness and replay-stable execution

- [x] Same seed and same inputs produce same final authoritative hash. <!-- VERIFIED v2: deterministic hash/replay/RNG contract tests cover this scope; call-order independence remains separately unchecked. -->
- [ ] Same seed and same inputs produce same replay-visible trajectory.
- [ ] Different seeds produce meaningful divergence.
- [ ] Local sequential execution and worker execution produce equivalent gameplay outcomes.
- [ ] Thread scheduling order cannot change gameplay outcome.
- [ ] Work queue order cannot change gameplay outcome except through documented priority.
- [ ] RNG API supports domain separation or another proven call-order-independent scheme.
- [ ] RNG calls are scoped by deterministic context such as domain/entity/tick/sub-id or equivalent.
- [ ] Spawn randomness is isolated from tactical randomness.
- [ ] Tactical randomness is isolated from social randomness.
- [ ] Social randomness is isolated from world-event randomness.
- [ ] Economy/shop randomness is isolated from combat randomness.
- [ ] Quest generation randomness is isolated from movement randomness.
- [ ] Calamity/world-boss randomness is isolated from local action randomness.
- [ ] Adding a new actor cannot perturb unrelated actor decisions unless interaction requires it.
- [ ] Adding a new non-interacting system cannot perturb existing RNG outcomes.
- [ ] Debug/logging/presentation cannot consume gameplay RNG.
- [ ] Tests detect accidental use of global `random` in gameplay code.
- [ ] Tests detect nondeterministic set/dict ordering where it affects gameplay.
- [ ] Replay hash includes all gameplay-relevant domains.
- [ ] Replay hash excludes presentation-only fields.
- [x] Replay hash includes inventory. <!-- VERIFIED v2: deterministic hash/replay/RNG contract tests cover this scope; call-order independence remains separately unchecked. -->
- [x] Replay hash includes entities and positions. <!-- VERIFIED v2: deterministic hash/replay/RNG contract tests cover this scope; call-order independence remains separately unchecked. -->
- [x] Replay hash includes combat state. <!-- VERIFIED v2: deterministic hash/replay/RNG contract tests cover this scope; call-order independence remains separately unchecked. -->
- [x] Replay hash includes strategic state. <!-- VERIFIED v2: deterministic hash/replay/RNG contract tests cover this scope; call-order independence remains separately unchecked. -->
- [x] Replay hash includes social state. <!-- VERIFIED v2: deterministic hash/replay/RNG contract tests cover this scope; call-order independence remains separately unchecked. -->
- [x] Replay hash includes world resources. <!-- VERIFIED v2: deterministic hash/replay/RNG contract tests cover this scope; call-order independence remains separately unchecked. -->
- [x] Replay hash includes groups/parties. <!-- VERIFIED v2: deterministic hash/replay/RNG contract tests cover this scope; call-order independence remains separately unchecked. -->
- [x] Replay hash includes quests/contracts. <!-- VERIFIED v2: deterministic hash/replay/RNG contract tests cover this scope; call-order independence remains separately unchecked. -->
- [x] Replay hash includes progression. <!-- VERIFIED v2: deterministic hash/replay/RNG contract tests cover this scope; call-order independence remains separately unchecked. -->
- [x] Replay tests include repeated identical runs. <!-- VERIFIED v2: deterministic hash/replay/RNG contract tests cover this scope; call-order independence remains separately unchecked. -->
- [ ] Replay tests include sequential vs concurrent execution.
- [ ] Replay tests include save/load continuation.

## Z7. Purpose-driven group and party cooperation

- [x] Party formation can be driven by accepted social contract. <!-- VERIFIED v2: contract-backed group formation, proximity rejection, group/anchor/coordination tests cover this. -->
- [ ] Party formation can be driven by shared quest or project.
- [ ] Party formation can be driven by raid membership.
- [x] Party formation can be driven by escort/expedition/mercenary/revenge contract kind. <!-- VERIFIED v2: contract-backed group formation, proximity rejection, group/anchor/coordination tests cover this. -->
- [x] Proximity alone does not create a party unless explicitly modeled as a temporary tactical group. <!-- VERIFIED v2: contract-backed group formation, proximity rejection, group/anchor/coordination tests cover this. -->
- [ ] Temporary tactical group is distinct from contract party.
- [x] Contract party has founder/leader. <!-- VERIFIED v2: contract-backed group formation, proximity rejection, group/anchor/coordination tests cover this. -->
- [x] Contract party has member roles. <!-- VERIFIED v2: contract-backed group formation, proximity rejection, group/anchor/coordination tests cover this. -->
- [x] Contract party has shared goal. <!-- VERIFIED v2: contract-backed group formation, proximity rejection, group/anchor/coordination tests cover this. -->
- [ ] Contract party links back to the contract/obligation that created it.
- [x] Member entity links back to party/group ID. <!-- VERIFIED v2: contract-backed group formation, proximity rejection, group/anchor/coordination tests cover this. -->
- [ ] Member contract links back to party/group ID where relevant.
- [x] Party anchor follows leader or agreed anchor rule. <!-- VERIFIED v2: contract-backed group formation, proximity rejection, group/anchor/coordination tests cover this. -->
- [x] Party cohesion is updated from member positions. <!-- VERIFIED v2: contract-backed group formation, proximity rejection, group/anchor/coordination tests cover this. -->
- [x] Party cohesion affects regroup behavior. <!-- VERIFIED v2: contract-backed group formation, proximity rejection, group/anchor/coordination tests cover this. -->
- [ ] Party cohesion can trigger warnings or replan before dissolution.
- [ ] Party dissolves when leader is dead/missing according to explicit rule.
- [ ] Party dissolves when membership falls below minimum according to explicit rule.
- [ ] Party dissolves when contract is completed/abandoned/failed according to explicit rule.
- [ ] Party dissolution updates member group IDs.
- [ ] Party dissolution updates contract status when appropriate.
- [ ] Party dissolution applies social/reputation consequences when appropriate.
- [ ] Party target propagation occurs inside authoritative tick pipeline, not manual test-only invocation.
- [x] Shared target is valid and alive when assigned. <!-- VERIFIED v2: contract-backed group formation, proximity rejection, group/anchor/coordination tests cover this. -->
- [ ] Shared target clears when invalid/dead.
- [ ] Group focus fire does not override individual legality constraints.
- [x] Group behavior does not teleport or force illegal movement. <!-- VERIFIED v2: contract-backed group formation, proximity rejection, group/anchor/coordination tests cover this. -->
- [x] Group formation test covers accepted contract. <!-- VERIFIED v2: contract-backed group formation, proximity rejection, group/anchor/coordination tests cover this. -->
- [x] Group formation test covers no contract / proximity-only rejection. <!-- VERIFIED v2: contract-backed group formation, proximity rejection, group/anchor/coordination tests cover this. -->
- [ ] Group dissolution test covers dead leader.
- [ ] Group dissolution test covers scattered members.
- [ ] Group dissolution test covers contract abandonment consequence.
- [x] Group coordination test runs through normal kernel tick, not only manual system call. <!-- VERIFIED v2: contract-backed group formation, proximity rejection, group/anchor/coordination tests cover this. -->

## Z8. Strategic projects, blockers, leads, and cognition as RPG behavior

- [x] Strategic project creation is based on current needs/world state. <!-- VERIFIED v2: strategic/blocker/detour persistence/regression tests cover this atomic slice. -->
- [x] Strategic project retention is bounded by interruption resistance. <!-- VERIFIED v2: strategic/blocker/detour persistence/regression tests cover this atomic slice. -->
- [x] Strategic project switching requires margin or explicit emergency. <!-- VERIFIED v2: strategic/blocker/detour persistence/regression tests cover this atomic slice. -->
- [x] Current project has reservation priority. <!-- VERIFIED v2: strategic/blocker/detour persistence/regression tests cover this atomic slice. -->
- [ ] Current objective has continuity priority.
- [ ] Objective derivation can create executable objectives.
- [x] Objective derivation can create blockers when execution is impossible. <!-- VERIFIED v2: strategic/blocker/detour persistence/regression tests cover this atomic slice. -->
- [x] Blockers have kind. <!-- VERIFIED v2: strategic/blocker/detour persistence/regression tests cover this atomic slice. -->
- [x] Blockers have subject/reference. <!-- VERIFIED v2: strategic/blocker/detour persistence/regression tests cover this atomic slice. -->
- [x] Blockers have severity. <!-- VERIFIED v2: strategic/blocker/detour persistence/regression tests cover this atomic slice. -->
- [ ] Blockers have origin/spawned-from reference where useful.
- [ ] Blockers can be resolved by acquiring missing knowledge.
- [x] Blockers can be resolved by acquiring missing material. <!-- VERIFIED v2: strategic/blocker/detour persistence/regression tests cover this atomic slice. -->
- [ ] Blockers can be resolved by acquiring missing gold/resource.
- [ ] Blockers can be resolved by finding location/target.
- [ ] Blockers can be misdiagnosed under low cognition.
- [ ] Misdiagnosis is bounded and explainable.
- [ ] Leads have kind.
- [ ] Leads have subject.
- [ ] Leads have certainty.
- [ ] Leads have source trust.
- [ ] Leads can be tested.
- [ ] Tested bad leads are suppressed.
- [ ] Exhausted leads are not retried blindly.
- [ ] Lead retention obeys cognition profile capacity.
- [ ] Concern intake obeys cognition profile capacity.
- [x] Detour breadth obeys cognition profile capacity. <!-- VERIFIED v2: strategic/blocker/detour persistence/regression tests cover this atomic slice. -->
- [x] Detour depth obeys cognition profile capacity. <!-- VERIFIED v2: strategic/blocker/detour persistence/regression tests cover this atomic slice. -->
- [ ] Detour overflow suspends or reprioritizes project explicitly.
- [ ] Strategic overload is represented and observable.
- [x] Strategic state persists across ticks. <!-- VERIFIED v2: strategic/blocker/detour persistence/regression tests cover this atomic slice. -->
- [ ] Strategic state survives serialization.
- [x] Strategic state appears in replay/fingerprint. <!-- VERIFIED v2: strategic/blocker/detour persistence/regression tests cover this atomic slice. -->
- [ ] Strategic explanation exposes why project/objective changed.
- [ ] Strategic explanation does not mutate strategic state.
- [ ] Strategic tests include high-cognition accurate diagnosis.
- [ ] Strategic tests include low-cognition misdiagnosis.
- [x] Strategic tests include tested-lead suppression. <!-- VERIFIED v2: strategic/blocker/detour persistence/regression tests cover this atomic slice. -->
- [x] Strategic tests include detour depth overflow. <!-- VERIFIED v2: strategic/blocker/detour persistence/regression tests cover this atomic slice. -->
- [ ] Strategic tests include current-objective retention.
- [ ] Strategic tests include project-switch margin.

## Z9. Social contracts, trust, reputation, and lived consequence law

- [x] Public reputation and private relationship/bond are separate state. <!-- VERIFIED v2: social appraisal, betrayal, recruitment, and contract tests cover this atomic law. -->
- [x] Private betrayal can override public reputation. <!-- VERIFIED v2: social appraisal, betrayal, recruitment, and contract tests cover this atomic law. -->
- [ ] Familiarity changes through interaction evidence.
- [ ] Trust/sentiment changes through interaction evidence.
- [ ] Social source trust changes through fulfilled/failed information.
- [ ] Turning points persist as narrative/life-event records.
- [ ] Turning points influence later strategic/social appraisal.
- [x] Contract offer has recruiter/founder. <!-- VERIFIED v2: social appraisal, betrayal, recruitment, and contract tests cover this atomic law. -->
- [x] Contract offer has candidate. <!-- VERIFIED v2: social appraisal, betrayal, recruitment, and contract tests cover this atomic law. -->
- [x] Contract offer has terms. <!-- VERIFIED v2: social appraisal, betrayal, recruitment, and contract tests cover this atomic law. -->
- [x] Contract offer has status. <!-- VERIFIED v2: social appraisal, betrayal, recruitment, and contract tests cover this atomic law. -->
- [x] Contract appraisal uses trust/private bond. <!-- VERIFIED v2: social appraisal, betrayal, recruitment, and contract tests cover this atomic law. -->
- [x] Contract appraisal uses greed or reward preference. <!-- VERIFIED v2: social appraisal, betrayal, recruitment, and contract tests cover this atomic law. -->
- [ ] Contract appraisal uses capability/role fit.
- [x] Contract appraisal uses prior trauma/betrayal. <!-- VERIFIED v2: social appraisal, betrayal, recruitment, and contract tests cover this atomic law. -->
- [x] Accepted contract creates explicit obligation/contract state. <!-- VERIFIED v2: social appraisal, betrayal, recruitment, and contract tests cover this atomic law. -->
- [ ] Honored contract improves relevant social/reputation state.
- [ ] Broken contract worsens relevant social/reputation state.
- [ ] Abandoned contract can create betrayal/turning point where appropriate.
- [ ] Contract outcome propagates to all affected members.
- [ ] Contract outcome can affect public reputation.
- [ ] Contract outcome can affect private bonds.
- [ ] Contract outcome can affect future recruitment decisions.
- [ ] Contract outcome can affect strategic directives.
- [x] Social updates are authoritative updates, not direct mutation during appraisal. <!-- VERIFIED v2: social appraisal, betrayal, recruitment, and contract tests cover this atomic law. -->
- [ ] Social appraisal is bounded by social bandwidth/cognition where applicable.
- [x] Social tests include direct betrayal by recruiter. <!-- VERIFIED v2: social appraisal, betrayal, recruitment, and contract tests cover this atomic law. -->
- [x] Social tests include general betrayal trauma. <!-- VERIFIED v2: social appraisal, betrayal, recruitment, and contract tests cover this atomic law. -->
- [x] Social tests include high reward overcoming neutral reluctance when legal. <!-- VERIFIED v2: social appraisal, betrayal, recruitment, and contract tests cover this atomic law. -->
- [ ] Social tests include reputation distinct from private trust.
- [ ] Social tests include contract honored.
- [ ] Social tests include contract broken/abandoned.
- [ ] Social tests include party dissolution consequence if contract-backed.

## Z10. Progression, classes, skills, equipment, and growth law

- [x] XP/reward grant is authoritative and traceable to event. <!-- VERIFIED v2: progression, attribute, shop/crafting/resource transaction tests cover this. -->
- [x] Level-up thresholds are deterministic. <!-- VERIFIED v2: progression, attribute, shop/crafting/resource transaction tests cover this. -->
- [x] Attribute point grant is deterministic. <!-- VERIFIED v2: progression, attribute, shop/crafting/resource transaction tests cover this. -->
- [x] Attribute allocation checks available points. <!-- VERIFIED v2: progression, attribute, shop/crafting/resource transaction tests cover this. -->
- [x] Attribute allocation checks valid attribute name. <!-- VERIFIED v2: progression, attribute, shop/crafting/resource transaction tests cover this. -->
- [x] Attribute allocation applies aptitude multiplier or explicit divergence. <!-- VERIFIED v2: progression, attribute, shop/crafting/resource transaction tests cover this. -->
- [ ] Attribute caps are enforced.
- [ ] Effective stats recompute from base stats plus gear plus traits plus modifiers.
- [ ] Effective stats clamp to valid ranges.
- [ ] Skill definition includes ID.
- [ ] Skill definition includes type/category.
- [ ] Skill definition includes target rule.
- [ ] Skill definition includes range where relevant.
- [ ] Skill definition includes cost/cooldown where relevant.
- [ ] Physical skill scaling is defined.
- [ ] Magical skill scaling is defined.
- [ ] Elemental skill scaling is defined.
- [ ] Hybrid skill scaling is defined.
- [ ] Skill scaling uses effective stats, not raw stats, where intended.
- [ ] Passive skills apply through defined modifier path.
- [ ] Active skills require legality checks before applying effects.
- [ ] Class selection or assignment is explicit.
- [ ] Class starting gear is data-driven or documented.
- [ ] Class skill unlocks are deterministic.
- [ ] Gear equip validates slot.
- [ ] Gear equip validates ownership/inventory.
- [ ] Gear equip changes effective stats through authoritative update.
- [ ] Gear ranking logic is deterministic.
- [ ] Better gear can be selected by equipment service if that behavior is supported.
- [x] Home storage preserves items. <!-- VERIFIED v2: progression, attribute, shop/crafting/resource transaction tests cover this. -->
- [x] Shop buy checks gold before adding item. <!-- VERIFIED v2: progression, attribute, shop/crafting/resource transaction tests cover this. -->
- [x] Shop buy checks inventory capacity before subtracting gold. <!-- VERIFIED v2: progression, attribute, shop/crafting/resource transaction tests cover this. -->
- [x] Shop sell checks item exists before adding gold. <!-- VERIFIED v2: progression, attribute, shop/crafting/resource transaction tests cover this. -->
- [ ] Crafting checks recipe exists.
- [x] Crafting checks materials. <!-- VERIFIED v2: progression, attribute, shop/crafting/resource transaction tests cover this. -->
- [x] Crafting checks gold/cost. <!-- VERIFIED v2: progression, attribute, shop/crafting/resource transaction tests cover this. -->
- [x] Crafting checks inventory capacity for output. <!-- VERIFIED v2: progression, attribute, shop/crafting/resource transaction tests cover this. -->
- [x] Crafting consumes materials and adds output atomically. <!-- VERIFIED v2: progression, attribute, shop/crafting/resource transaction tests cover this. -->
- [ ] Progression replay includes XP/level/attributes/skills/gear.
- [x] Progression tests include attribute allocation. <!-- VERIFIED v2: progression, attribute, shop/crafting/resource transaction tests cover this. -->
- [ ] Progression tests include cap enforcement.
- [ ] Progression tests include skill scaling.
- [ ] Progression tests include gear equip.
- [x] Progression tests include crafting atomicity. <!-- VERIFIED v2: progression, attribute, shop/crafting/resource transaction tests cover this. -->

## Z11. World, region, spawn, calamity, and ecology law

- [ ] World seed controls world generation.
- [ ] World generation is deterministic under same seed.
- [ ] Region assignment is deterministic.
- [ ] Region topology is stable for same seed.
- [ ] Region resource placement is deterministic.
- [ ] Region difficulty/threat appraisal is deterministic.
- [ ] Spawn tables are data-driven or documented as hardcoded intentionally.
- [ ] Spawned entity loadout is valid.
- [ ] Spawned entity faction is valid.
- [ ] Spawned entity role is valid.
- [ ] Spawned entity home/leash fields are valid where needed.
- [ ] Spawn avoids invalid terrain.
- [ ] Spawn avoids occupied tile or uses conflict-safe placement.
- [ ] Camp placement is deterministic.
- [ ] Camp guards spawn near camp.
- [ ] Camp guards have leash/return behavior.
- [ ] Leash gives up chase after explicit condition.
- [ ] Return-to-camp path is legal movement, not teleport, unless intentional.
- [ ] Calamity trigger rule is deterministic.
- [ ] Calamity escalation rule is deterministic.
- [ ] Calamity consequences affect world state authoritatively.
- [ ] World boss spawn rule is deterministic.
- [ ] World boss state appears in replay/fingerprint.
- [x] Building sabotage affects building state authoritatively. <!-- VERIFIED v2: world/arena/regional/lifecycle tests cover this atomic slice. -->
- [ ] Building damage affects service availability if supported.
- [ ] Building repair restores service availability if supported.
- [x] World dynamics run even on quiet ticks where required. <!-- VERIFIED v2: world/arena/regional/lifecycle tests cover this atomic slice. -->
- [x] World dynamics do not depend on presentation/API polling. <!-- VERIFIED v2: world/arena/regional/lifecycle tests cover this atomic slice. -->
- [ ] World state survives serialization.
- [ ] World replay includes resources/buildings/regions/camps/calamities.
- [ ] World tests include same-seed generation.
- [ ] World tests include different-seed divergence.
- [ ] World tests include camp/leash behavior.
- [ ] World tests include calamity/boss behavior if supported.

## Z12. Phase guard, authorization, and mutation boundary law

- [ ] Each engine phase declares allowed read domains.
- [ ] Each engine phase declares allowed write/update domains.
- [ ] Each engine phase declares allowed emit/event domains.
- [ ] Unauthorized read is detected or explicitly allowed.
- [ ] Unauthorized write is rejected.
- [ ] Unauthorized emit is rejected or flagged.
- [ ] Phase guard cannot be disabled silently in production/certification profiles.
- [ ] Phase guard failure is structured and observable.
- [ ] Phase guard failure does not partially mutate world state.
- [ ] AI/thought phase cannot write authoritative state directly.
- [ ] Presentation phase cannot mutate authoritative state.
- [ ] Replay phase cannot mutate authoritative state.
- [ ] Metrics/logging phase cannot mutate authoritative state.
- [ ] Worker phase cannot bypass authoritative apply.
- [ ] Phase guard tests cover allowed read.
- [ ] Phase guard tests cover blocked write.
- [ ] Phase guard tests cover blocked emit.
- [ ] Phase guard tests cover no partial mutation on violation.

## Z13. Spatial index and map authority law

- [x] Spatial index can add entity/object to cell. <!-- VERIFIED v2: SpatialIndexV2/SpatialHashV2 source and contract tests cover this. -->
- [x] Spatial index can remove entity/object from cell. <!-- VERIFIED v2: SpatialIndexV2/SpatialHashV2 source and contract tests cover this. -->
- [x] Spatial index can move entity/object between cells. <!-- VERIFIED v2: SpatialIndexV2/SpatialHashV2 source and contract tests cover this. -->
- [x] Spatial index query returns current occupants. <!-- VERIFIED v2: SpatialIndexV2/SpatialHashV2 source and contract tests cover this. -->
- [x] Spatial index does not return removed occupants. <!-- VERIFIED v2: SpatialIndexV2/SpatialHashV2 source and contract tests cover this. -->
- [x] Spatial index does not duplicate moved occupants. <!-- VERIFIED v2: SpatialIndexV2/SpatialHashV2 source and contract tests cover this. -->
- [ ] Spatial index supports deterministic query ordering when order matters.
- [ ] Spatial index stays consistent with authoritative entity positions after apply.
- [ ] Spatial index update is atomic with position update.
- [x] Movement legality uses authoritative map/spatial truth. <!-- VERIFIED v2: SpatialIndexV2/SpatialHashV2 source and contract tests cover this. -->
- [x] Combat range queries use authoritative map/spatial truth where applicable. <!-- VERIFIED v2: SpatialIndexV2/SpatialHashV2 source and contract tests cover this. -->
- [ ] Perception queries use authoritative map/spatial truth where applicable.
- [ ] Spawn placement uses authoritative map/spatial truth.
- [x] Spatial index survives serialization or can be rebuilt deterministically. <!-- VERIFIED v2: SpatialIndexV2/SpatialHashV2 source and contract tests cover this. -->
- [x] Spatial tests include add/query. <!-- VERIFIED v2: SpatialIndexV2/SpatialHashV2 source and contract tests cover this. -->
- [x] Spatial tests include remove/query. <!-- VERIFIED v2: SpatialIndexV2/SpatialHashV2 source and contract tests cover this. -->
- [x] Spatial tests include move/query. <!-- VERIFIED v2: SpatialIndexV2/SpatialHashV2 source and contract tests cover this. -->
- [ ] Spatial tests include sync after movement apply.

## Z14. API, inspector, logging, and replay truth surface

- [x] API state view is derived from authoritative state. <!-- VERIFIED v2: API/presenter/replay/shutdown/degraded-mode tests cover this atomic law. -->
- [x] API state view does not mutate authoritative state. <!-- VERIFIED v2: API/presenter/replay/shutdown/degraded-mode tests cover this atomic law. -->
- [x] API exposes supported gameplay state only. <!-- VERIFIED v2: API/presenter/replay/shutdown/degraded-mode tests cover this atomic law. -->
- [ ] API marks unsupported state honestly.
- [x] Inspector reads authoritative/presenter state only. <!-- VERIFIED v2: API/presenter/replay/shutdown/degraded-mode tests cover this atomic law. -->
- [ ] Inspector does not crash on empty optional gameplay state.
- [ ] Inspector exposes cognition/strategy where supported.
- [ ] Inspector exposes social/contracts where supported.
- [ ] Inspector exposes inventory/equipment where supported.
- [ ] Inspector exposes combat/progression where supported.
- [ ] Logs are structured.
- [ ] Logs include timestamp.
- [ ] Logs include level.
- [ ] Logs include component.
- [ ] Logs include message.
- [ ] Logs include rejection reasons where relevant.
- [x] Logs include final authoritative hash at shutdown. <!-- VERIFIED v2: API/presenter/replay/shutdown/degraded-mode tests cover this atomic law. -->
- [ ] Metrics count accepted/rejected updates where relevant.
- [ ] Metrics count resource pressure rejection where relevant.
- [ ] Metrics count movement congestion where relevant.
- [ ] Metrics count unsupported behavior attempts where relevant.
- [x] Replay manifest write is atomic. <!-- VERIFIED v2: API/presenter/replay/shutdown/degraded-mode tests cover this atomic law. -->
- [x] Replay manifest failure preserves old manifest. <!-- VERIFIED v2: API/presenter/replay/shutdown/degraded-mode tests cover this atomic law. -->
- [x] Replay finalization respects timeout. <!-- VERIFIED v2: API/presenter/replay/shutdown/degraded-mode tests cover this atomic law. -->
- [ ] Replay includes enough state to debug RPG logic.
- [ ] Replay excludes presentation-only noise.
- [ ] API/inspector/logging tests include missing optional fields.
- [ ] Replay tests include corrupt/write-failure behavior.

## Z15. Safe degraded mode and infrastructure fallback law

- [x] Broker-disabled mode still runs core RPG simulation. <!-- VERIFIED v2: API/presenter/replay/shutdown/degraded-mode tests cover this atomic law. -->
- [ ] RabbitMQ disabled mode does not import/connect unexpectedly.
- [ ] Kafka disabled mode does not import/connect unexpectedly.
- [ ] Redis disabled/missing mode does not crash RPG core if optional.
- [ ] Local sequential executor preserves gameplay laws.
- [ ] Worker executor preserves gameplay laws.
- [ ] Fallback executor does not change deterministic outcome.
- [x] Shutdown suspends new work arrival. <!-- VERIFIED v2: API/presenter/replay/shutdown/degraded-mode tests cover this atomic law. -->
- [x] Shutdown flushes replay/logging within timeout. <!-- VERIFIED v2: API/presenter/replay/shutdown/degraded-mode tests cover this atomic law. -->
- [x] Shutdown emits final authoritative hash. <!-- VERIFIED v2: API/presenter/replay/shutdown/degraded-mode tests cover this atomic law. -->
- [ ] Failure in optional infrastructure does not corrupt authoritative state.
- [ ] Degraded mode is observable, not silent.
- [ ] Degraded mode tests cover brokerless execution.
- [x] Degraded mode tests cover shutdown. <!-- VERIFIED v2: API/presenter/replay/shutdown/degraded-mode tests cover this atomic law. -->
- [ ] Degraded mode tests cover optional dependency absence.

## Z16. Small but necessary default/no-op/safety laws

- [x] Empty world tick does not crash. <!-- VERIFIED v2: API/presenter/replay/shutdown/degraded-mode tests cover this atomic law. -->
- [ ] Empty world tick advances passive time if required.
- [ ] Entity with missing optional strategy state gets safe default.
- [ ] Entity with missing optional social state gets safe default.
- [ ] Entity with missing optional inventory state gets safe default or explicit invalid-state error.
- [ ] Entity with missing required combat state fails loudly before gameplay.
- [ ] No-op update preserves state hash except allowed time/metadata changes.
- [ ] Copy/clone of state is deep enough to protect authoritative state from worker mutation.
- [ ] Frozen snapshot cannot be mutated by AI proposal code.
- [ ] Serialization round-trip preserves gameplay state.
- [ ] Serialization round-trip rejects unknown critical fields unless intentionally allowed.
- [ ] Default values do not create free items/gold/XP.
- [ ] Default values do not create hidden contracts/quests.
- [ ] Default values do not create invisible movement permissions.
- [ ] Default values do not bypass capacity/caps.
- [ ] Clamp/floor/ceiling rules are explicit for HP.
- [ ] Clamp/floor/ceiling rules are explicit for stamina/readiness.
- [ ] Clamp/floor/ceiling rules are explicit for reputation/trust if bounded.
- [ ] Clamp/floor/ceiling rules are explicit for cognition metrics if bounded.
- [ ] Clamp/floor/ceiling rules are explicit for inventory weight/slots.
- [ ] Safe fallback tests cover empty state.
- [ ] Safe fallback tests cover no-op update.
- [ ] Safe fallback tests cover copy isolation.
- [ ] Safe fallback tests cover serialization round-trip.
